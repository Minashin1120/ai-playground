...
Androidでは、履歴ドロワーの「バブルで開く」からバブルを利用できます。Android 17以上は通知バブルではなく、`resizeableActivity` / `allowEmbedded` / `documentLaunchMode="always"` を備えたアプリをLauncherのアイコン長押しからユーザーがアプリバブルに追加する仕様のため、その手順を表示します。Android 10〜16では現在選択中のチャットを会話通知・バブル通知・会話ショートカットで登録し、バブルを展開すると既存の `MainActivity` を表示します。Android 8・9では通常通知として動作します。Android 13以上で通知権限が未許可の場合は権限要求を行い、Android 10〜16でOSのバブル設定が無効な場合はAI Playgroundの通知設定を開きます。バブルを閉じた状態でのストリーミング継続や常駐Serviceは対象外です。
...
- GCMの保存済みIVを復号に使用できるよう、Android Keystore鍵では呼び出し側IVを許可し、鍵別名の変更で既存の不適合鍵と混在しないようにする。
...
表示済みのスレッド一覧・メッセージ・ファイルライブラリ情報と、実際に表示または開いた添付ファイルは、アカウントIDごとに暗号化して `context.noBackupFilesDir` 配下へ保存します。Androidのシステムキャッシュ削除対象である `cacheDir` には保存しません。暗号化鍵はAndroid Keystoreで管理し、履歴とファイルのキャッシュは設定からカテゴリ単位で削除できます。明示的なログアウトではキャッシュを保持しますが、ログアウト後はアカウントを再連携するまで表示しません。

履歴キャッシュの初期値は「表示済み部分のみ」、通信条件の初期値は「Wi‑Fiのみ」です。「全件同期」を選択した場合は起動・復帰時に自動同期し、設定から手動同期とキャンセルも実行できます。モバイルデータ通信の利用は設定で許可した場合だけキャッシュ同期に使います。未取得の履歴や未表示のファイル本体はオフラインでは利用できません。送信、アップロード、サーバー上の変更、Batch、Realtimeなどの通信依存操作はオフライン中に無効化します。
...
通信表示は「送信中」「応答待ち」「受信中」など実際に観測した状態から決めます。`finally` で待機表示を解除します。Web側の共通スピナーは `static/js/progress_spinner.js` が管理しており、Androidは自身のViewModelで同じ考え方を実装します。接続状態は `/api/version?heartbeat=...` を5秒間隔（切断系は2秒間隔）で監視し、HTTP 503をメンテナンス、502／504／520〜524をサーバー停止、2秒以上の応答が3回続いた場合を不安定として表示します。ネットワーク復帰時は復帰表示を5秒間出し、送信・アップロード・Realtimeなど長時間処理中は監視を抑制します。
...
| BAN・Turnstile | BAN応答を受けたらWeb版と同様に専用画面へ切り替え、チャット・設定・アップロード・高度な機能を操作できない状態にする。Web版の詳細／異議申し立て導線とログアウトは利用できる |
| アプリ内更新 | GitHubページを開かず、進捗・キャンセル・再試行・SHA-256検証・標準インストールが動作する |
| 更新インストール許可 | Android設定へ案内し、許可後にインストーラーを再開する。拒否時はAPKをインストールしない |
