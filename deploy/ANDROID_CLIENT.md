...
Androidでは、履歴ドロワーの「バブルで開く」から、現在選択中のチャットをAndroidのバブルとして登録できます。バブルを展開すると既存の `MainActivity` を表示し、スレッドIDは端末内のIntentだけで渡します。Android 10以上では会話通知・バブル通知・会話ショートカットを使用し、Android 8・9では通常通知として動作します。通知権限が未許可の場合は権限要求を行い、OSのバブル設定が無効な場合はAI Playgroundの通知設定を開きます。バブルを閉じた状態でのストリーミング継続や常駐Serviceは対象外です。
