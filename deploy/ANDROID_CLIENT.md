...
Androidでは、履歴ドロワーの「バブルで開く」から、現在選択中のチャットをAndroidのバブルとして登録できます。バブルを展開すると既存の `MainActivity` を表示し、スレッドIDは端末内のIntentだけで渡します。Android 10以上ではバブル通知と会話ショートカットを使用し、Android 8・9では通常通知として動作します。通知権限またはOSのバブル設定が無効な場合も、通常通知からアプリを開けます。バブルを閉じた状態でのストリーミング継続や常駐Serviceは対象外です。
