...
app = Flask(__name__)
app.config['APP_VERSION'] = os.getenv('APP_VERSION', '2026-04-19-005')
...
def _build_rich_paste_pdf_bytes_plain(title, content_html, created_at=None):
    from bs4 import BeautifulSoup, NavigableString, Tag
    from reportlab import rl_config
    from reportlab.lib import colors
    from reportlab.lib.enums import TA_LEFT
    ...
    def paragraph_style(name, font_size=10.5, leading=15, bold=False, italic=False, color="#111827", space_after=6, left_indent=0):
        return ParagraphStyle(
            name,
            parent=styles["BodyText"],
            fontName=bold_font if bold else base_font,
    ...
    title_style = paragraph_style("RichPastePlainTitle", font_size=18, leading=23, bold=True, color="#0f172a", space_after=10)
    meta_style = paragraph_style("RichPastePlainMeta", font_size=9, leading=12, color="#64748b", space_after=12)
    body_style = paragraph_style("RichPastePlainBody", font_size=10.5, leading=15, color="#111827", space_after=7)
    heading_styles = {
        1: paragraph_style("RichPastePlainH1", font_size=16, leading=20, bold=True, color="#0f172a", space_after=8),
        2: paragraph_style("RichPastePlainH2", font_size=14, leading=18, bold=True, color="#0f172a", space_after=8),
        3: paragraph_style("RichPastePlainH3", font_size=12.5, leading=16, bold=True, color="#0f172a", space_after=7),
        4: paragraph_style("RichPastePlainH4", font_size=11.5, leading=15, bold=True, color="#0f172a", space_after=6),
        5: paragraph_style("RichPastePlainH5", font_size=10.8, leading=14, bold=True, color="#0f172a", space_after=6),
        6: paragraph_style("RichPastePlainH6", font_size=10.5, leading=14, bold=True, color="#0f172a", space_after=6),
    }
    quote_style = paragraph_style("RichPastePlainQuote", font_size=10.2, leading=15, italic=True, color="#334155", space_after=4, left_indent=6)
    code_style = ParagraphStyle(
        "RichPastePlainCode",
        parent=styles["Code"],
        fontName=mono_font,
    ...
    list_style = paragraph_style("RichPastePlainList", font_size=10.5, leading=15, color="#111827", space_after=3)
    table_cell_style = paragraph_style("RichPastePlainTableCell", font_size=9.3, leading=12.5, color="#111827", space_after=0)
    note_style = paragraph_style("RichPastePlainNote", font_size=9.2, leading=12.2, color="#64748b", space_after=4)
    ...
    def add_paragraph_text(text, style):
        clean = normalize_text(text)
        if not clean:
            return
        story.append(Paragraph(esc(clean).replace("\n", "<br/>"), style))

    def add_blockquote_text(text):
        clean = normalize_text(text)
        if not clean:
            return
        para = Paragraph(esc(clean).replace("\n", "<br/>"), quote_style)
        box = Table([[para]], colWidths=[available_width])
        box.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#fff9eb")),
            ("LEFTPADDING", (0, 0), (-1, -1), 10),
    ...
    def add_image(node):
        ...
        try:
            ...
            image.hAlign = "CENTER"
            try:
                image._restrictSize(available_width, 150 * mm)
            except Exception:
                pass
            story.append(image)
            caption = normalize_text(node.get("alt") or "")
    ...
    def flatten_inline(node):
        ...
        if tag_name == "img":
            ...
            return alt or "[image]"
        parts = [flatten_inline(child) for child in node.children]
        return "".join(parts)

    def add_list(list_node, ordered=False):
        items = list_node.find_all("li", recursive=False)
        for idx, li in enumerate(items, start=1):
            item_text = normalize_text(flatten_inline(li), preserve_newlines=True)
            if not item_text:
                continue
            prefix = f"{idx}." if ordered else "-"
            add_paragraph_text(f"{prefix} {item_text}", list_style)
    ...
    def add_table(table_node):
        ...
        for tr in tr_nodes:
            ...
            if any((cell.name or "").lower() == "th" for cell in cell_nodes) and header_rows == 0:
                header_rows = 1
            for cell in cell_nodes:
                cell_text = normalize_text(flatten_inline(cell), preserve_newlines=True) or " "
                cells.append(Paragraph(esc(cell_text).replace("\n", "<br/>"), table_cell_style))
            rows.append(cells)
        if not rows:
            return
        col_count = max(len(r) for r in rows)
    ...
    def add_pre(node):
        ...
        if not text:
            return
        story.append(Preformatted(text, code_style, dedent=0))
    ...
    def render_node(node):
        ...
        if isinstance(node, NavigableString):
            text = normalize_text(str(node))
            if text:
                add_paragraph_text(text, body_style)
            return
        if not isinstance(node, Tag):
            return
        tag_name = (node.name or "").lower()
        ...
        if tag_name in {"h1", "h2", "h3", "h4", "h5", "h6"}:
            level = int(tag_name[1])
            add_paragraph_text(flatten_inline(node), heading_styles.get(level, body_style))
            return
        if tag_name == "p":
            add_paragraph_text(flatten_inline(node), body_style)
            return
        if tag_name in {"div", "section", "article", "main", "header", "footer", "aside"}:
            child_tags = [child for child in node.children if isinstance(child, Tag)]
            if not child_tags:
            ...
            return
        if tag_name == "blockquote":
            add_blockquote_text(flatten_inline(node))
            return
        if tag_name == "pre":
            add_pre(node)
            return
        ...
        if tag_name == "figure":
            ...
            caption = node.find("figcaption")
            if caption:
                add_paragraph_text(flatten_inline(caption), note_style)
            return
        if tag_name in {"ul", "ol"}:
            add_list(node, ordered=(tag_name == "ol"))
            return
        if tag_name == "table":
            add_table(node)
            return
        text_markup = flatten_inline(node)
        if text_markup.strip():
            add_paragraph_text(text_markup, body_style)
    ...
    if not any(isinstance(item, (Paragraph, Preformatted, Table, Image, HRFlowable, Spacer)) for item in story):
        story.append(Paragraph("内容がありません。", body_style))
...
def rich_paste_pdf():
    ...
    try:
        ...
        logger.exception("Server-side rich paste PDF generation failed (rich renderer)")
        try:
            pdf_bytes = _build_rich_paste_pdf_bytes_plain(title, content_html, created_at=created_at)
            logger.info("Server-side rich paste PDF fallback renderer succeeded")
        except Exception as fallback_error:
            logger.exception("Server-side rich paste PDF generation failed (plain fallback)")
            return jsonify({'error': 'pdf_generation_failed', 'message': str(fallback_error)}), 500
