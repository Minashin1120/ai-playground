...
| `models.py` | SQLAlchemy モデル（`User`, `Thread`, `Message`, `GeminiBatchJob`, `Gem`、セッション、BAN 等）。`mcp_service` のモデルもここで `db` に載せる | カラム追加、ユーザー設定、チャット保存形式、Batch状態 |
...
| `background.py` | RQ のチャット生成本体、Gemini Batch送信、E2EE 移行、Coding Mode、ストリームの Redis 蓄積 | 生成ジョブ、Batch送信、ストリーム、コーディングモード |
...
| `routes_chat.py` | `/chat_stream`、Gemini Batch状態取得、トークン見積、停止、一時チャット heartbeat、タイトル生成 | 通常のテキストチャット送信、Batch状態更新 |
