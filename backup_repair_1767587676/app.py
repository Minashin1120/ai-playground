...
def migrate_e2ee_task(user_id, enable):
    with app.app_context():
        ...
        try:
            ...
            if enable and user.system_prompt and not user.enable_e2ee: user.system_prompt = encrypt_val(user.system_prompt)
            elif not enable and user.system_prompt and user.enable_e2ee: user.system_prompt = decrypt_val(user.system_prompt)
    ...
    with app.app_context():
        ...
        try:
            ...
            if is_gem:
                ...
                if "nano" not in model_key:
                    if options.get('enable_thinking'):
                        lvl = options.get('thinking_level', 'high').upper()
                        if lvl not in ['MINIMAL', 'LOW', 'MEDIUM', 'HIGH']: lvl = 'HIGH'
                            ...
                            for part in cand.content.parts:
                                ...
                                if hasattr(part, 'inline_data') and part.inline_data:
                                    try:
                                        ...
                                        else: Image.open(BytesIO(part.inline_data.data)).save(fp2)
            ...
            # --- xAI Grok (Native SDK) ---
                        ...
                        if m['image_url']:
                            try:
                                for h_img in json.loads(m['image_url']):
                                    ...
                                    ep2 = bp2 + '.enc'
                                    d2 = None
                                    if os.path.exists(bp2): with open(bp2, 'rb') as f: d2 = f.read()
                                    elif os.path.exists(ep2): with open(ep2, 'rb') as f: d2 = decrypt_bytes(f.read())
                                    if d2:
                ...
                stream = chat_session.stream(tools=tools if tools else None)
                for chunk in stream:
                    if chunk.content:
            ...
            # --- OpenAI / Fallback ---
                ...
                for m in history:
                    ...
                    if m['image_url']:
                        try:
                            img_arr = json.loads(m['image_url'])
                            if img_arr:
                                content_list = [{"type": "text", "text": m['content']}]
                                for h_img in img_arr:
                                    bp2 = os.path.join(app.config['UPLOAD_FOLDER'], h_img)
                                    ep2 = bp2 + '.enc'
                                    d2 = None
                                    if os.path.exists(bp2): with open(bp2, 'rb') as f: d2 = f.read()
                    ...
                    msgs.append({"role": m['role'], "content": content_block})
                ...
                if options.get('enable_search') and not is_grok: kwargs['tools'] = [{"type": "web_search"}]
                if options.get('reasoning_effort') and not is_grok: kwargs['reasoning_effort'] = options.get('reasoning_effort')
                if not is_grok: kwargs['max_completion_tokens'] = 4096
                ...
                for chunk in stream:
                    ...
                    if r_content is None and hasattr(delta, 'model_extra') and delta.model_extra:
                        r_content = (delta.model_extra.get('reasoning_content') or delta.model_extra.get('thinking'))
                    if r_content:
...
def chat_stream():
    ...
    # FIXED: Save User Message (Persistence)
    try:
