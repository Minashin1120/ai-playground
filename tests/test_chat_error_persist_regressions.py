...
class ChatErrorPersistRegressionTests(unittest.TestCase):
    ...
    def test_version_assets_include_chat_error_support(self):
        ...
        self.assertTrue(assets[0].name.endswith(".757.js"))
        app_source = (APP_ROOT / "app.py").read_text(encoding="utf-8")
        self.assertIn("SYSTEM_VERSION'] = 'V4.8.757'", app_source)
        self.assertIn("2026-08-07-008", app_source)
        ...
        self.assertTrue(re.search(r"v4\.8\.757", js[0].name))
        self.assertTrue(re.search(r"v4\.8\.757", css[0].name))
        self.assertTrue(re.search(r"v4\.8\.757", tw[0].name))
