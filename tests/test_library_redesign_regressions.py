...
class LibraryRedesignRegressionTests(unittest.TestCase):
    ...
    def test_library_client_loads_pages_instead_of_retrying_full_list(self):
        ...
        self.assertIn("const LIBRARY_PAGE_SIZE = 40", script)
        self.assertIn("limit: String(LIBRARY_PAGE_SIZE)", script)
        self.assertIn("offset: String(offset)", script)
        self.assertIn("loadLibraryFiles(true)", script)
        self.assertIn("if (loadMore && !lib.hasMore) return", script)
        self.assertIn("requestSucceeded", script)
        self.assertIn("追加読み込みに失敗しました。もう一度お試しください。", script)
