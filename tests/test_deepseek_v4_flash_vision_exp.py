...
class DeepSeekV41FlashVisionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_model_entry_is_registered_with_metadata(self):
        self.assertRegex(
            self.js_source,
            re.compile(
                r'id:\s*"deepseek-v4\.1-flash"[^}]*implementedAt:\s*"2026-09-13"'
                r'[^}]*implementedRank:\s*9600[^}]*name:\s*"DeepSeek V4\.1 Flash"'
            ),
        )
        self.assertIn('<option value="deepseek-v4.1-flash">DeepSeek V4.1 Flash</option>', self.setup_source)
        self.assertIn("'deepseek-v4.1-flash'", self.setup_source)
    ...
    def test_backend_sends_images_natively_for_vision_exp(self):
        ...
        # Native vision flag is derived from the official V4.1 Flash model ID.
        self.assertIn('deepseek_native_vision = model_key_l in {"deepseek-v4.1-flash", "deepseek-v4-flash-vision-exp"}', build)
    ...
    def test_effort_mapping_and_ui_follow_flash_family(self):
        self.assertIn('"deepseek-v4.1-flash", "deepseek-v4-flash-0731", "deepseek-v4-flash", "deepseek-v4-flash-vision-exp"}', self.app_source)
    ...
    def test_input_limits_and_vision_notice_reflect_native_vision(self):
        self.assertIn("model === 'deepseek-v4.1-flash'", self.js_source)
        self.assertIn("DeepSeek V4.1 Flash 入力制限", self.js_source)
        self.assertIn("JPEG・PNG・GIF・WebP / 画像1枚あたり最大32MB / リクエスト合計48MB", self.js_source)
        self.assertIn("uploadModelLower === 'deepseek-v4.1-flash'", self.js_source)
        self.assertIn("vmi.classList.toggle('hidden', modelLower === 'deepseek-v4.1-flash'", self.js_source)
