var di=Object.defineProperty;var a=(e,t)=>di(e,"name",{value:t,configurable:!0});const get=a(e=>document.getElementById(e),"get"),nativeConsoleLog=typeof console.log=="function"?console.
...
se=l.get(F)||{color:E,weight:0};se.weight+=L,l.set(F,se),u+=L}}if(!!(String(w.style.backgroundColor||
...
pageSize.getHeight(),x=794,L=Math.floor(w/b*x),T=g.scrollHeight||g.offsetHeight;let E=0,F=!0;const se=Math.
...
p&&h&&(h.textContent=`${Number(p.value||1).toFixed(2)}x`)}a(updateTtsUi,"updateTtsUi");function getModelTags(e,t){
...
getBoundingClientRect(),f=c.getBoundingClientRect(),m=f.left+f.width/2-(d.left+d.width/2),y=f.top+f.
...
classList.add("hidden"):f.classList.remove("hidden"))}a(o,"updateGptImageUi");function r(){const c=get(
...
.0",y=d==="grok-imagine-image-2.0";if(f){c.classList.remove("hidden");const _=get("grok-image-resolu\
...
"hidden",!y)}}a(l,"updateGrokImageUi");function u(){var m;const c=get("grok-video-options");if(!c)return;
const d=String(((m=get("model-select"))==null?void 0:m.value)||"").toLowerCase();isGrokVideoModel()?
c.classList.remove("hidden"):c.classList.add("hidden");const f=get("grok-video-resolution");if(f){const y=Array.
from(f.options).find(k=>k.value==="1080p");y&&(y.disabled=d!=="grok-imagine-video-1.5"),d!=="grok-im\
agine-video-1.5"&&f.value==="1080p"&&(f.value="720p")}}a(u,"updateGrokVideoUi");function p(){var m;const c=get(
"gemini-video-options");if(!c)return;const d=String(((m=get("model-select"))==null?void 0:m.value)||
"").toLowerCase();isGeminiVideoModel()?c.classList.remove("hidden"):c.classList.add("hidden");const f=get(
...
k),k&&f.value==="4K"&&(f.value="1080p")}}a(p,"updateGeminiVideoUi");function h(){const c=get("gemini\
-music-options");if(!c)return;const d=isGeminiRealtimeMusicModel(),f=isGeminiMusicModel()&&!d;c.classList.
toggle("hidden",!f);const m=get("lyria-realtime-studio-bar");m&&m.classList.toggle("hidden",!d)}a(h,
...
d)&&!isGrokVideoModel(d)&&!d.includes("voice");c.classList.toggle("hidden",!f);const m=get("xai-logp\
...
istral-ocr-options");d&&d.classList.toggle("hidden",!c);const f=get("modal-mistral-ocr-options");f&&
f.classList.toggle("hidden",!c),["canvas-mode-container","coding-mode-container","browser-fast-mode-\
container"].forEach(m=>{const y=get(m);y&&(y.classList.toggle("opacity-50",c),y.classList.toggle("po\
...
s");if(!c)return;const d=(get("model-select").value||"").toLowerCase();let f="",m=!1;d.includes("gpt\
...
join(""))),m?(c.innerHTML=f,c.classList.remove("hidden")):(c.classList.add("hidden"),c.innerHTML="")}
...
o(),r(),l(),u(),p(),h(),g(),v(),b(),purgeUnsupportedAttachments(!0),refreshMinimalOptionsIfOpen()}a(
...
use=="function"&&window.marked.use({renderer:{code(c,d,f){const m=(d||"").match(/\S*/)[0];if(m==="py\
exec")return"";if(m==="chat_error")return buildChatErrorBubbleHtml(c||"");const y=c||"",k=(m||"").toLowerCase();
...
-${m}">${_}</code></pre></div></div>`},link(c,d,f){return`<a href="${c}" title="${d||""}" target="_b\
lank">${f}</a>`},image(c,d,f){const m=escapeHtml(f||""),y=d?` title="${escapeHtml(d)}"`:"";if(String(
c||"").startsWith("sandbox:"))return`<span class="text-xs text-gray-500" title="${escapeHtml(c)}">${m||
"\uFF08\u753B\u50CF\u30C7\u30FC\u30BF\u306F\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\uFF09"}\
</span>`;const k=escapeHtml(c||"");return`<img src="${c}" data-viewer-src="${k}" alt="${m}"${y} clas\
...
ionTime"),se=a(c=>{const d=Array.isArray(c)?c:[],f=get("passkey-list"),m=get("passkey-count");if(m&&
...
_)),S.appendChild(G),f.appendChild(S)})}},"renderPasskeyList"),Y=a(c=>{const d=get("session-list");if(d){
if(!c||!c.length){d.innerHTML='<div class="text-xs text-gray-500">\u30A2\u30AF\u30C6\u30A3\u30D6\u306A\u30BB\u30C3\u30B7\u30E7\u30F3\u306F\u3042\u308A\u307E\u305B\u3093\u3002</div>';
return}d.innerHTML=c.map(f=>{const m=f.is_current?'<span class="text-[10px] bg-blue-600 text-white p\
...
return}const m=(f.sessions||[]).filter(y=>!y.is_revoked);Y(m)},"loadSessions"),O=a(()=>{const c=get(
...
c.default_vision_model||"gemini-3-flash-preview"),applyChatDefaults(c),c&&c.theme_color&&applyThemeColor(
c.theme_color,!0),c&&Object.prototype.hasOwnProperty.call(c,"minimal_prompt_mode")&&c.minimal_prompt_mode?
setMinimalPromptMode(!0):c&&Object.prototype.hasOwnProperty.call(c,"compact_prompt_mode")&&setCompactPromptMode(
!!c.compact_prompt_mode),get("set-client-debug-log")&&syncClientDebugLogToggle(c.enable_client_debug_log===
!0,"settings sync");const d=get("enable-sys-prompt");d&&c&&c.system_prompt&&String(c.system_prompt).
trim()&&(!d.disabled&&!c.default_enable_system_prompt&&!c.use_last_chat_settings&&(d.checked=!0),w())}).
...
(Z.onchange=()=>{const c=Z.checked,d=get("compression-max-size"),f=get("compression-max-dim");d&&(d.
disabled=c),f&&(f.disabled=c);const m=get("compression-size-wrap"),y=get("compression-dim-wrap");m&&
(m.style.opacity=c?"0.4":"1"),y&&(y.style.opacity=c?"0.4":"1")});const pe=a(()=>{const c=get("enable\
-temporary-chat");!c||c.dataset.bound==="1"||(c.dataset.bound="1",c.checked=!!temporaryChatEnabled,c.
onchange=async()=>{const d=temporaryChatEnabled;await applyTemporaryChatSetting(c.checked)||(setTemporaryChatUiState(
...
h");oe&&(oe.onclick=()=>loadStorageUsage());let ce=null;const xe=a(()=>{const c=new Uint8Array(16);return window.
crypto.getRandomValues(c),Array.from(c,d=>d.toString(16).padStart(2,"0")).join("")},"createAccountTr\
ansferId"),X=a((c={})=>{const d=get("account-transfer-progress"),f=get("account-transfer-progress-ba\
...
"hidden"),f&&(f.style.width=`${_}%`),m&&(m.textContent=`${Math.round(_)}%`),y&&(y.textContent=c.message||
...
ed","cancelled","expired"].includes(c.phase))},"renderAccountTransferProgress"),ke=a(c=>{H&&(H.disabled=
!!c);const d=get("account-import-btn");d&&(d.disabled=!!c);const f=get("account-transfer-cancel-btn");
f&&(f.disabled=!c)},"setAccountTransferControls"),K=a((c={})=>{const d=get("account-export-ready"),f=get(
"account-export-ready-text"),m=get("account-export-expiry"),y=get("account-export-download-btn"),k=!!(c.
...
GB`:`${(_/(1024*1024)).toFixed(1)} MB`;if(f){const M=Number(c.unreadable_count)>0?`\uFF08\u8AAD\u53D6\u4E0D\u80FD ${Number(
c.unreadable_count)}\u4EF6\u3092\u5FA9\u65E7\u7528\u3068\u3057\u3066\u53CE\u9332\uFF09`:"";f.textContent=
`\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8ZIP\u3092\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u3067\u304D\u307E\u3059\uFF1A${S}${M}`}
if(m){const M=c.expires_at?new Date(c.expires_at):null;m.textContent=M&&!Number.isNaN(M.getTime())?`\
\u4FDD\u5B58\u671F\u9650\uFF1A${M.toLocaleString()}\uFF08\u671F\u9650\u5F8C\u306B\u81EA\u52D5\u524A\u9664\uFF09`:
"\u5B8C\u6210\u304B\u30891\u6642\u9593\u5F8C\u306B\u81EA\u52D5\u524A\u9664\u3055\u308C\u307E\u3059\u3002"}
y&&(y.href=c.download_url)},"renderAccountExportAvailability"),W=a(async c=>{for(;ce===c&&!c.stopped;){
...
return null},"pollAccountTransfer"),C=a((c,d,f=!0)=>{d&&(X(d),K(d),f&&d.state==="ready"?showToast(d.
...
f);m&&C(f,m,!0)}catch{}},"refreshLatestAccountExport"),q=a(c=>{ce===c&&(ce=null),c.stopped=!0,ke(!1)},
"finishAccountTransfer"),Q=get("account-transfer-cancel-btn");Q&&(Q.onclick=async()=>{const c=ce;if(!(!c||
...
"info")}});const H=get("account-export-btn");H&&(H.onclick=async()=>{if(ce)return;const c={id:xe(),type:"\
export",stopped:!1};ce=c,ke(!0),K({available:!1}),X({progress:0,phase:"queued",message:"\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u3092\u53D7\u3051\u4ED8\u3051\u3066\
...
f=await d.json().catch(()=>({}));if(d.status===409&&f.error==="export_in_progress"&&f.job_id)c.id=f.
...
const m=await W(c);!c.cancelRequested&&m&&C(c,m,!0)}catch(d){const f=d&&d.message?d.message:"\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u3092\
\u958B\u59CB\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F";X({progress:0,phase:"failed",message:f}),
showToast(f,"error",!0),q(c)}});const te=get("account-export-download-btn");te&&te.addEventListener(
"click",async c=>{const d=te.getAttribute("href");if(!(!d||d==="#")){c.preventDefault();try{const f=await apiFetch(
...
"import-files-info"),Fe=get("import-files-summary"),je=a(c=>{const d=Math.max(0,Number(c)||0);return d>=
...
return;const c=Se.files,d=Se.selection;let f=0;c.forEach(k=>{d.has(k.archive_path)&&(f+=Number(k.size_bytes)||
0)});const m=Number(Se.available_bytes)||0,y=f>m;Fe&&(Fe.textContent=`\u9078\u629E\u4E2D: ${je(f)} /\
 \u5229\u7528\u53EF\u80FD: ${je(m)}${y?" \uFF08\u5BB9\u91CF\u8D85\u904E\uFF09":""}`,Fe.classList.toggle(
"text-red-300",y)),le&&(le.textContent=`${c.length} files`)},"updateImportFileSelectionUi"),Qe=a(()=>{
if(!D||!Se)return;D.innerHTML="";const c=Se.files;if(!c.length){D.innerHTML='<div class="text-xs tex\
t-gray-500">\u30A4\u30F3\u30DD\u30FC\u30C8\u53EF\u80FD\u306A\u30D5\u30A1\u30A4\u30EB\u304C\u3042\u308A\u307E\u305B\u3093\u3002</div>',
st();return}c.forEach(d=>{const f=document.createElement("label"),m=Se.selection.has(d.archive_path);
...
"border-gray-600",!y.checked),st()}),D.appendChild(f)}),st()},"renderImportFileItems"),ft=a(c=>new Promise(
d=>{if(Se={files:c.files||[],selection:new Set((c.files||[]).map(f=>f.archive_path)),available_bytes:c.
...
Se=null,d(c)}},"closeImportFileSelection"),mt=get("import-files-close");mt&&(mt.onclick=()=>at(null));
...
Confirmation"),gt=a(c=>new Promise(d=>{if(!get("settings-confirmation-modal")){d(!0);return}_t=d;const m=Array.
isArray(c&&c.settings_changes)?c.settings_changes:[],y=get("settings-confirmation-list");y&&(m.length?
...
ze){const c=a(()=>ze.classList.toggle("hidden",!Ce.checked),"syncInplaceWarn");Ce.addEventListener("\
...
l");if(we&&we.classList.contains("modal-open"))try{An(Ee)}catch{}Ee.theme_color&&applyThemeColor(Ee.
...
"needs_settings_confirmation"&&Array.isArray(me.settings_changes))return await gt({settings_changes:me.
...
Array.isArray(re.settings_changes)){if(!await gt(re)){X({progress:0,phase:"cancelled",message:"\u8A2D\u5B9A\u306E\u30A4\u30F3\
...
"showDedupeResult");He&&(He.onclick=async()=>{const c=a(async()=>{const d=await apiFetch("/api/accou\
...
let d="/api/encryption_scan";c&&(d+=`?thread_id=${encodeURIComponent(c)}`);try{const f=await apiFetch(
...
list");let Te=null,$e=!1;const Ke=a(c=>!c||!c.length?null:c.some(d=>!!d.is_encrypted),"computeThread\
EncryptedFromMessages"),en=a(()=>{Te=Ke(allMessages)},"refreshCurrentThreadEncStateFromMessages"),Vt=a(
async(c,d,{confirmPrompt:f=!0,reloadCurrent:m=!0}={})=>{if(!c)return showToast("\u30C1\u30E3\u30C3\u30C8\u304C\u3042\u308A\u307E\u305B\u3093",
...
c)&&await loadMessages(currentThreadId,{preserveDraft:!0,silent:!0,skipHistory:!0}),et&&await de(),!0):
...
de,window.__refreshAdminThreadEncState=en,window.__setAdminThreadEncryption=Vt;const he=get("encrypt\
...
return}const f=c.target.closest(".admin-enc-toggle");if(!f||$e)return;const m=f.getAttribute("data-i\
...
f.disabled=!1,f.textContent=_,await de()}}),get("file-input").onchange=c=>{const d=Array.from(c.target.
files||[]);c.target.value="",d.length&&handleFiles(d)},get("photo-input")&&(get("photo-input").onchange=
c=>{const d=Array.from(c.target.files||[]);c.target.value="",d.length&&handleFiles(d)});const Ve=a(c=>{
const d=get("ban-appeal-list");if(d){if(!c||!c.length){d.innerHTML='<div class="text-[11px] text-gra\
y-500">\u73FE\u5728\u3001\u7533\u3057\u7ACB\u3066\u306F\u3042\u308A\u307E\u305B\u3093\u3002</div>';return}
d.innerHTML=c.map(f=>{const m=f.status||"new",y=f.admin_read_at?'<span class="text-[10px] text-gray-\
...
ok)return;const y=(await f.json()).unread_count||0;d.textContent=String(y),c&&y>0&&showToast(`BAN\u7570\u8B70\u7533\
...
\u9664\u3055\u308C\u307E\u3059\u3002</div>
                ...
                `,c.appendChild(d)},"ensureTemporaryChatSettingsCard"),Ln=a(()=>{const c=get("set-st\
...
"success")})},"ensureLlmTranscribePromptSettingsUi"),tn=[{key:"python",label:"Python \u5B9F\u884C\u6848\u5185"},
...
buildAutoSystemPromptRows=(c,d=!1)=>{const f=d?"w-full h-14 bg-gray-950 border border-gray-700 round\
ed p-2 text-[11px] text-gray-200":"w-full h-20 bg-gray-950 border border-gray-700 rounded p-2 text-x\
s text-gray-200";return tn.map(m=>`
                    <div class="rounded border border-gray-700 p-2 bg-gray-950/40">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <label class="flex items-center gap-1 text-[10px] text-gray-500">
                                <input type="checkbox" id="${c}-auto-sys-${m.key}-enabled" class="ac\
cent-yellow-500 w-3 h-3">
                                <span>\u9069\u7528</span>
                            </label>
                        </div>
                        <textarea id="${c}-auto-sys-${m.key}-text" class="${f}" placeholder="\u81EA\u52D5\u6CE8\u5165\u6587\u8A00"\
></textarea>
                        ${m.hint?`<div class="text-[10px] text-gray-500 mt-1">${m.hint}</div>`:""}
                    </div>
                `).join("")},window.applyAutoSystemPromptConfigToForm=(c,d={})=>{tn.forEach(f=>{const m=d&&
typeof d=="object"?d[f.key]||{}:{},y=get(`${c}-auto-sys-${f.key}-enabled`),k=get(`${c}-auto-sys-${f.
key}-text`);y&&(y.checked=m.enabled!==!1),k&&(k.value=m.text||"",k.placeholder=m.default_text||"\u81EA\u52D5\u6CE8\u5165\
\u6587\u8A00")})};const Mn=a((c,d=null)=>{if(d){const f=get(d);f&&(f.checked=!0)}tn.forEach(f=>{const m=get(
`${c}-auto-sys-${f.key}-enabled`),y=get(`${c}-auto-sys-${f.key}-text`);if(m&&(m.checked=!0),y){const k=y.
placeholder||"";y.value=k}})},"resetAutoSystemPromptConfigToCodeDefaults"),Zn=a(c=>{const d={};return tn.
forEach(f=>{const m=get(`${c}-auto-sys-${f.key}-enabled`),y=get(`${c}-auto-sys-${f.key}-text`);d[f.key]=
...
"set",!1)}</div>
                ...
                `,d.appendChild(f)},window.ensureThreadAutoSystemPromptCard=()=>{const c=get("thread\
...
"thread",!0)}</div>
                `,d.appendChild(f)},Qn(),Ln(),pe();const ei=a(()=>{const c=get("set-default-model");
if(!c)return;const d=c.value;c.innerHTML="",MODELS.forEach(m=>{const y=document.createElement("optgr\
oup");y.label=m.category,(m.items||[]).forEach(k=>{const _=document.createElement("option");_.value=
k.id,_.textContent=k.name,y.appendChild(_)}),c.appendChild(y)});const f=userSettingsSnapshot&&userSettingsSnapshot.
default_model||d||"gemini-3.6-flash";f&&Array.from(c.options).some(m=>m.value===f)&&(c.value=f)},"po\
pulateDefaultModelOptions"),ti=a(()=>{const c=get("set-default-vision-model");if(!c)return;const d=c.
value;c.innerHTML="",MODELS.forEach(m=>{const y=(m.items||[]).filter(_=>{const S=(_.id||"").toLowerCase();
...
S)}),c.appendChild(k)});const f=userSettingsSnapshot&&userSettingsSnapshot.default_vision_model||d||
"gemini-3-flash-preview";f&&Array.from(c.options).some(m=>m.value===f)&&(c.value=f)},"populateDefaul\
tVisionModelOptions"),An=a(c=>{if(!c)return;cacheUserSettings(c);const d=get("app-global-sys-prompt-\
preview");d&&(d.value=c.global_system_prompt_effective||"");const f=get("app-global-sys-prompt-previ\
ew-status");f&&(c.global_system_prompt_enabled===!1?f.textContent="\u73FE\u5728\u306F\u7121\u52B9\u5316\u3055\u308C\u3066\u3044\u307E\u3059\u3002":
c.global_system_prompt_uses_time_fallback?f.textContent="\u7BA1\u7406\u8005\u8A2D\u5B9A\u304C\u7A7A\u6B04\u306E\u305F\u3081\u3001\u6642\u523B\u306E\u65E2\u5B9A\u30D7\u30ED\u30F3\u30D7\u30C8\u304C\u9069\u7528\u3055\u308C\u3066\u3044\u307E\u3059\u3002":
...
c),get("set-enter-to-send")&&(get("set-enter-to-send").checked=!!c.enter_to_send),writePromptBarModeToForm(
!!c.compact_prompt_mode,!!c.minimal_prompt_mode),get("set-use-sw-cache")&&(get("set-use-sw-cache").checked=
!!c.use_sw_cache),get("set-clear-cache-on-version-update")&&(get("set-clear-cache-on-version-update").
checked=!!c.clear_cache_on_version_update),get("set-liquid-glass")&&(get("set-liquid-glass").checked=
!!c.liquid_glass_enabled),get("set-auto-search-links")&&(get("set-auto-search-links").checked=c.auto_search_on_links!==
!1),get("set-use-last-settings")&&(get("set-use-last-settings").checked=!!c.use_last_chat_settings),
get("set-default-model")&&(get("set-default-model").value=c.default_model||"gemini-3.6-flash"),get("\
set-default-vision-model")&&(get("set-default-vision-model").value=c.default_vision_model||"gemini-3\
-flash-preview"),applyTemporaryChatTimeoutSeconds(c.temp_chat_timeout_seconds),get("set-default-sear\
ch")&&(get("set-default-search").checked=!!c.default_enable_search),get("set-default-url-context")&&
(get("set-default-url-context").checked=!!c.default_enable_url_context),get("set-default-maps")&&(get(
"set-default-maps").checked=!!c.default_enable_maps),get("set-default-python")&&(get("set-default-py\
thon").checked=!!c.default_enable_python),get("set-default-thinking")&&(get("set-default-thinking").
checked=!!c.default_enable_thinking),get("set-default-sys-prompt")&&(get("set-default-sys-prompt").checked=
!!c.default_enable_system_prompt),get("set-default-thinking-level")&&(get("set-default-thinking-leve\
l").value=c.default_thinking_level||"high"),get("set-default-thinking-budget")&&(get("set-default-th\
inking-budget").value=c.default_thinking_budget||4096),get("set-default-reasoning-effort")&&(get("se\
t-default-reasoning-effort").value=c.default_reasoning_effort||"medium"),get("set-default-safety")&&
(get("set-default-safety").value=c.default_safety_setting||"default"),get("set-e2ee").checked=c.enable_e2ee,
get("set-bot-detect")&&(get("set-bot-detect").checked=c.bot_detection_enabled!==!1),get("set-bot-det\
ect-global")&&(get("set-bot-detect-global").checked=c.bot_detection_global_enabled!==!1);const m=get(
"bot-status");m&&(c.is_bot_banned?(m.textContent=`BAN\u4E2D: ${c.bot_ban_reason||"Bot detection"}`,m.
classList.remove("hidden"),m.classList.add("text-red-400")):m.classList.add("hidden")),c&&c.theme_color?
(applyThemeColor(c.theme_color,!0),syncThemeInputs(c.theme_color)):syncThemeInputs(localStorage.getItem(
THEME_STORAGE_KEY)||INITIAL_THEME_COLOR||THEME_DEFAULT),snapshotSidebarHistory("settings-theme-synce\
d"),syncGeminiLocalPyDialogSetting(),syncCompressionSettingsUi(),get("set-username")&&(get("set-user\
name").value=c.username);const y=get("2fa-badge"),k=get("disable-2fa-btn");c.is_2fa_enabled?(y.innerText=
"ENABLED",y.classList.replace("bg-gray-700","bg-green-600"),y.classList.replace("text-gray-400","tex\
t-white"),k.classList.remove("hidden")):(y.innerText="DISABLED",y.classList.replace("bg-green-600","\
bg-gray-700"),y.classList.replace("text-white","text-gray-400"),k.classList.add("hidden")),get("set-\
skip-2fa-google")&&(get("set-skip-2fa-google").checked=!!c.skip_2fa_on_google_login),get("set-defaul\
t-2fa-method")&&(get("set-default-2fa-method").value=c.default_2fa_method||"totp");const _=get("set-\
passkey-only-login"),S=get("passkey-only-note"),M=Array.isArray(c.passkey_credentials)?c.passkey_credentials:
...
loadStorageUsage(),loadSiteCacheUsage(),N(),Ln(),typeof window.__loadAdminEncThreads=="function")try{
...
then(m=>m.json()).then(m=>{An(m)}).catch(()=>{settingsModalLoaded=!1,setSettingsSaveEnabled(!1),showToast(
"\u8A2D\u5B9A\u306E\u8AAD\u307F\u8FBC\u307F\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002\u9589\u3058\u3066\u518D\u5EA6\u958B\u3044\u3066\u304F\u3060\u3055\u3044",
"error",!0)}),bn(),O(),be()};const Nt=a((c=!1)=>{snapshotSidebarHistory("settings-close-before"),hideModal(
"settings-modal"),revealPersistentSidebarLists(),snapshotSidebarHistory("settings-close-after"),setTimeout(
()=>snapshotSidebarHistory("settings-close-later-300ms"),300),!c&&location.pathname==="/settings"&&history.
back()},"closeSettingsModal"),ni=a(()=>{const c=get("set-theme-color"),d=get("set-theme-color-text"),
f=get("theme-reset-btn"),m=document.querySelectorAll("#theme-presets .theme-swatch"),y=a((k,_=!0)=>{
const S=normalizeHex(k);S&&(applyThemeColor(S,_),syncThemeInputs(S))},"applyFromValue");c&&c.addEventListener(
"input",()=>y(c.value,!0)),d&&(d.addEventListener("change",()=>{const k=normalizeHex(d.value);if(!k){
...
ls"),ii=a(()=>{const c=get("reset-global-sys-prompt");c&&(c.onclick=()=>{get("sys-prompt-text")&&(get(
...
"success")});const d=get("reset-set-auto-sys-prompt-defaults");d&&(d.onclick=()=>{Mn("set","set-appl\
y-auto-sys-prompt-notices"),showToast("\u81EA\u52D5\u6CE8\u5165\u30B7\u30B9\u30C6\u30E0\u30D7\u30ED\u30F3\u30D7\u30C8\u3092\u65E2\u5B9A\u5024\u306B\u623B\u3057\u307E\u3057\u305F\uFF08\u4FDD\u5B58\u3057\u3066\u304F\u3060\u3055\u3044\uFF09",
"success")});const f=get("reset-thread-auto-sys-prompt-defaults");f&&(f.onclick=()=>{Mn("thread","th\
...
"close-settings-btn").onclick=()=>Nt();const En=get("settings-header-close");En&&(En.onclick=()=>Nt());
const Rt=get("settings-search");Rt&&(Rt.addEventListener("input",filterSettings),Rt.addEventListener(
"keydown",c=>{if(c.key==="Enter"){const d=get("tab-"+activeSettingsTab);if(!d)return;const f=d.querySelector(
":scope > .settings-match");f&&f.scrollIntoView({behavior:"smooth",block:"start"})}}));const $n=get(
"settings-search-clear");$n&&$n.addEventListener("click",()=>{Rt&&(Rt.value="",filterSettings(),Rt.focus())}),
ni(),ii(),bindModelApiKeySettingsControls(),syncGeminiLocalPyDialogSetting(),syncCompressionSettingsUi();
const pn=get("set-gemini-local-python-dialog");pn&&(pn.onchange=()=>setGeminiLocalPyDialogEnabled(pn.
checked));const Pn=get("set-gemini-backend");Pn&&(Pn.onchange=()=>syncGeminiBackendUi());const In=get(
"set-admin-api-key-mode");In&&(In.onchange=()=>syncAdminApiKeyModeUi());const fn=get("set-temp-chat-\
timeout-seconds");fn&&(fn.onchange=()=>{applyTemporaryChatTimeoutSeconds(fn.value)});const On=get("s\
lash-command-cancel-btn");On&&(On.onclick=()=>{hidePendingSlashCommandIndicator();const c=get("promp\
...
"error",!0);return}const c=get("set-username"),d=get("set-password"),f=readPromptBarModeFromForm(),m={
...
y.classList.remove("hidden")),c&&(c.value="");else{const k=m&&m.error?m.error:"\u89E3\u9664\u306B\u5931\u6557\u3057\u307E\u3057\u305F";
...
"click",async c=>{const d=c.target.closest("button");if(!d)return;const f=d.getAttribute("data-id");
...
()=>setMarkerMode("crop"));const mn=get("marker-color-picker");mn&&(mn.oninput=c=>setMarkerColor(c.target.
value),mn.onchange=c=>setMarkerColor(c.target.value));const gn=get("marker-opacity");gn&&(gn.oninput=
c=>setMarkerOpacity(c.target.value),gn.onchange=c=>setMarkerOpacity(c.target.value));const nn=get("m\
arker-opacity-number");nn&&(nn.onchange=c=>setMarkerOpacity(c.target.value),nn.onblur=c=>setMarkerOpacity(
c.target.value),nn.onkeydown=c=>{c.key==="Enter"&&(setMarkerOpacity(c.target.value),c.target.blur())}),
...
(applyMarkerTransform(),renderCropOverlay())});const pt=get("upload-dropzone");pt&&(pt.addEventListener(
"dragover",c=>{c.preventDefault(),pt.classList.add("dragover")}),pt.addEventListener("dragleave",()=>{
pt.classList.remove("dragover")}),pt.addEventListener("drop",c=>{c.preventDefault(),c.stopPropagation(),
pt.classList.remove("dragover");const d=c.dataTransfer?c.dataTransfer.files:null;d&&d.length&&handleFiles(
d)}));const Ft=get("drop-overlay");let Wt=0;const si=a(()=>{Ft&&(Ft.classList.remove("hidden"),Ft.classList.
add("flex"))},"showDropOverlay"),Nn=a(()=>{Ft&&(Ft.classList.add("hidden"),Ft.classList.remove("flex"))},
"hideDropOverlay");window.addEventListener("dragenter",c=>{!c.dataTransfer||!c.dataTransfer.types||!c.
dataTransfer.types.includes("Files")||(Wt+=1,si())}),window.addEventListener("dragover",c=>{!c.dataTransfer||
!c.dataTransfer.types||!c.dataTransfer.types.includes("Files")||c.preventDefault()}),window.addEventListener(
"dragleave",c=>{!c.dataTransfer||!c.dataTransfer.types||!c.dataTransfer.types.includes("Files")||(Wt=
Math.max(0,Wt-1),Wt===0&&Nn())}),window.addEventListener("drop",c=>{!c.dataTransfer||!c.dataTransfer.
files||c.dataTransfer.files.length===0||(c.preventDefault(),!(pt&&pt.contains(c.target))&&(Wt=0,Nn(),
handleFiles(c.dataTransfer.files)))});const Rn=get("bot-admin-modal"),ai=a(c=>{const d=get("bot-admi\
n-list");if(d){if(d.innerHTML="",!c||!c.length){d.innerHTML='<div class="text-xs text-gray-400">\u8A72\u5F53\u30E6\u30FC\
\u30B6\u30FC\u304C\u3044\u307E\u305B\u3093\u3002</div>';return}c.forEach((f,m)=>{const y=!!f.is_bot_banned,
k=f.bot_detection_enabled!==!1,_=document.createElement("div");_.className="flex items-center gap-2 \
bg-gray-900 border border-gray-700 rounded p-2 text-xs model-list-animate",_.style.animationDelay=`${Math.
min(m,12)*.02}s`,_.innerHTML=`
                        <div class="flex-1">
...
2 py-1 rounded" data-progress-expected-slow="true" data-user="${escapeHtml(f.username)}">\u524A\u9664</button>\
                    ...
                    `,d.appendChild(_)})}},"renderBotUsers"),Jt=a(async(c="")=>{const d=get("bot-adm\
...
c)}`),m=await f.json();f.ok&&m&&m.users?ai(m.users):(d&&(d.innerHTML='<div class="text-xs text-red-4\
...
showToast("\u30E6\u30FC\u30B6\u30FC\u4E00\u89A7\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
"error",!0)}},"loadBotUsers"),hn=a(async()=>{if(!isAdminUser||!(get("bot-admin-modal")||Rn))return;const d=get(
...
{modal:"admin-bots"},"","/admin-bots"),await Jt(get("bot-admin-search")?get("bot-admin-search").value.
trim():"")},"openBotAdminModal");window.openBotAdminModal=hn,window.closeBotAdminModal=(c=!1)=>{(get(
"bot-admin-modal")||Rn)&&hideModal("bot-admin-modal"),!c&&location.pathname==="/admin-bots"&&history.
back()},get("bot-admin-open")&&(get("bot-admin-open").onclick=()=>{hn()}),get("bot-admin-close")&&(get(
"bot-admin-close").onclick=()=>closeBotAdminModal()),get("bot-admin-search-btn")&&(get("bot-admin-se\
arch-btn").onclick=async()=>{await Jt(get("bot-admin-search")?get("bot-admin-search").value.trim():"")}),
get("bot-admin-refresh-btn")&&(get("bot-admin-refresh-btn").onclick=async()=>{await Jt("")}),get("bo\
t-admin-search")&&get("bot-admin-search").addEventListener("keydown",async c=>{c.key==="Enter"&&await Jt(
get("bot-admin-search").value.trim())}),get("bot-admin-list")&&(get("bot-admin-list").onclick=async c=>{
const d=c.target.closest("button");if(!d)return;const f=d.getAttribute("data-user");if(!f)return;let m;
...
"error",!0)}await Jt(get("bot-admin-search")?get("bot-admin-search").value.trim():"")}});const Bt={"\
...
dal",open:a(()=>hn(),"open")}},Fn=a((c,d=!1)=>{switch(c){case"settings-modal":Nt(d);break;case"uploa\
...
setItem("version_notified",f),hideModal(c);break;default:hideModal(c);break}},"closeModalById");window.
...
"",bn()};async function bn(){const d=await(await apiFetch("/api/feedback?all=1")).json(),f=get("fb-l\
...
on"},body:JSON.stringify({status:S,admin_reply:M})}),bn()},y.appendChild(_)})):m.classList.add("hidd\
en")}if(a(bn,"loadFeedback"),window.setupTOTP=async()=>{const d=await(await apiFetch("/api/2fa/totp/\
...
e"),f=d?String(d.value||"").trim():"";try{c&&(c.disabled=!0);const m=await apiFetch("/api/2fa/webaut\
...
"error",!0)}catch(m){showToast(`WebAuthn Error: ${m}`,"error",!0)}finally{c&&(c.disabled=!1)}},window.
...
lication/json"},body:JSON.stringify({id:c})}),f=await d.json().catch(()=>({}));if(d.ok){showToast("\u30D1\
...
if(slashSuggestionsVisible){const f=get("slash-command-suggestions");if(c.key==="ArrowDown"){c.preventDefault(),
...
if(c.key==="Enter"){c.preventDefault();const m=SLASH_COMMANDS.filter(y=>y.label.toLowerCase().includes(
(extractSlashCommandToken(d.value)||"").toLowerCase()));m[slashSelectedIndex]?selectSlashCommand(m[slashSelectedIndex].
id):m.length>0&&selectSlashCommand(m[0].id);return}if(c.key==="Escape"){c.preventDefault(),hideSlashCommandSuggestions();
return}}if(gemSuggestionsVisible){const f=d.value.trim();if(c.key==="ArrowDown"){c.preventDefault(),
gemSelectedIndex=gemSelectedIndex+1,showGemSuggestions(f.substring(1));return}if(c.key==="ArrowUp"){
c.preventDefault(),gemSelectedIndex=Math.max(gemSelectedIndex-1,0),showGemSuggestions(f.substring(1));
return}if(c.key==="Enter"){c.preventDefault();const m=f.substring(1).toLowerCase(),y=loadedGems.filter(
...
"paste",async c=>{const d=(c.clipboardData||window.clipboardData).items,f=[];for(let m=0;m<d.length;m++)
if(d[m].kind==="file"){const y=d[m].getAsFile();y&&f.push(y)}f.length>0&&(c.preventDefault(),await handleFiles(
...
c.addEventListener("paste",async d=>{const f=d.clipboardData||window.clipboardData;if(f){d.preventDefault();
...
"click",c=>{const d=c.target.closest("img.chat-image"),f=d?d.dataset.viewerSrc||d.currentSrc||d.src:
"";d&&f&&(c.preventDefault(),openImageViewer(f))});const Kt=document.querySelector(".viewer-content");
Kt&&(Kt.addEventListener("touchstart",onViewerTouchStart,{passive:!1}),Kt.addEventListener("touchmov\
e",onViewerTouchMove,{passive:!1}),Kt.addEventListener("touchend",onViewerTouchEnd),Kt.addEventListener(
...
let Ge,Le=null,sn=[],yn=!1,an=null,jt=null,Tt=null,Xt=null,vn=0,on=!1,Yt=null,Ht=null,ht=null,Qt=null,
rn=null,Ct=null,ln=null,cn=null;function jn(){const c=get("mic-waveform");if(!c)return[];if(Array.isArray(
ln)&&ln.length)return ln;c.innerHTML="";const d=[];for(let f=0;f<24;f++){const m=document.createElement(
...
"0.4",d.push(m),c.appendChild(m)}return ln=d,d}a(jn,"ensureMicWaveformBars");function Lt(c,d="hidden"){
const f=get("mic-recording-indicator"),m=get("mic-recording-text");if(f){if(cn&&(clearTimeout(cn),cn=
null),d==="hidden"){f.classList.add("hidden");return}m&&c&&(m.innerText=c),f.classList.remove("hidde\
...
style.height="2px",d.style.opacity="0.35"})}a(Hn,"resetMicWaveformBars");function Mt(){if(rn&&(cancelAnimationFrame(
rn),rn=null),Qt){try{Qt.disconnect()}catch{}Qt=null}if(Ht){try{Ht.close()}catch{}Ht=null}ht=null,Ct=
null,Hn()}a(Mt,"stopMicWaveform");function Dn(c){Mt();const d=jn();if(!d.length)return;const f=window.
AudioContext||window.webkitAudioContext;if(!f)return;try{Ht=new f,ht=Ht.createAnalyser(),ht.fftSize=
256,ht.smoothingTimeConstant=0,Qt=Ht.createMediaStreamSource(c),Qt.connect(ht),Ct=new Uint8Array(ht.
frequencyBinCount)}catch{Mt();return}const m=a(()=>{if(!ht||!Ct)return;ht.getByteFrequencyData(Ct);const y=Math.
...
.65}`}rn=requestAnimationFrame(m)},"render");m()}a(Dn,"startMicWaveform");function dn(){if(an&&(clearInterval(
an),an=null),Xt){try{Xt.disconnect()}catch{}Xt=null}if(jt){try{jt.close()}catch{}jt=null}Tt=null}a(dn,
"stopSilenceMonitor");function qn(c){if(!isStsModel()||!stsOpt("sts-auto-send"))return;dn();const d=window.
AudioContext||window.webkitAudioContext;if(!d)return;jt=new d,Tt=jt.createAnalyser(),Tt.fftSize=2048,
Xt=jt.createMediaStreamSource(c),Xt.connect(Tt);const f=new Uint8Array(Tt.fftSize),m=getStsSilenceMs(),
y=.02;vn=0,on=!1,an=setInterval(()=>{if(!Tt)return;Tt.getByteTimeDomainData(f);let k=0;for(let S=0;S<
f.length;S++){const M=(f[S]-128)/128;k+=M*M}if(Math.sqrt(k/f.length)>y){on||(on=!0),vn=Date.now();return}
on&&Date.now()-vn>m&&Ge&&Ge.state==="recording"&&Ge.stop()},200)}a(qn,"startSilenceMonitor");const kn=class kn{constructor(){
...
onloadend=()=>f(m.result.split(",")[1]),m.readAsDataURL(d)})}};a(kn,"GeminiLiveClient");let wn=kn;const _n=class _n{constructor(d=24e3){
...
S.start(M),this.nextStartTime=M+_.duration}stop(){this.ctx&&(this.ctx.close(),this.ctx=null)}};a(_n,
"RealTimeAudioPlayer");let Zt=_n;const Sn=class Sn{constructor(){this.active=!1,this.capturing=!1,this.
...
(this.rtPlayer=new Zt(this.rateOut||24e3),qt=this.rtPlayer),setStsStatus("\u518D\u751F\u4E2D...",!0),
...
active||!this.capturing)return;const M=S.inputBuffer.getChannelData(0),B=oi(M,k,y);!B||!B.byteLength||
...
this.audioCtx.close()}catch{}this.audioCtx=null}dn(),Mt()}async stop(){if(!this.active)return;this.active=
...
classList.remove("bg-red-600","animate-pulse"),d.classList.add("bg-gray-700"))}};a(Sn,"RealtimeVoice\
Session");let xn=Sn;function oi(c,d,f){let m=c;if(d!==f&&d>0&&f>0){const k=d/f,_=Math.floor(m.length/
...
return y.buffer}a(oi,"pcm16FromFloat32");const Dt=new xn;(()=>{const f={idle:"bg-gray-600",connecting:"\
...
ee=A("lyria-reset-btn"),ge=!!m,Me=G==="streaming"||G==="connecting";if($){$.disabled=ne||!ge;const Ue=$.
querySelector("i");Ue&&(Ue.className="fas fa-play")}j&&(j.disabled=ne||!Me),fe&&(fe.disabled=ne||!ge||
...
prompt-row-wrap"),j=[];return $.forEach(fe=>{const ee=fe.querySelector('input[type="text"]'),ge=fe.querySelector(
'input[type="range"]'),Me=(ee?ee.value:"").trim();Me&&j.push({text:Me,weight:parseFloat(ge?ge.value:
1)||1})}),j}a(We,"collectPrompts");function Ye(){const $={},j=a(ci=>{const Cn=A(ci);return Cn&&Cn.value!==
""?parseFloat(Cn.value):void 0},"num"),fe=j("lyria-bpm");fe!==void 0&&($.bpm=Math.round(fe));const ee=j(
...
const ee=A(j),ge=A(fe);!ee||!ge||ee.addEventListener("input",()=>{const Me=parseFloat(ee.value);ge.textContent=
...
const j=$.body.getReader(),fe=new TextDecoder;let ee="";for(;k;){const{done:ge,value:Me}=await j.read();
if(ge)break;ee+=fe.decode(Me,{stream:!0});const Ue=ee.split(`
...
M);Me.start(Ue),M=Ue+ge.duration}a(re,"playChunk");async function ue($,j){const fe=await fetch("/api\
...
scale!==void 0&&$.scale!==j.scale,ge=fe||ee;ne=!0;try{await ue("config",{config:$,reset_context:ge}),
...
ie(),bt(),J("\u6E96\u5099\u5B8C\u4E86","idle"),showModal("lyria-studio-modal")}a(Tn,"open");function li(){
const $=A("lyria-open-studio-btn");$&&$.addEventListener("click",()=>Tn(""));const j=A("lyria-studio\
...
Tn}return a(li,"init"),{init:li,open:Tn}})().init(),(()=>{let c=null,d=null;const f=a(I=>document.getElementById(
...
(c=I.parentNode,A.appendChild(I));const U=f("file-preview"),J=f("voice-studio-file-host");U&&J&&U.parentNode!==
J&&(d=U.parentNode,J.appendChild(U),J.classList.remove("hidden"))}a(S,"movePanelIntoModal");function M(){
const I=f("sts-panel");I&&c&&I.parentNode!==c&&c.appendChild(I);const A=f("file-preview");A&&d&&A.parentNode!==
d&&d.appendChild(A);const U=f("voice-studio-file-host");U&&U.classList.add("hidden"),c=null,d=null}a(
...
if(qt&&(qt.stop(),qt=null),Yt){try{Yt.pause()}catch{}try{Yt.src=""}catch{}Yt=null}}a(Gn,"stopStsPlay\
back");async function fi(c){Gn();const d=new Audio;return d.src=c,d.preload="auto",d.autoplay=!0,d.playsInline=
!0,Yt=d,await d.play(),new Promise(f=>{d.onended=()=>f("ended"),d.onerror=()=>f("error")})}a(fi,"pla\
...
"recording"&&(yn=!0,Ge.stop())}a(Un,"cancelRecording");function zn(){if(isStsModel())return{audio:!0};
...
"sts-include-thoughts")?get("sts-include-thoughts").checked:!1;if(Le=new wn,stsOpt("sts-auto-play")&&
!isGeminiLiveTranscribeModel()&&(Le.rtPlayer=new Zt),isGeminiLiveTranscribeModel()){const I=get("sts\
...
getUserMedia(zn());Ge=new MediaRecorder(c),sn=[],yn=!1;const d=isStsModel();Ge.ondataavailable=f=>sn.
push(f.data),Ge.onstop=async()=>{if(yn){sn=[],get("file-preview").classList.add("hidden"),c.getTracks().
forEach(_=>_.stop()),dn(),Mt(),d||(Lt("\u9332\u97F3\u3092\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F",
"idle"),cn=setTimeout(()=>Lt("","hidden"),900)),isStsModel()&&setStsStatus("Canceled",!1),setTimeout(
()=>{isStsModel()&&setStsStatus("Tap to speak",!1)},800);return}const f=new Blob(sn,{type:"audio/web\
...
"sts-auto-play")&&(G=new Zt,qt=G),setStsStatus(isTranscriptionModel()?"Transcribing...":"Processing \
...
stop()),dn(),Mt(),k||Lt("","hidden"),k&&setStsStatus("Tap to speak",!1)}},Ge.start(),get("mic-btn").
classList.remove("bg-gray-700"),get("mic-btn").classList.add("bg-red-600","animate-pulse"),isStsModel()||
(Lt("\u9332\u97F3\u4E2D\u2026","recording"),Dn(c)),qn(c),isStsModel()&&setStsStatus("Recording... Ta\
p to stop",!0)}catch{Mt(),isStsModel()||Lt("","hidden"),alert("Microphone access denied or not avail\
able.")}};const un=a((c,d)=>{if(!c)return;const f=c.querySelector("span");f?f.textContent=d:c.textContent=
...
me-btn");if(d&&(d.disabled=c===0,un(d,c?`\u524A\u9664 (${c})`:"\u524A\u9664")),f&&(f.disabled=c===0,
un(f,c?`\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9 (${c})`:"\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9")),m&&(m.
disabled=c===0,un(m,c?`\u6DFB\u4ED8 (${c})`:"\u6DFB\u4ED8")),y&&(y.disabled=c!==1,un(y,"\u540D\u524D\u5909\u66F4")),
lib.modal){const k=window.matchMedia("(max-width: 768px)").matches;lib.modal.classList.toggle("lib-s\
electing",k&&c>0)}};function Vn(c){lib.attachMode=!!c}a(Vn,"setLibAttachMode");const Wn=a((c=!1)=>{Vn(
...
row")&&(get("add-gem-fixed-prompt-row").onclick=()=>addGemFixedPromptRow());const ri=a(()=>{editingGemUuid=
...
pathname==="/gem"&&history.back()},get("add-gem-btn").onclick=()=>ri(),get("save-gem-btn").onclick=async()=>{
const c=get("gem-name").value,d=get("gem-desc").value,f=get("gem-inst").value,m=collectGemFixedPrompts();
if(c&&f){const y=editingGemUuid?"PUT":"POST",k=editingGemUuid?`/api/gems/${editingGemUuid}`:CHAT_CONFIG.
urls.handleGems;await apiFetch(k,{method:y,headers:{"Content-Type":"application/json"},body:JSON.stringify(
{name:c,description:d,instruction:f,fixed_prompts:m,default_model:get("gem-default-model").value||null})}),
window.closeGemModal(),loadGems(),editingGemUuid&&activeGem&&activeGem.uuid===editingGemUuid&&(activeGem.
name=c,activeGem.instruction=f,activeGem.fixed_prompts=m,applyActiveGem(activeGem))}else alert("Name\
 and Instruction are required.")},document.addEventListener("click",function(c){if(c.target.closest(
".edit-btn")){const f=c.target.closest(".edit-btn").getAttribute("data-id");beginEditMessage(f)}if(c.
target.closest(".code-toggle")){const d=c.target.closest(".code-toggle"),f=d.closest(".code-wrapper");
...
"aria-label",m?"\u5C55\u958B":"\u6298\u308A\u305F\u305F\u3080")}if(c.target.closest(".download-btn")){
const d=c.target.closest(".download-btn"),f=d.getAttribute("data-code"),m=(d.getAttribute("data-lang")||
...
.coding-target-btn")),c.target.closest(".copy-btn")){const d=c.target.closest(".copy-btn"),f=d.getAttribute(
"data-code");f&&window.copyCode(d,f)}if(c.target.closest(".html-preview-btn")){const f=c.target.closest(
".html-preview-btn").getAttribute("data-code");f&&openHtmlCodePreview(f)}if(c.target.closest(".canva\
...
l-progress"),l=get("upload-modal-total-progress-bar");if(!e||!t)return;if(s){const T=document.querySelectorAll(
...
T,E}a(u,"createFileThumb");const p=collectImageUrlsForSend(),h=uploadProgressState.total,g=uploadProgressState.
...
classList.remove("hidden"),t.innerText=T,o&&(o.innerText=`(${g}/${h})`);let E=g*100,F=0;for(let be in uploadProgressState.
perFilePct)E+=uploadProgressState.perFilePct[be],F++;const se=h>0?E/(h*100)*100:0,Y=`${Math.min(100,
...
g-l,L=a((T,E,F)=>Number.isFinite(T)?E>F?(E+F)/2:Math.min(F,Math.max(E,T)):0,"clampOffset");markerView.
...
"attachCameraCapturedFiles");function openUploadModal(){syncUploadRowsFromCurrent(),showModal("uploa\
d-modal"),location.pathname!=="/upload"&&history.pushState({modal:"upload"},"","/upload");const e=get(
"vision-model-info");if(e){const n=(get("model-select")?get("model-select").value:"").toLowerCase(),
i=n.includes("deepseek")&&n!=="deepseek-v4-flash-vision-exp";e.classList.toggle("hidden",!i)}_syncVisionModelDisplay()}
...
50)}a(_openVisionModelSelector,"_openVisionModelSelector");function closeUploadModal(e=!1){hideModal(
"upload-modal"),!e&&location.pathname==="/upload"&&history.back()}a(closeUploadModal,"closeUploadMod\
al");function syncUploadRowsFromCurrent(){const e=get("upload-list");if(!e)return;const t=new Set;e.
querySelectorAll("[data-filename]").forEach(n=>{const i=n.getAttribute("data-filename");i&&t.add(i)}),
currentImageUrls.forEach(n=>{t.has(n)||addStoredUploadRow(n,{source:getAttachmentSourceForPath(n),displayName:getAttachmentNameForPath(
n)})}),e.children.length===0&&(e.innerHTML='<div class="text-xs text-gray-500">\u307E\u3060\u30A2\u30C3\u30D7\u30ED\u30FC\u30C9\u304C\u3042\u308A\u307E\u305B\u3093\u3002</div>')}
...
g)||o));const L=g.querySelector(".upload-send-name");L&&(L.onclick=()=>promptRowAttachmentName(g));const T=g.
...
r=markerView.scale,l={x:0,y:0},u={x:0,y:0},p=[],h=16,g="",v=null,b=null,w=null,x=null,L=!1,T=null;const E=a(
...
q=C.x-N.x,Q=C.y-N.y,H=Math.hypot(q,Q),te=Math.max(.35,h*.04);if(H<te)return!1;const D=Math.max(1,h*.25),
...
p.length===1){const C=p[0];x.beginPath(),x.arc(C.x,C.y,h/2,0,Math.PI*2),x.fill();return}if(x.beginPath(),
...
scale,l={x:markerView.offsetX,y:markerView.offsetY},u=F(Q,H),renderCropOverlay(),e.setPointerCapture&&
...
maxScale,Math.max(markerView.minScale,le)),markerView.offsetX=l.x+(te.x-u.x),markerView.offsetY=l.y+
...
clientX,y:O.clientY}),n.size>=2){const pe=Array.from(n.values()),oe=pe[0],ce=pe[1];l=!0,i=!1,u=T(oe,
ce)||1,p=markerView.scale,h={x:markerView.offsetX,y:markerView.offsetY},g=L(oe,ce),e.setPointerCapture&&
e.setPointerCapture(O.pointerId),O.preventDefault();return}if(l)return;i=!0;const z=x(O),Z=E();o=F(z,
...
n.set(O.pointerId,{x:O.clientX,y:O.clientY}),l&&n.size>=2){const C=Array.from(n.values()),N=C[0],q=C[1],
...
o=escapeHtml(i)}const r=escapeHtml(s),l=encodeURIComponent(i).replace(/'/g,"%27"),u=encodeURIComponent(
...
ey]").forEach(l=>{const u=l.getAttribute("data-stream-math-key");u&&o.set(u,l)});const r=[];s.content.
querySelectorAll(".stream-math-segment[data-stream-math-key]").forEach(l=>{const u=o.get(l.getAttribute(
"data-stream-math-key"));u?l.replaceWith(u):r.push(l)}),e.replaceChildren(s.content),wrapRenderedSvgBoxes(
...
redSvgBoxes");function renderMessage(e,t,n,i,s,o,r=null,l=!0,u=null,p=null,h=null,g=null,v=null,b=null,w=null,x=null,L=!0,T=null,E=null){
...
if(z==null){const H=h!=null?Number(h):0,te=g!=null?Number(g):0;(h!=null||g!=null)&&(z=H+te)}messageMeta[e]=
...
i>${escapeHtml(E)}</span>`));const ke=[];if(h!=null&&ke.push(`In ${h}`),g!=null){let H=`Out ${g}`;w!=
...
r=>{const l=get(r);return!!(l&&l.checked)}))return"\u691C\u7D22\u30FBURL\u53C2\u7167\u30FB\u30B7\u30B9\u30C6\u30E0\u6A5F\u80FD\u5229\u7528\u6642\u306F\u901A\u5E38\u30E2\u30FC\u30C9\u304C\u5FC5\u8981\u3067\u3059";
...
abortController=new AbortController;let g="",v="";const b=[];let w=null,x=null,L=!1;const T={},E=[];
...
${D}
\`\`\`
`,F=`browserFastPy_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,se=D,T[F]||(h.insertAdjacentHTML(
...
"",output:D}),T[le]||(h.insertAdjacentHTML("afterbegin",browserFastPythonBoxHtml(le)),T[le]=h.querySelector(
`[data-py-id="${le}"]`)),updateBrowserFastPythonBox(T[le],"output",D);return}const te=typeof H.text==
"string"?H.text:"";te&&(H.thought===!0?v+=te:g+=te)})}),!L&&(g||v)){beginPendingToStreamTransition(h);
...
split(/\r?\n\r?\n/);pe=C.pop()||"",C.forEach(oe)}if(pe+=Z.decode(),pe.trim()&&oe(pe),!g.trim())throw new Error(
...
signal:abortController&&!abortController.signal.aborted?abortController.signal:void 0}),h),xe=await ce.
...
let Ke=ze.split(`
`);ze=Ke.pop();let en=!1,Vt=!1;for(let St of Ke)if(St.trim())try{const de=JSON.parse(St);if(de.type===
"thread_id"){ae();const he=de.content!==null&&de.content!==void 0?String(de.content):de.content;he&&
...
const he=de.content===null||de.content===void 0?"":String(de.content);if(xt("status",!!he),Ne&&D){const Ve=he||
...
H&&Fe(he||"\u63A8\u8AD6\u30D7\u30ED\u30BB\u30B9\u3092\u6E96\u5099\u4E2D...");continue}if(Ne){beginPendingToStreamTransition(
D);const he=D.querySelector(".content-area");he&&(he.innerHTML=""),Ne=!1}if(de.type==="coding_diff")
appendCodingLiveDiff(D,de.content||{}),xt("content",!0);else if(de.type==="thought"){if(Ie||(Ie=D.querySelector(
".thought-content")),Ze+=de.content,xt("thought",!!de.content),!Ie){const he='<div class="thought-co\
...
"afterend",he):D.insertAdjacentHTML("afterbegin",he),Ie=D.querySelector(".thought-content")}if(Ie&&Ie.
...
mmer")}Ze=de.content}Ie.classList.remove("collapsed"),Vt=!0}else if(de.type==="image_analysis"){const he=de.
...
".image-analysis-box")}const Xe=Ve.querySelector(".image-analysis-text");Xe&&(Xe.textContent=he)}else if(de.
type==="python"){const he=de.content||{},Ve=he.id||`py_${Date.now()}`;if(!Ot[Ve]){const qe=`<div cla\
...
"afterbegin",qe),Ot[Ve]=D.querySelector(`[data-py-id="${Ve}"]`)}const Xe=Ot[Ve];if(Xe){if(he.code!==
...
"data-code",encodeURIComponent(qe).replace(/'/g,"%27"))}}}else if(de.type==="content"){const he=de.content===
null||de.content===void 0?"":String(de.content);He+=he,/[`~]/.test(he)&&activateDeferredCodingModeFromStream(
He),De||(De=D.querySelector(".content-area")||document.createElement("div"),De.className="prose pros\
e-invert text-sm break-words",D.contains(De)||D.appendChild(De)),en=!0,xt("content",!!he)}else if(de.
...
(Ie.scrollTop=Ie.scrollHeight)),en&&De){const St=Date.now();if(St-zt>100){const de=snapshotCodeCollapse(
...
let h="",g="",v="",b=!0,w=null,x=null,L=null,T=!1;const E={};let F=0,se=!1;const Y=window.ProgressSpinner?
...
window.ConnectionMonitor.reportActivity(),Y&&Y.setPhase("receiving"),h+=pe.decode(ce,{stream:!0});let xe=h.
split(`
...
x);renderAiMarkdownInto(x,g,{incrementalMath:!0}),applyCodeCollapse(x,W,!0),F=K}}scrollToBottom()}if(Y&&
...
n>${g}</div><div class="flex gap-1 opacity-0 group-hover:opacity-100" data-thread-actions="1"><butto\
...
tor`,s=60,o=88,r=52,l=.5,u=8;let p=0,h=!1,g=0,v=null;const b=a(()=>get(i),"indicatorEl"),w=a(()=>{const T=b();
...
h=!1;return}if(n.scrollTop>0){h=!1;return}const E=T.touches[0];E&&(p=E.clientY,g=0,h=!0)},{passive:!0}),
n.addEventListener("touchmove",T=>{if(!h||v)return;if(n.scrollTop>0){h=!1;return}const E=T.touches[0];
if(!E)return;const F=E.clientY-p;if(F<=0){g>0&&(g=0,x(0)),h=!1;return}const se=b();se&&!se.classList.
contains("dragging")&&se.classList.add("dragging"),g=Math.min(F*l,o),x(g),F>=u&&T.preventDefault()},
{passive:!1}),n.addEventListener("touchend",()=>{if(!h||(h=!1,v))return;const T=b();T&&T.classList.remove(
"dragging");const E=g>=s;if(g=0,!E){L();return}let F;try{F=t()}catch{F=null}const se=b();if(se){se.classList.
...
(v=Promise.resolve(),setTimeout(()=>{v=null,L()},400))}),n.addEventListener("touchcancel",()=>{h=!1,
g=0,L()})}a(initPullToRefresh,"initPullToRefresh");const initThreadPullToRefresh=a(()=>initPullToRefresh(
