...
def secure_delete(path):
    if os.path.exists(path):
        try:
            ...
            with open(path, "wb") as f: f.write(os.urandom(length))
            os.remove(path)
        except Exception as e: logger.error(f"Secure delete failed for {path}: {e}")
            ...
            if os.path.exists(user_dir):
                for f in os.listdir(user_dir):
                    ...
                    if enable and not f.endswith('.enc'):
                        ...
                        with open(fp + '.enc', 'wb') as file: file.write(encrypt_bytes(data))
                        secure_delete(fp)
                    elif not enable and f.endswith('.enc'):
                        with open(fp, 'rb') as file: data = file.read()
                        with open(fp[:-4], 'wb') as file: file.write(decrypt_bytes(data))
                        secure_delete(fp)
...
def background_chat_task(job_id, thread_id, model_key, message_text, img_list, options, api_keys, user_id, user_config):
    with app.app_context():
        ...
        try:
            ...
            elif is_grok and XAIClient:
                x_client = XAIClient(api_key=key)
            else:
                o_client = OpenAI(api_key=key, base_url="https://api.x.ai/v1" if is_grok and not XAIClient else None)
            ...
            if is_gem:
                ...
                tools_list = []
                if options.get('enable_search'): tools_list.append(web_search())
                
                # Note: Correct usage depends on specific SDK version. Assuming create(tools=...) pattern for stream
                # If create() doesn't take tools, they might be added via session or params.
                # Here we use the session context pattern if possible, or re-create session with params.
                # Assuming typical usage:
                
                chat_session.append(x_user(message_text))
                # Re-create session with tools if needed or just stream
                # Due to SDK variation, we try simple stream first.
                
                stream = chat_session.stream(tools=tools_list if tools_list else None)
                ...
                # No search params for OpenAI to avoid error
                
                if options.get('reasoning_effort') and not is_grok:
...
def chat_stream():
    ...
    task_queue.enqueue(background_chat_task, job_id, thread_id, model, message, image_urls, data, api_keys, current_user.id, user_config, job_timeout=600)
    def generate():
