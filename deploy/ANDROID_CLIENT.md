...
Android版は通常チャット、メタデータ付きモデル選択、Thinking・Web検索・Prompt Cache、履歴の検索・ページ送り・削除、添付、生成停止、切断後の再接続、端末連携・失効に対応します。本文は見出し、強調、リスト、引用、安全なHTTP(S)リンク、コピー・折り畳み対応コードを選択可能なネイティブUIで表示します。数式、表、画像プレビュー、リアルタイム音声、Batch操作、高度なモデル別設定はまだWebを使います。
...
MarkdownはHTML/WebViewへ変換せず、許可したブロックとインライン記法だけをComposeで描画します。リンクはhostを持つHTTP(S)だけを外部ブラウザーで開き、認証付きHTTPクライアントへ渡しません。コードは選択、コピー、長い場合の折り畳みに対応します。音声権限、カメラ権限、通知権限は、その機能を実装して利用者が使用する段階で追加します。
