...
class PlaygroundApiTest {
    ...
    @Test fun preferencesParseSafeFields() {
        ...
        assertTrue(prefs.defaultEnableMaps)
        assertTrue(prefs.defaultEnablePython)
        assertFalse(prefs.defaultEnableFileCreation)
        assertTrue(prefs.defaultEnableSystemPrompt)
        assertFalse(prefs.defaultEnableMcp)
        assertEquals("medium", prefs.defaultThinkingLevel)
        assertEquals(8192, prefs.defaultThinkingBudget)
        assertEquals("high", prefs.defaultReasoningEffort)
        assertEquals("none", prefs.defaultSafetySetting)
    ...
    @Test fun batchJobsAndStructuredToolCardsParse() {
        ...
        val mcp = upsertToolCard(emptyList(), "mcp", JSONObject("""{"type":"start","id":"m1","server_name":"GitHub","tool_name":"search"}"""))
        assertEquals(CardKind.MCP, mcp.single().kind)
        assertEquals("GitHub / search", mcp.single().label)
        assertFalse(mcp.single().done)
        val failed = upsertToolCard(mcp, "mcp", JSONObject("""{"type":"error","id":"m1","server_name":"GitHub","tool_name":"search","message":"denied"}"""))
        assertTrue(failed.single().failed)
        assertEquals("denied", failed.single().note)
        val done = upsertToolCard(mcp, "mcp", JSONObject("""{"type":"result","id":"m1","tool_name":"search","summary":"ok\nmore"}"""))
