...
catch(()=>-1),"fileUrlStatus"),i=a((r,l)=>{const d=document.createElement("div");return d.style.cssText=
...
"/").pop(),h=a(x=>{const C=i(g,!!x);try{l.replaceWith(C)}catch{}},"showWarning"),y=a(()=>{const x=s(
g);try{l.replaceWith(x)}catch{}},"showBusyWarning"),b=a((x,C)=>{const T=l.cloneNode(!1);T.setAttribute(
"data-file-retry",String(C));const M=x+(x.includes("?")?"&":"?")+"retry="+Date.now()+"_"+C;T.setAttribute(
"src",M);try{l.replaceWith(T)}catch{}},"retryLoad"),w=a(x=>{if(x===429||x===503){y();return}if(x===409){
h(!0);return}if(x===404||x===410||x===403){h(!1);return}const C=parseInt(l.getAttribute&&l.getAttribute(
"data-file-retry")||"0",10);if(C<2){b(d,C+1);return}if(d.includes("/files/thumb/")&&!l.getAttribute(
"data-file-fallback")){l.setAttribute("data-file-fallback","1"),b(o(d),0);return}h(!1)},"handleStatu\
s");n(d).then(w).catch(()=>{const x=parseInt(l.getAttribute&&l.getAttribute("data-file-retry")||"0",
10);if(x<2){b(d,x+1);return}if(d.includes("/files/thumb/")){y();return}h(!1)})},!0)})();const isAdminSidebarDebugEnabled=a(
...
!1;const o=[...t].sort((h,y)=>h-y),r=Math.min(17.5,Math.max(7,o[Math.floor(o.length*.2)])),l=Math.max(
...
(e.disabled=g),t&&(t.disabled=g);const h=get("compression-size-wrap"),y=get("compression-dim-wrap");
h&&(h.style.opacity=g?"0.4":"1"),y&&(y.style.opacity=g?"0.4":"1")}const s=a((g,h)=>{get(g)&&get(h)&&
...
[]).reduce((x,C)=>C&&C.nodeType===Node.TEXT_NODE?x+String(C.textContent||"").replace(/\s+/g," ").trim().
length:x,0),"directTextLength");o.forEach(w=>{if(!w||w===s||!w.style)return;const x=window.getComputedStyle(
w),C=p(w);if(C>0){const M=parseRichPasteCssColor(x.color);if(M&&M.a>=.5){const F=richPasteColorCss(M),
se=l.get(F)||{color:M,weight:0};se.weight+=C,l.set(F,se),d+=C}}if(!!(String(w.style.backgroundColor||
"").trim()||String(w.style.background||"").trim())){const M=parseRichPasteCssColor(x.backgroundColor);
if(M&&M.a>=.72){const F=String(w.textContent||"").replace(/\s+/g," ").trim().length;r.push({color:M,
weight:Math.max(1,F)})}}});const g=Array.from(l.values()).sort((w,x)=>x.weight-w.weight),h=g.length?
g[0].color:null,y=g.reduce((w,x)=>w+(richPasteColorLuminance(x.color)>=.6?x.weight:0),0);r.sort((w,x)=>x.
weight-w.weight);let b=r.length?r[0].color:null;return b||(b=d>0&&y/d>=.55?{r:11,g:11,b:12,a:1}:t),makeRichPasteTheme(
...
from(b[1].matchAll(/(-?\d+(?:\.\d+)?)px/gi)).some(w=>Math.abs(Number(w[1])||0)>=96)),y=Array.from(g.
matchAll(/(?:^|;)\s*(?:width|min-width)\s*:\s*(-?\d+(?:\.\d+)?)px/gi)).some(b=>Math.abs(Number(b[1])||
0)>720);return d+(h||y?1:0)},0);if(n.length<=500&&i<=24&&s===0)return t.innerHTML;const o=new Set(["\
...
tyle")||"").split(";").forEach(h=>{if(!h||h.indexOf(":")<0)return;const y=h.indexOf(":"),b=h.slice(0,
y).trim().toLowerCase();let w=h.slice(y+1).trim();if(!(!b||!w||o.has(b))&&!["height","max-height","m\
in-height","overflow","overflow-x","overflow-y"].includes(b)&&!(["width","min-width"].includes(b)&&r.
has(p))){if(l.has(b)&&r.has(p)&&Array.from(w.matchAll(/(-?\d+(?:\.\d+)?)px/gi)).map(C=>Math.abs(Number(
C[1])||0)).some(C=>C>=96)&&(w="0px"),b==="display"){const x=w.toLowerCase();["flex","grid"].includes(
...
nd); font-weight: bold; }
            ...
            `,document.body.appendChild(h),await waitForRichPasteMedia(h,4e3),i(15);try{const y=new p(
{unit:"mm",format:"a4",orientation:"portrait",compress:!0}),b=y.internal.pageSize.getWidth(),w=y.internal.
pageSize.getHeight(),x=794,C=Math.floor(w/b*x),T=h.scrollHeight||h.offsetHeight;let M=0,F=!0;const se=Math.
ceil(T/C);let K=0;for(;M<T;){if(richPasteAbortController&&richPasteAbortController.signal.aborted)throw new DOMException(
"Aborted","AbortError");const N=Math.min(C,T-M),Q=(await new Promise((_e,X)=>{const ve=setTimeout(()=>X(
new Error("PDF chunk rendering timed out")),12e4);g(h,{scale:1,useCORS:!0,allowTaint:!1,backgroundColor:l.
background,logging:!1,imageTimeout:5e3,x:0,y:M,width:x,height:N,windowWidth:x,scrollX:0,scrollY:0,signal:richPasteAbortController?
richPasteAbortController.signal:void 0,onclone:a(re=>{prepareRichPastePdfClone(re,l);const j=re.querySelector(
...
ue=y.getImageProperties(Q),ae=Math.min(w,ue.height*b/ue.width);F||y.addPage(),y.addImage(Q,"JPEG",0,
0,b,ae),F=!1,M+=N,K++;const ce=Math.min(100,15+Math.round(K/se*85));i(ce),await new Promise(_e=>setTimeout(
_e,100))}return i(100),{blob:y.output("blob"),fileName:buildRichPastePdfFilename()}}finally{e&&(e.classList.
...
75,"PDF\u3092\u6DFB\u4ED8\u4E2D...");const h=await g.blob(),y=g.headers.get("X-Rich-Paste-Filename")||
buildRichPastePdfFilename();!!(get("rich-paste-download-only")&&get("rich-paste-download-only").checked)?
(setRichPasteProgress(90,"\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u4E2D..."),downloadBlob(h,y),showToast(
"PDF\u3092\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u3057\u307E\u3057\u305F","success"),hideModal("rich-p\
aste-modal",{skipConfirm:!0})):await attachRichPastePdfAndSend(h,y,r,l),setRichPasteProgress(100,"\u5B8C\u4E86"),
...
l.parentNode&&l.parentNode.replaceChild(g,l),o=!0,p=!0}else if(!d){const g=l.textContent||"";if(isPasswordPromptingScript(
...
\u305F",l.parentNode&&l.parentNode.replaceChild(h,l),o=!0,p=!0}}}),s.querySelectorAll('a[href^="java\
...
sys-prompt-option",gear:!0,gearAction:a(()=>{window.openThreadModal&&window.openThreadModal()},"gear\
...
king-level");if(l.length){const p=l.map(h=>thinkingIndexFromValue(h)),g=p.includes(r)?r:p.reduce((h,y)=>Math.
abs(y-r)<Math.abs(h-r)?y:h,p[0]);d&&(d.value=THINKING_LEVELS[g].value,d.dispatchEvent(new Event("cha\
...
Element?r.target:null,d=!0;for(;l&&l!==o;){if(l.scrollTop>0){d=!1;break}l=l.parentElement}popupSwipeAtTop=
d,d&&o.classList.add("dragging")},{passive:!0}),o.addEventListener("touchmove",r=>{if(!popupSwipeDragging||
...
abs(d)>Math.abs(l)?"v":"h"),popupSwipeAxis==="v"&&d>0&&(r.cancelable&&r.preventDefault(),o.style.transform=
`translateY(${Math.min(d*.6,140)}px)`)},{passive:!1}),o.addEventListener("touchend",r=>{if(!popupSwipeDragging)
...
find(d=>d.id==e),o=messageMeta[e]||{};s?currentParentId=s.parent_id===void 0?null:s.parent_id:o.parent_id!==
...
"");const y=normalizeAttachmentPath(h);return y&&setAttachmentSourceForPath(y,g),y}).filter(Boolean),
...
1,e.lastMove){const w=y.clientX-e.lastMove.x,x=y.clientY-e.lastMove.y,C=b-e.lastMove.t;if(C>0){const T=Math.
sqrt(w*w+x*x)/(C/1e3);e.speedMax=Math.max(e.speedMax,T),e.speedSum+=T,e.speedSamples+=1}}e.lastMove=
{x:y.clientX,y:y.clientY,t:b}}},"recordMove"),l=a(()=>{const y=Math.max(1,performance.now()-e.windowStart),
b=e.clickTimes.length,w=e.keyTimes.length,x=e.speedSamples?e.speedSum/e.speedSamples:0;let C=0,T=1;if(e.
clickIntervals.length>=3){const M=e.clickIntervals.reduce((se,K)=>se+K,0)/e.clickIntervals.length,F=e.
clickIntervals.reduce((se,K)=>se+Math.pow(K-M,2),0)/e.clickIntervals.length;C=M,T=M>0?Math.sqrt(F)/M:
1}return{window_ms:Math.round(y),clicks:e.clicks,keys:e.keys,moves:e.moves,fast_clicks:e.fastClicks,
fast_keys:e.fastKeys,untrusted_input:!!e.untrustedInput,click_burst:b,key_burst:w,avg_click_ms:C,click_cv:T,
event_rate:(e.clicks+e.keys+e.moves)/(y/1e3),pointer_speed_max:e.speedMax,pointer_speed_avg:x}},"com\
puteStats"),d=a(y=>y.fast_clicks>=4||y.fast_keys>=8||y.click_burst>=8||y.key_burst>=14||y.event_rate>=
20||y.avg_click_ms>0&&y.avg_click_ms<160&&y.click_cv<.08,"isSuspicious"),p=a(async(y=!1,b={})=>{if(!e.
enabled)return;const w=performance.now();if(!y&&w-e.lastSend<3e3)return;e.lastSend=w;const x=l();if(!(!b.
forceReport&&x.clicks+x.keys+x.moves===0&&!x.untrusted_input)&&!(!y&&!x.untrusted_input&&!d(x))){x.turnstile_token=
await getTurnstileToken(),botConfig&&botConfig.turnstileSiteKey&&!x.turnstile_token&&!botDetectionVerified&&
botDetectionOverlayShown&&(x.turnstile_failed=!0,x.challenged=!0);try{const C=await apiFetch("/api/b\
ot-telemetry",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(x)});if(C.
status===403){let T=null;try{T=await C.json()}catch{}if(T&&T.error==="banned"){showToast("\u30DC\u30C3\u30C8\u5224\u5B9A\u306B\u3088\u308ABA\
...
()=>{if(!e.enabled)return!1;const y=l();return d(y)},"looksSuspicious")}})();function openFileViewer(e,t=""){
...
g","json","docx"];if(i.includes(n)){openImageViewer(e);return}const l=get("file-viewer"),d=get("file\
-viewer-body"),p=get("file-viewer-title");if(!(!l||!d||!p)){if(p.textContent=t||"File Preview",d.replaceChildren(),
s.includes(n)){const g=document.createElement("video");g.src=String(e),g.controls=!0,g.playsInline=!0,
g.preload="metadata",d.appendChild(g)}else if(o.includes(n)){const g=document.createElement("audio");
...
"\u3053\u306E\u5F62\u5F0F\u306F\u30D7\u30EC\u30D3\u30E5\u30FC\u3067\u304D\u307E\u305B\u3093\u3002"));
const h=document.createElement("div");h.className="mt-3 flex justify-center gap-2";const y=document.
...
b.textContent="\u65B0\u3057\u3044\u30BF\u30D6\u3067\u958B\u304F",h.append(y,b),g.appendChild(h),d.appendChild(
g)}l.classList.add("visible")}}a(openFileViewer,"openFileViewer");function closeFileViewer(){const e=get(
...
d){if(d!==null){const C=document.createElement("div");C.className="border-t border-gray-700/50 my-1.\
5",r.appendChild(C)}if(p.tabId!==s){const C=document.createElement("div");C.className="text-[10px] t\
ext-gray-500 px-1 pb-1 font-bold",C.textContent="\u25BC "+(TAB_LABELS[p.tabId]||p.tabId),r.appendChild(
C)}d=p.tabId}const h=document.createElement("div");h.className="settings-search-result-item flex ite\
ms-start gap-2.5 px-3 py-2.5 rounded-lg cursor-pointer transition-all duration-150",h.style.animation=
"fadeIn 0.28s cubic-bezier(0.22, 1, 0.36, 1) both",h.style.animationDelay=g*30+"ms";const y=document.
...
b.appendChild(w),b.appendChild(x),h.appendChild(y),h.appendChild(b),h.addEventListener("click",()=>jumpToSetting(
...
!b)return;const C=Math.max(0,Math.min(x,t.scrollLeft+b));w&&typeof t.scrollTo=="function"?t.scrollTo(
{left:C,behavior:"smooth"}):t.scrollLeft=C,syncSettingsTabsOverflow()},"scrollTabsBy"),h=a(()=>{l=0,
o&&(clearTimeout(o),o=0),r&&(cancelAnimationFrame(r),r=0)},"stopHold"),y=a(b=>{h(),l=b,e.classList.toggle(
"is-edge-left",b<0),e.classList.toggle("is-edge-right",b>0),g(b*Math.max(120,t.clientWidth*.55),!0),
o=setTimeout(()=>{const w=a(()=>{l&&(g(l*14,!1),r=requestAnimationFrame(w))},"step");r=requestAnimationFrame(
...
t);if(w<=2)return;const C=Math.abs(b.deltaY)>=Math.abs(b.deltaX)?b.deltaY:b.deltaX;if(!C)return;const T=Math.
max(0,Math.min(w,t.scrollLeft+C));T!==t.scrollLeft&&(b.preventDefault(),t.scrollLeft=T,syncSettingsTabsOverflow())},
...
min(Math.abs(g)/(p*.45),.55),.4));const h=viewerSwipe.adjacent;if(h){const y=Number(h.dataset.dir)||
0;h.style.transition="none",h.style.transform=`translate(-50%, -50%) translateX(${y*p+n}px) scale(0.\
...
0%) translateX(${o*i}px) scale(0.97)`,d.style.opacity="0",setTimeout(()=>{g.isConnected&&g.remove()},
...
"sts-rate-out"),p=get("sts-thinking-wrap"),g=get("sts-note"),h=get("sts-voice-wrap"),y=get("sts-auto\
-play-wrap"),b=get("sts-mode-label"),w=isTranscriptionModel()||isGeminiLiveTranscribeModel(),x=get("\
sts-lang-wrap");if(w){b&&(b.textContent="Realtime Speech-to-Text"),h&&h.classList.add("hidden"),y&&y.
classList.add("hidden"),i&&i.classList.add("hidden"),r&&r.classList.add("hidden"),p&&p.classList.add(
"hidden"),x&&x.classList.add("hidden");const C=get("sts-transcribe-wrap"),T=get("sts-custom-vocab-wr\
ap");C&&C.classList.toggle("hidden",!isGeminiLiveTranscribeModel()),T&&T.classList.toggle("hidden",!isGeminiLiveTranscribeModel()),
...
t==="openai"?(b&&(b.textContent="Speech-to-Speech Live"),h&&h.classList.remove("hidden"),y&&y.classList.
...
"Speech-to-Speech Live"),h&&h.classList.remove("hidden"),y&&y.classList.remove("hidden"),setSelectOptions(
...
t==="gemini"&&(b&&(b.textContent="Speech-to-Speech Live"),h&&h.classList.remove("hidden"),y&&y.classList.
...
p&&g&&(g.textContent=`${Number(p.value||1).toFixed(2)}x`)}a(updateTtsUi,"updateTtsUi");let mcpServers=[],
...
\u30B9\u30BF\u30E0</span>',h=mcpAuthBlock(e),y=i?mcpOauthClientBlock(e):"";return`
...
"",h=(s&&s.value||"").trim();if(!l||!d){o&&(o.textContent="\u8868\u793A\u540D\u3068URL\u306F\u5FC5\u9808\u3067\u3059",
o.style.color="#f87171");return}r&&(r.disabled=!0),o&&(o.textContent="\u63A5\u7D9A\u30C6\u30B9\u30C8\u4E2D...",
o.style.color="#9ca3af");const y={name:l,url:d,auth_type:p,description:g};p==="bearer"&&h&&(y.bearer_token=
...
"save-oauth"){o.preventDefault();const p=r.getAttribute("data-pk")||"",g=r.closest("[data-mcp-server\
]")||document,h=g.querySelectorAll('[data-oauth-role="cid"], .mcp-oauth-edit-cid'),y=g.querySelectorAll(
'[data-oauth-role="secret"], .mcp-oauth-edit-sec'),b=h.length?h[h.length-1].value:"",w=y.length?y[y.
...
y=await h.json().catch(()=>({}));if(!h.ok){mcpStatusMsg("mcp-status-msg",y.error||"\u4FDD\u5B58\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
...
"deep-research-")||t.startsWith("antigravity-")),d=a((...g)=>g.forEach(h=>n.push(h,h.replace(/-/g," "))),
...
"gpt-5.6-"),h=t.includes("grok-4.6"),y=t.includes("grok-4.3")||t.includes("grok-4.5")||h||t.includes(
...
pt-5")||i;y&&d("medium"),b&&d("none"),(g||i)&&d("max"),(h||t.includes("multi-agent")||g)&&d("xhigh")}
return t.includes("claude")&&d("thinking","\u601D\u8003","thinking budget","budget"),e.agenticView&&
d("agentic view"),[...new Set(n)]}a(getModelCapabilitySearchTerms,"getModelCapabilitySearchTerms");const modelListGroups=[];
...
"alpha-bar");setTimeout(()=>{if(s){const c=document.getElementById("version-display");if(c){const u=s.
getBoundingClientRect(),m=c.getBoundingClientRect(),f=m.left+m.width/2-(u.left+u.width/2),v=m.top+m.
height/2-(u.top+u.height/2);s.style.transform=`translate(${f}px, ${v}px) scale(0.1)`,s.style.opacity=
...
forEach(v=>{v.value!=="1K"&&(v.disabled=m)}),m&&f.value!=="1K"&&(f.value="1K"))})}a(r,"updateGeminiI\
mageUi");function l(){const c=get("grok-image-options");if(!c)return;const u=(get("model-select").value||
...
"grok-image-quality")?get("grok-image-quality").parentElement:null;S&&S.classList.toggle("hidden",!v)}else
...
"hidden",!v)}}a(l,"updateGrokImageUi");function d(){var f;const c=get("grok-video-options");if(!c)return;
...
agine-video-1.5"&&m.value==="1080p"&&(m.value="720p")}}a(d,"updateGrokVideoUi");function p(){var v;const c=get(
...
"hidden",u==="gemini-omni-1.1-flash")}a(p,"updateGeminiVideoUi");function g(){const c=get("gemini-mu\
...
"updateGeminiMusicUi");function h(){var _;const c=get("xai-chat-options");if(!c)return;const u=String(
...
disabled=k,k&&(v.value=""))}a(h,"updateXaiChatUi");function y(){const c=isMistralOcrModel(),u=get("m\
...
setBrowserFastModeEnabled(!1))}a(y,"updateMistralOcrUi");function b(){const c=get("image-input-limit\
...
a(b,"updateImageInputLimits");function w(){const c=get("model-select");if(!c)return;const u=c.value,
m=String(u||"").toLowerCase(),f=m.includes("deepseek"),v=get("thinking-options"),k=get("reasoning-ef\
fort-container"),_=get("enable-thinking"),S=get("thinking-level"),A=get("thinking-budget"),B=get("en\
able-search"),P=get("search-container"),z=get("url-context-container"),te=get("enable-maps"),I=get("\
maps-grounding-container"),E=get("enable-sys-prompt"),W=get("sys-prompt-option"),J=get("enable-pytho\
n"),be=get("python-container"),V=get("prompt-cache-container"),ne=get("enable-prompt-cache"),ye=u===
"gpt-5-search-api",Ee=u.includes("tts"),Ie=isMistralOcrModel(u),Ve=m.includes("gemini-3.1-flash-lite\
-image"),Ye=m.includes("gemini-3.1-flash-image")&&!Ve,At=isClaudeModelKey(u),bt=isLlmModel()&&!f&&!Ee&&
...
V.classList.add("opacity-50","pointer-events-none"))),updatePromptCacheUi(),v&&v.classList.add("hidd\
...
add("hidden"),_&&(_.disabled=!1),A&&(A.disabled=!0,A.classList.add("opacity-50"));const ct=isGeminiImageModelKey(
u);if(Ee||Ie)P&&(get("enable-search").checked=!1,P.classList.add("opacity-50","pointer-events-none")),
...
"opacity-50","pointer-events-none")),E&&W&&(E.checked=!1,E.disabled=!0,W.classList.add("opacity-50"));else if(Ye||
Ve)I&&te&&(te.checked=!1,I.classList.add("hidden","opacity-50","pointer-events-none")),v.classList.remove(
...
"opacity-50","pointer-events-none"));else if(u.includes("gemini")&&!ct){v.classList.remove("hidden"),
z&&z.classList.remove("hidden","opacity-50","pointer-events-none");const de=u.includes("gemini-3");I&&
(de?I.classList.remove("hidden","opacity-50","pointer-events-none"):(te&&(te.checked=!1),I.classList.
add("hidden","opacity-50","pointer-events-none")));const $e=u.includes("flash");Array.from(S.options).
forEach(ke=>{u==="gemini-3.8-flash"||u==="gemini-3.7-flash"?ke.disabled=!["low","medium","high"].includes(
ke.value):u==="gemini-3.6-flash"?ke.disabled=!["medium","high"].includes(ke.value):u==="gemini-3.5-f\
lash-lite"?ke.disabled=!["minimal","medium","high"].includes(ke.value):["minimal","medium"].includes(
ke.value)?ke.disabled=!$e:ke.disabled=!1}),(u==="gemini-3.8-flash"||u==="gemini-3.7-flash")&&!["low",
...
_&&(_.disabled=!1),A&&u.includes("gemini-2.5")&&(A.disabled=!1,A.classList.remove("opacity-50")),A&&
!u.includes("gemini-2.5")&&(A.disabled=!0,A.classList.add("opacity-50"))}if(isLlmModel()&&(m.includes(
...
events-none"),(!ct||Ye)&&!u.includes("gpt-image")&&(E.disabled=!1,W.classList.remove("opacity-50"))),
(ct&&!Ye||u.includes("gpt-image")||isGrokImageModel()||isGrokVideoModel()||Ie)&&E&&W&&(E.checked=!1,
E.disabled=!0,W.classList.add("opacity-50")),be&&(isLlmModel()?(be.classList.remove("hidden"),J.disabled=
...
"opacity-50","pointer-events-none"))):B&&!u.includes("tts")&&!Ie&&!f&&!Ve&&(B.disabled=!1);const oe=get(
"mask-btn");oe&&(isGptImageModel()?oe.classList.remove("hidden"):(oe.classList.add("hidden"),currentMaskImage=
null,updateMaskPreview())),updateTtsUi(),updateStsUi(),updateStsOptions(),o(),r(),l(),d(),p(),g(),h(),
y(),b(),purgeUnsupportedAttachments(!0),refreshMinimalOptionsIfOpen(),applyMcpPromptChipUi()}a(w,"to\
...
x.checked))),syncCanvasModeUi(canvasModeEnabled,{persist:!1,skipReset:!1});const C=get("enable-codin\
...
let _="";try{const E=hljs.getLanguage(f)?f:"plaintext";activeStreamingBubbleId&&v.length>2e4?_=escapeHtml(
v):_=hljs.highlight(v,{language:E}).value}catch{_=escapeHtml(v)}const S=encodeURIComponent(v).replace(
/'/g,"%27"),A=detectBlockedScriptsInCode(v),B=hashString(`${f||"TEXT"}
${v||""}`);let P="";if(canvasModeEnabled){const E=String(canvasPreviewState.selectedKey||"")===B,W=E?
"Canvas\u3067\u8868\u793A\u4E2D":"Canvas\u3067\u30D7\u30EC\u30D3\u30E5\u30FC\u3059\u308B";P=`<button\
 class="canvas-preview-btn${E?" canvas-active":""}" data-code="${S}" data-code-key="${B}" data-canva\
s-lang="${escapeHtml(f||"txt")}" title="${W}" aria-label="${W}" aria-pressed="${E?"true":"false"}"><\
i class="fas ${E?"fa-layer-group":"fa-window-restore"}"></i></button>`}else if(isHtmlPreviewCandidate(
k,v)){const E=A?"\u30BB\u30FC\u30D5\u30D7\u30EC\u30D3\u30E5\u30FC":"\u30D7\u30EC\u30D3\u30E5\u30FC";
P=`<button class="html-preview-btn" data-code="${S}" ${A?'data-suspicious="1"':""} title="${E}" aria\
-label="${E}"><i class="fas ${A?"fa-shield-halved":"fa-up-right-from-square"}"></i></button>`}const z=`\
...
</button>`,I=(f||"TEXT")+(A?' <span class="suspicious-badge" title="polyfill.io \u306A\u3069\u306E\u5371\u967A\u30B9\u30AF\u30EA\u30D7\u30C8URL\u3092\u691C\u51FA\u3057\u307E\u3057\u305F\
...
lank">${m}</a>`},image(c,u,m){const f=escapeHtml(m||""),v=u?` title="${escapeHtml(u)}"`:"";if(String(
c||"").startsWith("sandbox:"))return`<span class="text-xs text-gray-500" title="${escapeHtml(c)}">${f||
"\uFF08\u753B\u50CF\u30C7\u30FC\u30BF\u306F\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\uFF09"}\
</span>`;const k=escapeHtml(c||"");return`<img src="${c}" data-viewer-src="${k}" alt="${f}"${v} clas\
...
setItem("version_notified",c),hideModal("version-update-modal")});const M=get("version-update-clear-\
cache");if(M&&(M.checked=!!(window.CHAT_CONFIG&&window.CHAT_CONFIG.clearCacheOnVersionUpdate),M.addEventListener(
"change",()=>{versionUpdateCachePreferenceSavePromise=saveVersionUpdateCachePreference(M.checked)})),
(Xn=get("version-update-reload"))==null||Xn.addEventListener("click",async()=>{var u;await versionUpdateCachePreferenceSavePromise.
...
c)}}else{const c=get("turnstile-container");c&&c.classList.add("hidden")}const F=a(c=>{if(!c)return"\
\u4E0D\u660E";const u=new Date(c);return Number.isNaN(u.getTime())?c:u.toLocaleString()},"formatSess\
ionTime"),se=a(c=>{const u=Array.isArray(c)?c:[],m=get("passkey-list"),f=get("passkey-count");if(f&&
(f.innerText=String(u.length)),!!m){if(!u.length){m.innerHTML='<div class="text-[11px] text-gray-500\
">\u767B\u9332\u6E08\u307F\u306E\u30D1\u30B9\u30AD\u30FC\u306F\u3042\u308A\u307E\u305B\u3093\u3002</div>';
return}m.innerHTML="",u.forEach((v,k)=>{const _=v&&v.id?String(v.id):"",S=document.createElement("di\
...
v&&v.created_at?`\u767B\u9332\u65E5\u6642: ${F(v.created_at)}`:"\u767B\u9332\u65E5\u6642: \u4E0D\u660E",
A.appendChild(B),A.appendChild(P),S.appendChild(A);const z=document.createElement("button");z.type="\
...
_)),S.appendChild(z),m.appendChild(S)})}},"renderPasskeyList"),K=a(c=>{const u=get("session-list");if(u){
if(!c||!c.length){u.innerHTML='<div class="text-xs text-gray-500">\u30A2\u30AF\u30C6\u30A3\u30D6\u306A\u30BB\u30C3\u30B7\u30E7\u30F3\u306F\u3042\u308A\u307E\u305B\u3093\u3002</div>';
...
/ \u4F5C\u6210: ${escapeHtml(F(m.created_at))}</div></div>${k}</div>`}).join(""),u.querySelectorAll(
...
"error",!0)}})}},"renderSessions"),he=a(async()=>{const c=get("session-list");c&&(c.innerHTML='<div \
...
return}const f=(m.sessions||[]).filter(v=>!v.is_revoked);K(f)},"loadSessions"),N=a(()=>{const c=get(
"session-refresh-btn");c&&(c.onclick=()=>he());const u=get("session-revoke-others-btn");u&&(u.onclick=
...
!c.default_enable_system_prompt&&!c.use_last_chat_settings&&(u.checked=!0),w())}).catch(()=>{}),installAdminSidebarDebugObserver(),
...
mpression-format-only");Q&&(Q.onchange=()=>{const c=Q.checked,u=get("compression-max-size"),m=get("c\
ompression-max-dim");u&&(u.disabled=c),m&&(m.disabled=c);const f=get("compression-size-wrap"),v=get(
"compression-dim-wrap");f&&(f.style.opacity=c?"0.4":"1"),v&&(v.style.opacity=c?"0.4":"1")});const ue=a(
...
sage-refresh");ae&&(ae.onclick=()=>loadStorageUsage());let ce=null;const _e=a(()=>{const c=new Uint8Array(
16);return window.crypto.getRandomValues(c),Array.from(c,u=>u.toString(16).padStart(2,"0")).join("")},
"createAccountTransferId"),X=a((c={})=>{const u=get("account-transfer-progress"),m=get("account-tran\
sfer-progress-bar"),f=get("account-transfer-progress-percent"),v=get("account-transfer-progress-text"),
k=get("account-transfer-progress-detail"),_=Math.max(0,Math.min(100,Number(c.progress)||0));if(u&&u.
classList.remove("hidden"),m&&(m.style.width=`${_}%`),f&&(f.textContent=`${Math.round(_)}%`),v&&(v.textContent=
c.message||"\u51E6\u7406\u72B6\u6CC1\u3092\u78BA\u8A8D\u3057\u3066\u3044\u307E\u3059"),k){const A={queued:"\
...
ed","cancelled","expired"].includes(c.phase))},"renderAccountTransferProgress"),ve=a(c=>{G&&(G.disabled=
!!c);const u=get("account-import-btn");u&&(u.disabled=!!c);const m=get("account-transfer-cancel-btn");
m&&(m.disabled=!c)},"setAccountTransferControls"),re=a((c={})=>{const u=get("account-export-ready"),
m=get("account-export-ready-text"),f=get("account-export-expiry"),v=get("account-export-download-btn"),
k=!!(c.available&&c.download_url);if(u&&u.classList.toggle("hidden",!k),!k){v&&v.removeAttribute("hr\
ef");return}const _=Math.max(0,Number(c.size_bytes)||0),S=_>=1024*1024*1024?`${(_/(1024*1024*1024)).
toFixed(2)} GB`:`${(_/(1024*1024)).toFixed(1)} MB`;if(m){const A=Number(c.unreadable_count)>0?`\uFF08\u8AAD\u53D6\u4E0D\u80FD\
 ${Number(c.unreadable_count)}\u4EF6\u3092\u5FA9\u65E7\u7528\u3068\u3057\u3066\u53CE\u9332\uFF09`:"";
m.textContent=`\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8ZIP\u3092\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u3067\u304D\u307E\u3059\uFF1A${S}${A}`}
...
"\u5B8C\u6210\u304B\u30891\u6642\u9593\u5F8C\u306B\u81EA\u52D5\u524A\u9664\u3055\u308C\u307E\u3059\u3002"}
v&&(v.href=c.download_url)},"renderAccountExportAvailability"),j=a(async c=>{for(;ce===c&&!c.stopped;){
try{const u=await apiFetch(`/api/account/transfer/${c.id}`,manualSpinnerRequestOptions({cache:"no-st\
ore"})),m=await u.json().catch(()=>({}));if(u.ok&&(m.state!=="pending"&&X(m),["ready","completed","f\
ailed","cancelled","expired"].includes(m.state)))return m}catch{}await new Promise(u=>setTimeout(u,700))}
return null},"pollAccountTransfer"),L=a((c,u,m=!0)=>{u&&(X(u),re(u),m&&u.state==="ready"?showToast(u.
...
"error",!0),q(c))},"handleFinishedAccountExport"),R=a(async()=>{try{const c=await apiFetch("/api/acc\
ount/export/latest",manualSpinnerRequestOptions({cache:"no-store"})),u=await c.json().catch(()=>({}));
if(!c.ok)return;if(re(u),u.state==="ready"){X(u);return}if(["failed","cancelled","expired"].includes(
u.state)){X(u);return}if(!["queued","running","cancelling"].includes(u.state)||!u.job_id||ce&&ce.id===
u.job_id||ce)return;const m={id:u.job_id,type:"export",stopped:!1,restored:!0};ce=m,ve(!0),X(u);const f=await j(
m);f&&L(m,f,!0)}catch{}},"refreshLatestAccountExport"),q=a(c=>{ce===c&&(ce=null),c.stopped=!0,ve(!1)},
...
\u3066\u3044\u307E\u3059"});try{const u=await apiFetch("/api/account/export",manualSpinnerRequestOptions(
{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({job_id:c.id}),keepalive:!0})),
m=await u.json().catch(()=>({}));if(u.status===409&&m.error==="export_in_progress"&&m.job_id)c.id=m.
job_id;else if(!u.ok)throw new Error(m.error==="rate_limit"?"\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u56DE\u6570\u306E\u4E0A\u9650\u306B\u9054\u3057\u307E\u3057\u305F":
m.error||"\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u3092\u958B\u59CB\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F");
X({progress:0,phase:"queued",message:"\u30D0\u30C3\u30AF\u30B0\u30E9\u30A6\u30F3\u30C9\u3067\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u3057\u3066\u3044\u307E\u3059"});
const f=await j(c);!c.cancelRequested&&f&&L(c,f,!0)}catch(u){const m=u&&u.message?u.message:"\u30A8\u30AF\u30B9\u30DD\u30FC\u30C8\u3092\
...
ick",async c=>{const u=Z.getAttribute("href");if(!(!u||u==="#")){c.preventDefault();try{const m=await apiFetch(
...
"warning",!0),R())}catch{window.location.assign(u)}}}),ve(!1),R();const we=get("import-files-grid"),
H=get("import-files-info"),Te=get("import-files-summary"),He=a(c=>{const u=Math.max(0,Number(c)||0);
return u>=1024*1024*1024?`${(u/(1024*1024*1024)).toFixed(2)} GB`:u>=1024*1024?`${(u/(1024*1024)).toFixed(
1)} MB`:u>=1024?`${Math.round(u/1024)} KB`:`${u} B`},"importFormatBytes");let Se=null;const it=a(()=>{
if(!Se)return;const c=Se.files,u=Se.selection;let m=0;c.forEach(k=>{u.has(k.archive_path)&&(m+=Number(
k.size_bytes)||0)});const f=Number(Se.available_bytes)||0,v=m>f;Te&&(Te.textContent=`\u9078\u629E\u4E2D: ${He(
m)} / \u5229\u7528\u53EF\u80FD: ${He(f)}${v?" \uFF08\u5BB9\u91CF\u8D85\u904E\uFF09":""}`,Te.classList.
toggle("text-red-300",v)),H&&(H.textContent=`${c.length} files`)},"updateImportFileSelectionUi"),Qe=a(
...
it();return}c.forEach(u=>{const m=document.createElement("label"),f=Se.selection.has(u.archive_path);
...
u.display_name)}</div><div class="text-[10px] text-gray-500">${He(u.size_bytes)}</div></div>`;const v=m.
querySelector(".import-file-check");v.addEventListener("change",()=>{v.checked?Se.selection.add(u.archive_path):
Se.selection.delete(u.archive_path),m.classList.toggle("border-blue-500",v.checked),m.classList.toggle(
"border-gray-600",!v.checked),it()}),we.appendChild(m)}),it()},"renderImportFileItems"),ft=a(c=>new Promise(
u=>{if(Se={files:c.files||[],selection:new Set((c.files||[]).map(m=>m.archive_path)),available_bytes:c.
available_bytes,resolve:u},Qe(),!get("import-files-modal")){u(null);return}showModal("import-files-m\
odal")}),"showImportFileSelection"),at=a(c=>{if(hideModal("import-files-modal"),Se){const u=Se.resolve;
Se=null,u(c)}},"closeImportFileSelection"),xt=get("import-files-close");xt&&(xt.onclick=()=>at(null));
...
enable_client_debug_log:"\u30C7\u30D0\u30C3\u30B0\u30ED\u30B0\u306E\u62E1\u5F35\u9001\u4FE1"},st=a(c=>{
if(c===!0)return"ON";if(c===!1)return"OFF";if(c==null||c==="")return"\u672A\u8A2D\u5B9A";const u=String(
c);return u.length>60?u.slice(0,60)+"\u2026":u},"formatAccountSettingValue");let ut=null;const kt=a(
c=>{if(ut){const u=ut;ut=null,hideModal("settings-confirmation-modal"),u(c)}},"resolveSettingsImport\
Confirmation"),_t=a(c=>new Promise(u=>{if(!get("settings-confirmation-modal")){u(!0);return}ut=u;const f=Array.
isArray(c&&c.settings_changes)?c.settings_changes:[],v=get("settings-confirmation-list");v&&(f.length?
v.innerHTML=f.map(_=>{const S=It[_.field]||_.field,A=st(_.current),B=st(_.incoming);return`<div clas\
...
Le){const c=a(()=>Le.classList.toggle("hidden",!xe.checked),"syncInplaceWarn");xe.addEventListener("\
change",c),c()}ie&&(ie.onclick=async()=>{const c=get("account-import-file"),u=c&&c.files?c.files[0]:
null,m=get("account-import-categories"),f=m?Array.from(m.querySelectorAll('input[type="checkbox"]:ch\
ecked')).map(I=>I.value):[],v=get("account-import-inplace"),k=!!(v&&v.checked),_=get("account-import\
-settings-bypass"),S=!!(_&&_.checked);let A=!1;if(!u){showToast("\u30A4\u30F3\u30DD\u30FC\u30C8\u3059\u308BZIP\u30D5\u30A1\u30A4\u30EB\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044",
...
const z=get("account-import-result");let te=Promise.resolve(null);try{const E=Math.max(1,Math.ceil(u.
size/10485760)),W=await apiFetch("/api/account/import/upload/start",manualSpinnerRequestOptions({method:"\
POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({size:u.size}),signal:P.controller.
signal})),J=await W.json().catch(()=>({}));if(!W.ok)throw new Error(J.error||"\u30A2\u30C3\u30D7\u30ED\u30FC\u30C9\u3092\u958B\u59CB\u3067\u304D\u307E\u305B\u3093");
P.uploadId=J.upload_id;const be=J.chunk_size||10485760;let V=0,ne=0;const ye=a(async()=>{for(;;){const me=ne++;
if(me>=E)return;const oe=u.slice(me*be,Math.min(u.size,(me+1)*be)),de=new FormData;de.append("chunk",
oe,u.name),de.append("index",String(me));const $e=await apiFetch(`/api/account/import/upload/${encodeURIComponent(
...
V++,X({progress:Math.min(35,Math.round(V/E*35)),phase:"uploading",message:`ZIP\u3092\u4E26\u5217\u30A2\u30C3\u30D7\u30ED\u30FC\u30C9\u3057\u3066\u3044\u307E\u3059\uFF08${V}\
...
Ee&&window.ConnectionMonitor&&window.ConnectionMonitor.operationEnded()}let Ie="",Ve=!1,Ye=0;const At=a(
async()=>{let me=!1;const oe=a(()=>{me||(me=!0,setTimeout(()=>{location.reload()},1100))},"scheduleR\
...
!!$e.compact_prompt_mode)}catch{}oe()},"refreshSettingsFormAfterImport"),bt=a(me=>{const oe=me&&me.message||
...
tus"),ct=a(async()=>{const me=await yt();if(!me)return{status:"unknown"};if(me.state==="completed")return bt(
...
settings_changes})?(A=!0,{status:"reselect"}):(X({progress:0,phase:"cancelled",message:"\u8A2D\u5B9A\u306E\u30A4\u30F3\u30DD\u30FC\u30C8\u3092\u30AD\u30E3\u30F3\
...
return{status:"unknown"}},"settleUnreadableImport");for(;!Ve;){P.stopped=!0,await te.catch(()=>null),
...
categories:f.join(","),job_id:P.id,selected_files:Ie,restore_inplace:k,confirm_settings:A||S}),signal:P.
controller.signal}))}catch(Be){if(P.cancelRequested||Be&&Be.name==="AbortError")throw Be;const tt=await ct();
if(tt.status==="done"){Ve=!0;break}if(tt.status==="cancelled")return;if(tt.status==="reselect")continue;
...
one"){Ve=!0;break}if(Be.status==="cancelled")return;if(Be.status==="reselect")continue;if(me.ok)throw new Error(
...
DELETE"})).catch(()=>null);return}A=!0;continue}if(!me.ok)throw new Error(oe.error||"\u30A4\u30F3\u30DD\u30FC\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F");
...
`/api/account/import/upload/${encodeURIComponent(P.uploadId)}`,manualSpinnerRequestOptions({method:"\
DELETE"})).catch(()=>null),P.cancelRequested||I&&I.name==="AbortError")return;const E=I&&I.message?I.
message:"",W=E==="storage_limit_exceeded"?"\u30B9\u30C8\u30EC\u30FC\u30B8\u4E0A\u9650\u3092\u8D85\u3048\u308B\u305F\u3081\u30A4\u30F3\u30DD\u30FC\u30C8\u3067\u304D\u307E\u305B\u3093":
E||"\u30A4\u30F3\u30DD\u30FC\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F";X({progress:0,phase:"f\
ailed",message:W}),z&&(z.textContent=W,z.classList.remove("hidden","text-emerald-300"),z.classList.add(
"text-red-300")),showToast(W,"error",!0)}finally{P.stopped=!0,await te.catch(()=>null),q(P)}});const je=get(
"account-dedupe-btn"),De=get("account-dedupe-result"),et=a((c,u=!1)=>{De&&(De.textContent=c,De.classList.
remove("hidden"),De.classList.toggle("text-red-300",!!u),De.classList.toggle("text-emerald-300",!u))},
"showDedupeResult");je&&(je.onclick=async()=>{const c=a(async()=>{const u=await apiFetch("/api/accou\
...
0&&f.push(`${v[P]} ${z}\u4EF6`)}const k=Number(m.kept_referenced_files)>0?`
...
const A=[];for(const P of["chats","gems","files","feedback","diagnostics"]){const z=Number(S.removed&&
S.removed[P])||0;z>0&&A.push(`${v[P]} ${z}\u4EF6`)}const B=Number(S.kept_referenced_files)>0?`\uFF08\u53C2\u7167\u306E\u305F\u3081\
...
try{await c()}catch(u){et(u&&u.message||"\u91CD\u8907\u30C7\u30FC\u30BF\u306E\u4FEE\u5FA9\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
...
const Oe=get("enc-scan-result"),qe=a(async(c=null)=>{Oe&&(Oe.textContent="\u30B9\u30AD\u30E3\u30F3\u4E2D...");
let u="/api/encryption_scan";c&&(u+=`?thread_id=${encodeURIComponent(c)}`);try{const m=await apiFetch(
u,{cache:"no-store"}),f=await m.json();if(!m.ok){Oe&&(Oe.textContent=f.error||"\u5931\u6557\u3057\u307E\u3057\u305F");
return}const v=f.total||0,k=f.encrypted||0,_=f.unencrypted||0;let S=`Total: ${v} / Encrypted: ${k} /\
 Plain: ${_}`;if(f.samples&&f.samples.length){const A=f.samples.slice(0,8).map(B=>{const P=B.timestamp?
new Date(B.timestamp).toLocaleString():"";return`#${B.id} (${B.role||""}) ${P}`}).join(" / ");S+=`<d\
iv class="text-[10px] text-gray-400 mt-1">\u4F8B: ${A}</div>`}Oe&&(Oe.innerHTML=S)}catch{Oe&&(Oe.textContent=
"\u5931\u6557\u3057\u307E\u3057\u305F")}},"runEncScan"),Wt=get("enc-scan-all");Wt&&(Wt.onclick=()=>qe(
...
list");let Nt=null,Ce=!1;const Ne=a(c=>!c||!c.length?null:c.some(u=>!!u.is_encrypted),"computeThread\
EncryptedFromMessages"),Ke=a(()=>{Nt=Ne(allMessages)},"refreshCurrentThreadEncStateFromMessages"),Vt=a(
...
`${v}\u306B\u5931\u6557\u3057\u307E\u3057\u305F`,"error",!0),!1}finally{Ce=!1}},"setAdminThreadEncry\
ption"),sn=a(c=>{if(!Ze)return;const u=c.threads||[];if(!u.length){Ze.innerHTML='<div class="text-[1\
1px] text-gray-400">\u30C1\u30E3\u30C3\u30C8\u304C\u3042\u308A\u307E\u305B\u3093\u3002</div>';return}
Ze.innerHTML=u.map(m=>{const f=m.encrypted_count>0?"enc":"plain",v=f==="enc"?"\u5FA9\u53F7\u5316":"\u518D\
\u6697\u53F7\u5316",k=f==="enc"?"bg-amber-600 hover:bg-amber-500":"bg-cyan-700 hover:bg-cyan-600",_=m.
updated_at?new Date(m.updated_at).toLocaleString():"",S=escapeHtml(String(m.thread_id)),A=currentThreadId&&
...
ded shrink-0" data-id="${S}" data-enable="${f==="enc"?"0":"1"}" data-progress-expected-slow="true">${v}\
</button>
                    </div>`}).join("")},"renderAdminEncThreads"),rt=a(async()=>{if(Ze){Ze.innerHTML=
...
</div>`;return}if(sn(u),currentThreadId&&Array.isArray(u.threads)){const m=u.threads.find(f=>String(
...
rt,window.__refreshAdminThreadEncState=Ke,window.__setAdminThreadEncryption=Vt;const le=get("encrypt\
...
.admin-enc-open");if(u){c.preventDefault();const S=u.getAttribute("data-id");if(!S)return;typeof Rt==
...
const _=m.textContent;m.textContent="\u51E6\u7406\u4E2D...";try{await Vt(f,v,{confirmPrompt:!1,reloadCurrent:!0})}finally{
m.disabled=!1,m.textContent=_,await rt()}}),get("file-input").onchange=c=>{const u=Array.from(c.target.
files||[]);c.target.value="",u.length&&handleFiles(u)},get("photo-input")&&(get("photo-input").onchange=
c=>{const u=Array.from(c.target.files||[]);c.target.value="",u.length&&handleFiles(u)});const ge=a(c=>{
...
ver:bg-rose-600 rounded" data-id="${m.id}">\u7533\u3057\u7ACB\u3066\u30D6\u30ED\u30C3\u30AF</button>
                    ...
                    `}).join("")}},"renderBanAppeals"),Ge=a(async(c=!1)=>{if(!isAdminUser)return;const u=get(
...
ealSummary"),Ue=a(async()=>{if(!isAdminUser)return;const c=get("ban-appeal-list");if(c){c.innerHTML=
'<div class="text-[11px] text-gray-500">\u8AAD\u307F\u8FBC\u307F\u4E2D...</div>';try{const u=await apiFetch(
"/api/ban/appeals?limit=80",{cache:"no-store"});if(!u.ok)return;const m=await u.json();ge(m.items||[]),
await Ge(!1)}catch{}}},"loadBanAppeals"),Xe=a(async(c=null)=>{if(!isAdminUser)return;const u=c?{ids:c}:
{all:!0};try{(await apiFetch("/api/ban/appeals/mark_read",{method:"POST",headers:{"Content-Type":"ap\
plication/json"},body:JSON.stringify(u)})).ok&&await Ue()}catch{}},"markBanAppealsRead"),lt=a(async c=>{
if(isAdminUser)try{(await apiFetch("/api/ban/appeals/update",{method:"POST",headers:{"Content-Type":"\
application/json"},body:JSON.stringify(c)})).ok&&await Ue()}catch{}},"updateBanAppealStatus"),St=a(()=>{
...
\u9664\u3055\u308C\u307E\u3059\u3002</div>
                ...
                `,c.appendChild(u)},"ensureTemporaryChatSettingsCard"),An=a(()=>{const c=get("set-st\
...
"success")})},"ensureLlmTranscribePromptSettingsUi"),an=[{key:"python",label:"Python \u5B9F\u884C\u6848\u5185"},
...
0 rounded p-2 text-[11px] text-gray-200":"w-full h-20 bg-gray-950 border border-gray-700 rounded p-2\
 text-xs text-gray-200";return an.map(f=>{const v=f.mcpLocked===!0,k=v?'<div class="text-[10px] text\
...
></textarea>
                ...
                `}).join("")},window.applyAutoSystemPromptConfigToForm=(c,u={})=>{an.forEach(m=>{const f=u&&
...
const En=a((c,u=null)=>{if(u){const m=get(u);m&&(m.checked=!0)}an.forEach(m=>{const f=get(`${c}-auto\
...
"resetAutoSystemPromptConfigToCodeDefaults"),Zn=a(c=>{const u={};return an.forEach(m=>{const f=get(`${c}\
...
"thread",!0)}</div>
                `,u.appendChild(m)},St(),An(),ue();const ei=a(()=>{const c=get("set-default-model");
...
pulateDefaultModelOptions"),ti=a(()=>{const c=get("set-default-vision-model");if(!c)return;const u=c.
value;c.innerHTML="",MODELS.forEach(f=>{const v=(f.items||[]).filter(_=>{const S=(_.id||"").toLowerCase();
...
tVisionModelOptions"),$n=a(c=>{if(!c)return;cacheUserSettings(c);const u=get("app-global-sys-prompt-\
...
"set-passkey-only-login"),S=get("passkey-only-note"),A=Array.isArray(c.passkey_credentials)?c.passkey_credentials:
[];if(se(A),_){_.checked=!!c.passkey_only_login;const I=A.length>0||!!c.has_webauthn;_.disabled=!I,I||
...
const E=parseInt(I[0]||"0",10),W=parseInt(I[1]||"0",10);P&&(P.innerText=`${E} / ${W}`),z&&W>0&&(z.style.
width=`${Math.min(100,Math.floor(E/W*100))}%`)}}else B.classList.add("hidden"),z&&(z.style.width="0%"),
...
"settings-open-before");const c=await ensureUserSettingsSnapshot();c&&$n(c);const u=get("search-box"),
...
"error",!0)),vn(),N(),he();try{loadMcpServers()}catch{}};const Rt=a((c=!1)=>{snapshotSidebarHistory(
...
me-presets .theme-swatch"),v=a((k,_=!0)=>{const S=normalizeHex(k);S&&(applyThemeColor(S,_),syncThemeInputs(
S))},"applyFromValue");c&&c.addEventListener("input",()=>v(c.value,!0)),u&&(u.addEventListener("chan\
...
"reset-set-auto-sys-prompt-defaults");u&&(u.onclick=()=>{En("set","set-apply-auto-sys-prompt-notices"),
...
try{const z=await _.json();z&&z.message&&(S=z.message)}catch{}Rt();const A=currentUsername,B=CHAT_CONFIG.
...
2)}`,"cacheBust"),v=a((V,ne)=>!V||!ne||ne<=0?0:V*8/(ne/1e3)/1e3/1e3,"toMbps"),k=a(V=>Number.isFinite(
V)?`${V.toFixed(0)} ms`:"-","fmtMs"),_=a(V=>Number.isFinite(V)?`${V.toFixed(V>=100?0:1)} Mbps`:"-","\
fmtMbps"),S=a(async(V,ne)=>{const ye=await V.json().catch(()=>({}));return ye&&ye.error?ye.error:ne},
"parseErr"),A=[];m("\u6E2C\u5B9A\u4E2D... ping");for(let V=0;V<4;V++){const ne=performance.now(),ye=await apiFetch(
`/api/speedtest/ping?_=${f()}`,{cache:"no-store"}),Ee=performance.now();if(!ye.ok)throw new Error(await S(
ye,"ping_failed"));await ye.json().catch(()=>({})),A.push(Ee-ne)}const B=A.reduce((V,ne)=>V+ne,0)/Math.
max(1,A.length),P=Math.min(...A),z=a(async V=>{const ne=performance.now(),ye=await apiFetch(`/api/sp\
eedtest/download?bytes=${V}&_=${f()}`,{cache:"no-store"});if(!ye.ok)throw new Error(await S(ye,"down\
load_failed"));const Ee=await ye.arrayBuffer(),Ie=performance.now();return{bytes:Ee.byteLength||V,ms:Ie-
ne,mbps:v(Ee.byteLength||V,Ie-ne)}},"runDownload");m(`\u6E2C\u5B9A\u4E2D... ping ${k(B)}
\u6E2C\u5B9A\u4E2D... download`);const te=[];for(const V of[2*1024*1024,8*1024*1024])te.push(await z(
V)),m(`\u6E2C\u5B9A\u4E2D... ping ${k(B)}
download ${_(Math.max(...te.map(ne=>ne.mbps)))}
\u6E2C\u5B9A\u4E2D... upload`);const I=Math.max(...te.map(V=>V.mbps)),E=a(async V=>{const ne=new Uint8Array(
V),ye=performance.now(),Ee=await apiFetch(`/api/speedtest/upload?_=${f()}`,{method:"POST",headers:{"\
Content-Type":"application/octet-stream"},body:ne,cache:"no-store"}),Ie=performance.now();if(!Ee.ok)
throw new Error(await S(Ee,"upload_failed"));const Ve=await Ee.json().catch(()=>({})),Ye=Number(Ve.bytes_received||
V)||V;return{bytes:Ye,ms:Ie-ye,mbps:v(Ye,Ie-ye),serverMs:Number(Ve.server_elapsed_ms||0)||0}},"runUp\
load"),W=[];for(const V of[1*1024*1024,4*1024*1024])W.push(await E(V));const J=Math.max(...W.map(V=>V.
mbps)),be=["\u7D50\u679C (\u30D6\u30E9\u30A6\u30B6\u21D4\u3053\u306E\u30B5\u30FC\u30D0\u30FC)",`Ping\
 (avg/min): ${k(B)} / ${k(P)}`,`Download (best): ${_(I)}`,`Upload (best): ${_(J)}`,`Download runs: ${te.
map(V=>`${Math.round(V.bytes/1024/1024)}MB=${_(V.mbps)}`).join(", ")}`,`Upload runs: ${W.map(V=>`${Math.
...
if(u.classList.contains("ban-appeal-mark")){m&&await Xe([Number(m)]);return}if(u.classList.contains(
"ban-appeal-status")){const f=u.getAttribute("data-status");m&&f&&await lt({id:Number(m),status:f});
return}if(u.classList.contains("ban-appeal-reply-send")){const f=u.closest("[data-appeal-id]"),v=f?f.
querySelector(".ban-appeal-reply"):null,k=v?v.value:"";m&&await lt({id:Number(m),admin_reply:k});return}
...
c=>setMarkerOpacity(c.target.value),bn.onchange=c=>setMarkerOpacity(c.target.value));const on=get("m\
arker-opacity-number");on&&(on.onchange=c=>setMarkerOpacity(c.target.value),on.onblur=c=>setMarkerOpacity(
c.target.value),on.onkeydown=c=>{c.key==="Enter"&&(setMarkerOpacity(c.target.value),c.target.blur())}),
...
!c.classList.contains("hidden"))},"isUploadModalOpen"),Ft=get("drop-overlay");let Jt=0;const ai=a(()=>{
si()||Ft&&(Ft.classList.remove("hidden"),Ft.classList.add("flex"))},"showDropOverlay"),Kt=a(()=>{Jt=
...
"drop",c=>{c.preventDefault(),c.stopPropagation(),mt.classList.remove("dragover"),Kt();const u=c.dataTransfer?
...
files))});const jn=get("bot-admin-modal"),oi=a(c=>{const u=get("bot-admin-list");if(u){if(u.innerHTML=
...
2 py-1 rounded" data-progress-expected-slow="true" data-user="${escapeHtml(m.username)}">\u524A\u9664</button>\
                    ...
                    `,u.appendChild(_)})}},"renderBotUsers"),Xt=a(async(c="")=>{const u=get("bot-adm\
...
c)}`),f=await m.json();m.ok&&f&&f.users?oi(f.users):(u&&(u.innerHTML='<div class="text-xs text-red-4\
...
showToast("\u30E6\u30FC\u30B6\u30FC\u4E00\u89A7\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
"error",!0)}},"loadBotUsers"),yn=a(async()=>{if(!isAdminUser||!(get("bot-admin-modal")||jn))return;const u=get(
...
dal",open:a(()=>yn(),"open")}},Fn=a((c,u=!1)=>{switch(c){case"settings-modal":Rt(u);break;case"uploa\
...
addEventListener("popstate",c=>{let u=!1;Object.values(Bt).forEach(v=>{const k=get(v.id);k&&k.classList.
contains("modal-open")&&location.pathname!==Object.keys(Bt).find(_=>Bt[_].id===v.id)&&(Fn(v.id,!0),u=
...
{skipHistory:!0});const f=Bt[location.pathname];if(f){const v=get(f.id);v&&!v.classList.contains("mo\
...
"",vn()};async function vn(){const u=await(await apiFetch("/api/feedback?all=1")).json(),m=get("fb-l\
...
ave").onclick=async()=>{const S=_.querySelector(".fb-status").value,A=_.querySelector(".fb-reply").value;
await apiFetch(`/api/feedback/${k.id}/update`,{method:"POST",headers:{"Content-Type":"application/js\
on"},body:JSON.stringify({status:S,admin_reply:A})}),vn()},v.appendChild(_)})):f.classList.add("hidd\
en")}if(a(vn,"loadFeedback"),window.setupTOTP=async()=>{const u=await(await apiFetch("/api/2fa/totp/\
...
\u305F","error",!0);return}const _=await(await ensureWebAuthnJson()).create({publicKey:v}),S=await apiFetch(
"/api/2fa/webauthn/register/verify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.
stringify(Object.assign({},_,{name:m}))}),A=await S.json().catch(()=>({}));S.ok?(u&&(u.value=""),showToast(
...
"";u&&m&&(c.preventDefault(),openImageViewer(m))});const Yt=document.querySelector(".viewer-content");
...
let ze,Me=null,rn=[],wn=!1,ln=null,Ht=null,Ct=null,Qt=null,xn=0,cn=!1,Zt=null,Dt=null,ht=null,en=null,
dn=null,Tt=null,un=null,pn=null;function Hn(){const c=get("mic-waveform");if(!c)return[];if(Array.isArray(
un)&&un.length)return un;c.innerHTML="";const u=[];for(let m=0;m<24;m++){const f=document.createElement(
...
"0.4",u.push(f),c.appendChild(f)}return un=u,u}a(Hn,"ensureMicWaveformBars");function Lt(c,u="hidden"){
const m=get("mic-recording-indicator"),f=get("mic-recording-text");if(m){if(pn&&(clearTimeout(pn),pn=
...
m.style.color="rgb(209 213 219)"}}a(Lt,"setMicRecordingIndicator");function Dn(){Hn().forEach(u=>{u.
style.height="2px",u.style.opacity="0.35"})}a(Dn,"resetMicWaveformBars");function Mt(){if(dn&&(cancelAnimationFrame(
dn),dn=null),en){try{en.disconnect()}catch{}en=null}if(Dt){try{Dt.close()}catch{}Dt=null}ht=null,Tt=
null,Dn()}a(Mt,"stopMicWaveform");function qn(c){Mt();const u=Hn();if(!u.length)return;const m=window.
AudioContext||window.webkitAudioContext;if(!m)return;try{Dt=new m,ht=Dt.createAnalyser(),ht.fftSize=
256,ht.smoothingTimeConstant=0,en=Dt.createMediaStreamSource(c),en.connect(ht),Tt=new Uint8Array(ht.
frequencyBinCount)}catch{Mt();return}const f=a(()=>{if(!ht||!Tt)return;ht.getByteFrequencyData(Tt);const v=Math.
max(1,Math.floor(Tt.length/u.length));for(let k=0;k<u.length;k++){const S=(Tt[Math.min(Tt.length-1,k*
v)]||0)/255,A=Math.max(2,Math.round(2+S*10));u[k].style.height=`${A}px`,u[k].style.opacity=`${.35+S*
.65}`}dn=requestAnimationFrame(f)},"render");f()}a(qn,"startMicWaveform");function mn(){if(ln&&(clearInterval(
ln),ln=null),Qt){try{Qt.disconnect()}catch{}Qt=null}if(Ht){try{Ht.close()}catch{}Ht=null}Ct=null}a(mn,
"stopSilenceMonitor");function Gn(c){if(!isStsModel()||!stsOpt("sts-auto-send"))return;mn();const u=window.
AudioContext||window.webkitAudioContext;if(!u)return;Ht=new u,Ct=Ht.createAnalyser(),Ct.fftSize=2048,
Qt=Ht.createMediaStreamSource(c),Qt.connect(Ct);const m=new Uint8Array(Ct.fftSize),f=getStsSilenceMs(),
v=.02;xn=0,cn=!1,ln=setInterval(()=>{if(!Ct)return;Ct.getByteTimeDomainData(m);let k=0;for(let S=0;S<
m.length;S++){const A=(m[S]-128)/128;k+=A*A}if(Math.sqrt(k/m.length)>v){cn||(cn=!0),xn=Date.now();return}
cn&&Date.now()-xn>f&&ze&&ze.state==="recording"&&ze.stop()},200)}a(Gn,"startSilenceMonitor");const Sn=class Sn{constructor(){
...
ing setup...");const S=!!(v&&v.transcriptionConfig),A={setup:{model:`models/${f}`,generationConfig:{
responseModalities:S?["TEXT"]:["AUDIO"]},inputAudioTranscription:S?v.transcriptionConfig||{}:{},outputAudioTranscription:{}}};
...
console.log("Sending setup:",JSON.stringify(A)),this.ws.send(JSON.stringify(A))},this.ws.onmessage=S=>this.
...
WebSocket.OPEN||!this.setupComplete)return;const A=S.inputBuffer.getChannelData(0),B=new Int16Array(
A.length);for(let P=0;P<A.length;P++)B[P]=Math.max(-1,Math.min(1,A[P]))*32767;this.ws.send(JSON.stringify(
...
_.length);for(let A=0;A<_.length;A++)S[A]=_.charCodeAt(A);this.assistantAudioChunks.push(S)}}),f.outputTranscription&&
...
onloadend=()=>m(f.result.split(",")[1]),f.readAsDataURL(u)})}};a(Sn,"GeminiLiveClient");let kn=Sn;const Cn=class Cn{constructor(u=24e3){
...
S.start(A),this.nextStartTime=A+_.duration}stop(){this.ctx&&(this.ctx.close(),this.ctx=null)}};a(Cn,
...
`)){if(!S.startsWith("data: "))continue;let A=null;try{A=JSON.parse(S.slice(6))}catch{continue}this.
...
audioCtx=new u({sampleRate:this.rateIn||24e3}),navigator.mediaDevices.getUserMedia(Wn()).then(m=>{this.
...
active||!this.capturing)return;const A=S.inputBuffer.getChannelData(0),B=ri(A,k,v);!B||!B.byteLength||
...
this.audioCtx.close()}catch{}this.audioCtx=null}mn(),Mt()}async stop(){if(!this.active)return;this.active=
...
classList.remove("bg-red-600","animate-pulse"),u.classList.add("bg-gray-700"))}};a(Tn,"RealtimeVoice\
Session");let _n=Tn;function ri(c,u,m){let f=c;if(u!==m&&u>0&&m>0){const k=u/m,_=Math.floor(f.length/
k),S=new Float32Array(_);for(let A=0;A<_;A++)S[A]=f[Math.min(Math.floor(A*k),f.length-1)];f=S}const v=new Int16Array(
f.length);for(let k=0;k<f.length;k++){const _=Math.max(-1,Math.min(1,f[k]));v[k]=_<0?_*32768:_*32767}
return v.buffer}a(ri,"pcm16FromFloat32");const qt=new _n;(()=>{const m={idle:"bg-gray-600",connecting:"\
bg-amber-500 animate-pulse",streaming:"bg-emerald-600 animate-pulse",paused:"bg-amber-500",stopped:"\
bg-gray-600",error:"bg-red-600",closed:"bg-gray-600"};let f=null,v=null,k=!1,_=null,S=!1,A=0,B=0,P=null,
z="idle",te=!1,I=null;const E=a($=>document.getElementById($),"$"),W=a($=>{const D=Object.assign({},
$||{});return window.ProgressSpinner&&typeof window.ProgressSpinner.manualRequestOptions=="function"?
window.ProgressSpinner.manualRequestOptions(D):(D.progressSpinner=!1,D)},"noSpinner");function J($,D){
z=D;const pe=E("lyria-status-text"),ee=E("lyria-status-dot");pe&&(pe.textContent=$),ee&&(ee.className=
"w-2 h-2 rounded-full inline-block "+(m[D]||m.idle)),ye(),Ee()}a(J,"setStatus");function be(){const $=B?
Math.floor((Date.now()-B)/1e3):0,D=String(Math.floor($/60)).padStart(2,"0"),pe=String($%60).padStart(
2,"0");return`${D}:${pe}`}a(be,"formatElapsed");function V(){B||(B=Date.now());const $=E("lyria-elap\
sed");$&&($.textContent=be()),P||(P=window.setInterval(()=>{const D=E("lyria-elapsed");D&&(D.textContent=
be())},1e3))}a(V,"startElapsedTimer");function ne(){P&&(window.clearInterval(P),P=null)}a(ne,"stopEl\
apsedTimer");function ye(){const $=E("lyria-play-btn"),D=E("lyria-pause-btn"),pe=E("lyria-stop-btn"),
ee=E("lyria-reset-btn"),fe=!!f,Ae=z==="streaming"||z==="connecting";if($){$.disabled=te||!fe;const We=$.
querySelector("i");We&&(We.className="fas fa-play")}D&&(D.disabled=te||!Ae),pe&&(pe.disabled=te||!fe||
!Ae),ee&&(ee.disabled=te||!fe||!Ae)}a(ye,"updateTransportButtons");function Ee(){const $=E("lyria-sa\
ve-btn");if(!$)return;const D=!!f&&z!=="idle"&&z!=="connecting"&&z!=="error";$.classList.toggle("hid\
den",!D)}a(Ee,"updateSaveButton");function Ie($,D){const pe=E("lyria-prompt-rows");if(!pe)return;const ee=document.
...
const We=ee.querySelector(".lyria-prompt-remove");We&&We.addEventListener("click",()=>{pe.querySelectorAll(
".lyria-prompt-row-wrap").length<=1||ee.remove()}),ee.classList.add("lyria-prompt-row-wrap"),pe.appendChild(
ee)}a(Ie,"addPromptRow");function Ve(){const $=document.querySelectorAll("#lyria-prompt-rows .lyria-\
prompt-row-wrap"),D=[];return $.forEach(pe=>{const ee=pe.querySelector('input[type="text"]'),fe=pe.querySelector(
'input[type="range"]'),Ae=(ee?ee.value:"").trim();Ae&&D.push({text:Ae,weight:parseFloat(fe?fe.value:
1)||1})}),D}a(Ve,"collectPrompts");function Ye(){const $={},D=a(di=>{const Mn=E(di);return Mn&&Mn.value!==
""?parseFloat(Mn.value):void 0},"num"),pe=D("lyria-bpm");pe!==void 0&&($.bpm=Math.round(pe));const ee=D(
"lyria-guidance");ee!==void 0&&($.guidance=ee);const fe=D("lyria-density");fe!==void 0&&($.density=fe);
const Ae=D("lyria-brightness");Ae!==void 0&&($.brightness=Ae);const We=D("lyria-temperature");We!==void 0&&
($.temperature=We);const Re=E("lyria-scale");Re&&Re.value&&($.scale=Re.value);const Je=E("lyria-mode");
Je&&Je.value&&($.music_generation_mode=Je.value);const nt=E("lyria-mute-bass"),$t=E("lyria-mute-drum\
s"),Qn=E("lyria-only-bass-drums");return nt&&($.mute_bass=nt.checked),$t&&($.mute_drums=$t.checked),
Qn&&($.only_bass_and_drums=Qn.checked),$}a(Ye,"collectConfig");function At(){[["lyria-bpm","lyria-bp\
m-label"],["lyria-guidance","lyria-guidance-label"],["lyria-density","lyria-density-label"],["lyria-\
brightness","lyria-brightness-label"],["lyria-temperature","lyria-temperature-label"]].forEach(([D,pe])=>{
const ee=E(D),fe=E(pe);!ee||!fe||ee.addEventListener("input",()=>{const Ae=parseFloat(ee.value);fe.textContent=
D==="lyria-bpm"?String(Math.round(Ae)):Ae.toFixed(1)})})}a(At,"bindRangeLabels");function bt(){if(_){
try{_.close()}catch{}_=null}S=!1,A=0}a(bt,"resetPlayback");function yt(){if(k=!1,v&&typeof v.abort==
"function")try{v.abort()}catch{}v=null}a(yt,"closeStream");async function ct(){yt(),v=new AbortController,
k=!0;try{const $=await fetch(`/api/gemini/music/stream?session_id=${encodeURIComponent(f)}`,W({method:"\
GET",signal:v.signal,headers:{Accept:"text/event-stream"},cache:"no-store"}));if(!$.ok){const fe=await $.
json().catch(()=>({}));throw new Error(fe.error||"\u30B9\u30C8\u30EA\u30FC\u30E0\u63A5\u7D9A\u306B\u5931\u6557\u3057\u307E\u3057\u305F")}
const D=$.body.getReader(),pe=new TextDecoder;let ee="";for(;k;){const{done:fe,value:Ae}=await D.read();
if(fe)break;ee+=pe.decode(Ae,{stream:!0});const We=ee.split(`

`);ee=We.pop();for(const Re of We){const Je=Re.split(`
...
"connecting"),window.setTimeout(()=>{k&&f&&ct()},1200))}finally{k=!1}}a(ct,"openStream");function me($){
if($&&$.snapshot){const D=$.status;if(D==="error"){J("\u30A8\u30E9\u30FC","error"),ne();return}if(D===
"closed"||D==="stopped"){J("\u7D42\u4E86","closed"),ne();return}J(D==="paused"?"\u4E00\u6642\u505C\u6B62\u4E2D":
"\u63A5\u7D9A\u4E2D...",D==="paused"?"paused":"connecting");return}if($&&$.audio){J("\u518D\u751F\u4E2D...",
"streaming"),V(),oe($.audio);return}if($&&$.error){J("\u30A8\u30E9\u30FC: "+$.error,"error"),ne();return}
if($&&$.final){J("\u7D42\u4E86","closed"),ne(),ye();return}}a(me,"handleStreamMessage");function oe($){
if(!$)return;if(!_){const Re=window.AudioContext||window.webkitAudioContext;if(!Re)return;_=new Re({
sampleRate:48e3}),S=!1,A=0}let D;try{const Re=atob($);D=new Uint8Array(Re.length);for(let Je=0;Je<Re.
length;Je++)D[Je]=Re.charCodeAt(Je)}catch{return}const pe=new Int16Array(D.buffer),ee=Math.floor(pe.
...
Ae.buffer=fe,Ae.connect(_.destination),S||(A=_.currentTime+.08,S=!0);const We=Math.max(_.currentTime,
A);Ae.start(We),A=We+fe.duration}a(oe,"playChunk");async function de($,D){const pe=await fetch("/api\
/gemini/music/command",W({method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(
Object.assign({session_id:f,type:$},D||{}))})),ee=await pe.json().catch(()=>({}));if(!pe.ok)throw new Error(
ee.error||"\u30B3\u30DE\u30F3\u30C9\u9001\u4FE1\u306B\u5931\u6557\u3057\u307E\u3057\u305F");return ee}
a(de,"apiCommand");async function $e(){if(te)return;const $=Ve();if(!$.length){showToast("\u30D7\u30ED\u30F3\u30D7\u30C8\u3092\u5165\u529B\u3057\u3066\
\u304F\u3060\u3055\u3044","warning",!0);return}te=!0,ye(),J("\u63A5\u7D9A\u4E2D...","connecting");try{
const D=await fetch("/api/gemini/music/start",W({method:"POST",headers:{"Content-Type":"application/\
json"},body:JSON.stringify({weighted_prompts:$,config:Ye()})})),pe=await D.json().catch(()=>({}));if(!D.
ok)throw new Error(pe.error||"\u30BB\u30C3\u30B7\u30E7\u30F3\u958B\u59CB\u306B\u5931\u6557\u3057\u307E\u3057\u305F");
f=pe.session_id,I=Ye(),J("\u63A5\u7D9A\u4E2D...","connecting"),ct()}catch(D){J("\u30A8\u30E9\u30FC: "+
D.message,"error"),showToast("Lyria RealTime: "+D.message,"error",!0)}finally{te=!1,ye()}}a($e,"star\
...
"connecting")}catch(D){showToast("Lyria RealTime: "+D.message,"error",!0),J("\u30A8\u30E9\u30FC: "+D.
message,"error")}finally{te=!1,ye()}}}a(ke,"control");async function vt(){if(!f)return;const $=Ve();
...
"success")}catch(D){showToast("Lyria RealTime: "+D.message,"error",!0)}finally{te=!1,ye()}}a(vt,"app\
lyPrompts");async function wt(){if(!f)return;const $=Ye(),D=I||{},pe=$.bpm!==void 0&&$.bpm!==D.bpm,ee=$.
scale!==void 0&&$.scale!==D.scale,fe=pe||ee;te=!0;try{await de("config",{config:$,reset_context:fe}),
...
ime: "+Ae.message,"error",!0)}finally{te=!1,ye()}}a(wt,"applyConfig");async function Et(){if(f){te=!0,
J("\u4FDD\u5B58\u4E2D...","connecting"),ye();try{const $=await fetch("/api/gemini/music/save",W({method:"\
POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:f,thread_id:currentThreadId||
null})})),D=await $.json().catch(()=>({}));if(!$.ok)throw new Error(D.error||"\u4FDD\u5B58\u306B\u5931\u6557\u3057\u307E\u3057\u305F");
...
get("welcome-screen").classList.add("hidden")),await loadMessages(D.thread_id||currentThreadId),Yn(!0)}catch($){
J("\u30A8\u30E9\u30FC: "+$.message,"error"),showToast("Lyria RealTime: "+$.message,"error",!0)}finally{
te=!1,ye()}}}a(Et,"saveSession");async function Be(){if(yt(),f)try{await fetch("/api/gemini/music/ca\
ncel",W({method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:f})}))}catch{}
f=null,ne(),bt(),J("\u6E96\u5099\u5B8C\u4E86","idle")}a(Be,"cancelSession");function tt(){const $=E(
"lyria-prompt-rows");$&&($.innerHTML=""),Ie("",1),I=null,B=0,["lyria-bpm","lyria-guidance","lyria-de\
nsity","lyria-brightness","lyria-temperature"].forEach(ee=>{const fe=E(ee);fe&&(fe.value=ee==="lyria\
-bpm"?"120":ee==="lyria-guidance"?"4":ee==="lyria-temperature"?"1.1":"0.5")});const D=E("lyria-scale");
D&&(D.value="");const pe=E("lyria-mode");pe&&(pe.value="QUALITY"),["lyria-mute-bass","lyria-mute-dru\
ms","lyria-only-bass-drums"].forEach(ee=>{const fe=E(ee);fe&&(fe.checked=!1)}),At()}a(tt,"resetContr\
ols");function Yn($){yt(),f&&fetch("/api/gemini/music/cancel",W({method:"POST",headers:{"Content-Typ\
e":"application/json"},body:JSON.stringify({session_id:f})})).catch(()=>{}),f=null,k=!1,ne(),bt(),hideModal(
"lyria-studio-modal")}a(Yn,"closeAndCleanup");function Ln($){if(!isLyriaRealtimeModel()){showToast("\
Lyria RealTime \u30E2\u30C7\u30EB\u3092\u9078\u629E\u3057\u3066\u304B\u3089\u958B\u3044\u3066\u304F\u3060\u3055\u3044",
"warning",!0);return}const D=E("lyria-studio-modal");if(D&&D.classList.contains("modal-open")&&f){if($&&
typeof $=="string"){const ee=E("lyria-prompt-rows");ee&&(ee.innerHTML=""),Ie($,1)}return}if(f&&Be(),
tt(),$&&typeof $=="string"){const ee=E("lyria-prompt-rows");ee&&(ee.innerHTML=""),Ie($,1)}f=null,k=!1,
ne(),bt(),J("\u6E96\u5099\u5B8C\u4E86","idle"),showModal("lyria-studio-modal")}a(Ln,"open");function ci(){
const $=E("lyria-open-studio-btn");$&&$.addEventListener("click",()=>Ln(""));const D=E("lyria-studio\
-close");D&&D.addEventListener("click",()=>Yn(!1));const pe=E("lyria-play-btn");pe&&pe.addEventListener(
"click",()=>{if(!f){$e();return}ke("PLAY")});const ee=E("lyria-pause-btn");ee&&ee.addEventListener("\
click",()=>ke("PAUSE"));const fe=E("lyria-stop-btn");fe&&fe.addEventListener("click",()=>ke("STOP"));
const Ae=E("lyria-reset-btn");Ae&&Ae.addEventListener("click",()=>ke("RESET_CONTEXT"));const We=E("l\
yria-add-prompt-btn");We&&We.addEventListener("click",()=>Ie("",1));const Re=E("lyria-apply-prompts-\
btn");Re&&Re.addEventListener("click",vt);const Je=E("lyria-apply-config-btn");Je&&Je.addEventListener(
"click",wt);const nt=E("lyria-save-btn");nt&&nt.addEventListener("click",Et),At(),tt(),window.openLyriaStudio=
Ln}return a(ci,"init"),{init:ci,open:Ln}})().init(),(()=>{let c=null,u=null;const m=a(I=>document.getElementById(
I),"$");function f(){return isStsModel()&&voiceStudioUiEnabled!==!1}a(f,"isStudioMode");function v(){
const I=get("model-select")?get("model-select").value:"",E=m("voice-studio-title");E&&(I==="gpt-tran\
scribe"||I==="gpt-live-transcribe"?E.textContent="\u97F3\u58F0\u6587\u5B57\u8D77\u3053\u3057\u30B9\u30BF\u30B8\u30AA":
I==="gemini-3.5-live-translate-preview"?E.textContent="\u30EA\u30A2\u30EB\u30BF\u30A4\u30E0\u97F3\u58F0\u7FFB\u8A33\u30B9\u30BF\u30B8\u30AA":
E.textContent="\u97F3\u58F0\u30B9\u30BF\u30B8\u30AA")}a(v,"updateTitle");function k(){const I=m("voi\
ce-studio-transcript");I&&(I.innerHTML='<div class="text-[10px] text-gray-500">\u4F1A\u8A71\u306E\u6587\u5B57\u8D77\u3053\u3057\u304C\u3053\u3053\u306B\u8868\u793A\u3055\u308C\u307E\u3059\u3002</\
div>')}a(k,"resetTranscript");function _(I,E){if(!E||!String(E).trim())return;const W=m("voice-studi\
o-transcript");if(!W||!window.VoiceStudioOpen)return;const J=I==="user"?"\u3042\u306A\u305F":"AI",be=I===
"user"?"text-cyan-300":"text-gray-100",V=W.querySelectorAll(".voice-studio-line");let ne=null;for(let Ee=V.
length-1;Ee>=0;Ee--)if(V[Ee].dataset.role===I){ne=V[Ee];break}const ye=`<span class="${be} font-bold\
">${escapeHtml(J)}:</span> <span class="text-gray-200">${escapeHtml(E)}</span>`;if(ne)ne.innerHTML=ye;else{
const Ee=W.querySelector(".text-gray-500");Ee&&Ee.remove();const Ie=document.createElement("div");Ie.
className="voice-studio-line",Ie.dataset.role=I,Ie.innerHTML=ye,W.appendChild(Ie)}W.scrollTop=W.scrollHeight}
a(_,"log");function S(){const I=m("sts-panel"),E=m("voice-studio-panel-host");I&&E&&I.parentNode!==E&&
(c=I.parentNode,E.appendChild(I));const W=m("file-preview"),J=m("voice-studio-file-host");W&&J&&W.parentNode!==
J&&(u=W.parentNode,J.appendChild(W),J.classList.remove("hidden"))}a(S,"movePanelIntoModal");function A(){
const I=m("sts-panel");I&&c&&I.parentNode!==c&&c.appendChild(I);const E=m("file-preview");E&&u&&E.parentNode!==
u&&u.appendChild(E);const W=m("voice-studio-file-host");W&&W.classList.add("hidden"),c=null,u=null}a(
A,"movePanelBack");function B(){if(!f()){showToast("\u97F3\u58F0\u7CFB\u30E2\u30C7\u30EB\u3092\u9078\u629E\u3057\u3066\u304B\u3089\u958B\u3044\u3066\u304F\u3060\u3055\u3044",
"warning",!0);return}S();const I=m("sts-panel");I&&I.classList.remove("hidden"),v(),k(),window.VoiceStudioOpen=
!0,showModal("voice-studio-modal")}a(B,"open");function P(){if(window.VoiceStudioOpen&&(Me||ze&&ze.state===
"recording"||qt.isActive())&&zn(),window.VoiceStudioOpen=!1,A(),hideModal("voice-studio-modal"),isStsModel()&&
voiceStudioUiEnabled!==!1){const I=m("sts-panel");I&&I.classList.add("hidden")}}a(P,"close");function z(){
window.VoiceStudioOpen&&P()}a(z,"closeIfOpen");function te(){window.VoiceStudioOpen=!1;const I=m("vo\
ice-studio-open-btn");I&&I.addEventListener("click",()=>B());const E=m("voice-studio-close");E&&E.addEventListener(
"click",()=>P()),window.VoiceStudio={open:B,close:P,closeIfOpen:z,log:_,isStudioMode:f}}return a(te,
"init"),{init:te,open:B,close:P,closeIfOpen:z,log:_,isStudioMode:f}})().init();let Gt=null;function Un(){
if(Gt&&(Gt.stop(),Gt=null),Zt){try{Zt.pause()}catch{}try{Zt.src=""}catch{}Zt=null}}a(Un,"stopStsPlay\
back");async function fi(c){Un();const u=new Audio;return u.src=c,u.preload="auto",u.autoplay=!0,u.playsInline=
!0,Zt=u,await u.play(),new Promise(m=>{u.onended=()=>m("ended"),u.onerror=()=>m("error")})}a(fi,"pla\
...
"recording"&&(wn=!0,ze.stop())}a(zn,"cancelRecording");function Wn(){if(isStsModel())return{audio:!0};
...
noiseSuppression&&(m.noiseSuppression=!1),u.autoGainControl&&(m.autoGainControl=!1),{audio:m}}a(Wn,"\
...
-vocab").value.split(/[,、\n]/).map(E=>E.trim()).filter(Boolean);I.length&&(f.custom_vocabulary=I.slice(
...
get("sts-target-lang").value);const v=await apiFetch("/api/gemini/session",{method:"POST",headers:{"\
Content-Type":"application/json"},body:JSON.stringify(f)});if(!v.ok)throw new Error("Failed to get s\
ession token");const{token:k,url:_}=await v.json(),S=get("model-select").value,A=get("sts-voice")?get(
...
-transcribe-mode")?get("sts-transcribe-mode").value:"VERBATIM",E={languageCodes:[]};if((I==="SMART"||
I==="VERBATIM")&&(E.mode=I),get("sts-custom-vocab")){const W=get("sts-custom-vocab").value.split(/[,、\n]/).
map(J=>J.trim()).filter(Boolean);W.length&&(E.customVocabulary=W.slice(0,1e3))}await Me.start(k,_,S,
{transcriptionConfig:E})}else if(isGeminiLiveTranslateModel()){const I=get("sts-target-lang")?get("s\
ts-target-lang").value:"ja";await Me.start(k,_,S,{translationConfig:{targetLanguageCode:I,echoTargetLanguage:!0}})}else
await Me.start(k,_,S,{speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:A}}},thinkingConfig:{
...
const E=Me.interimInputTranscript,W=Me.inputTranscript,J=W+(E&&!W.endsWith(E)?(W?`
`:"")+E:""),be=get("chat-messages");let V=document.getElementById(te);V||(V=document.createElement("\
div"),V.id=te,V.className="flex flex-col gap-2 mb-4 assistant-message bg-slate-800/40 p-3 rounded-lg\
...
eading-relaxed"></div>
                                            `,be.appendChild(V),be.scrollTop=be.scrollHeight);const ne=V.
...
z&&(setStsStatus("Gemini is speaking...",!1),z=!1);const E=get("chat-messages");let W=document.getElementById(
te);W||(W=document.createElement("div"),W.id=te,W.className="flex flex-col gap-2 mb-4 assistant-mess\
...
te-400 text-xs border-l-2 border-slate-600 pl-2 my-1"></div>
                                                <div class="message-content text-sm text-slate-100 l\
eading-relaxed"></div>
                                            `,E.appendChild(W),E.scrollTop=E.scrollHeight);const J=W.
querySelector(".thought-container"),be=W.querySelector(".message-content");Me.assistantThought&&(J.classList.
remove("hidden"),J.innerText=Me.assistantThought),be.innerText=Me.assistantText,E.scrollTop=E.scrollHeight,
...
getUserMedia(Wn());ze=new MediaRecorder(c),rn=[],wn=!1;const u=isStsModel();ze.ondataavailable=m=>rn.
push(m.data),ze.onstop=async()=>{if(wn){rn=[],get("file-preview").classList.add("hidden"),c.getTracks().
forEach(_=>_.stop()),mn(),Mt(),u||(Lt("\u9332\u97F3\u3092\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F",
"idle"),pn=setTimeout(()=>Lt("","hidden"),900)),isStsModel()&&setStsStatus("Canceled",!1),setTimeout(
()=>{isStsModel()&&setStsStatus("Tap to speak",!1)},800);return}const m=new Blob(rn,{type:"audio/web\
...
"/sts",{method:"POST",body:v});if(!_.ok){const W=await _.json().catch(()=>({}));throw new Error(W.error||
"Speech-to-speech failed")}const S=_.body.getReader(),A=new TextDecoder;let B="",P=null,z=null;stsOpt(
"sts-auto-play")&&(z=new tn,Gt=z),setStsStatus(isTranscriptionModel()?"Transcribing...":"Processing \
audio...",!0);let te=!0,I="",E="";for(;;){const{done:W,value:J}=await S.read();if(W)break;B+=A.decode(
J,{stream:!0});const be=B.split(`
`);B=be.pop();for(const V of be){if(!V.trim())continue;const ne=JSON.parse(V);if(ne.error)throw new Error(
...
ne.transcript_delta&&(E+=ne.transcript_delta,window.VoiceStudio&&window.VoiceStudio.log("assistant",
E)),ne.final&&(P=ne)}}window.VoiceStudio&&!I.trim()&&window.VoiceStudio.log("user","\uFF08\u97F3\u58F0\u30E1\u30C3\u30BB\u30FC\u30B8\uFF09"),
...
CHAT_CONFIG.urls.transcribe,{method:"POST",body:v})).json();if(B.transcript){const P=get("prompt-inp\
...
stop()),mn(),Mt(),k||Lt("","hidden"),k&&setStsStatus("Tap to speak",!1)}},ze.start(),get("mic-btn").
...
able.")}};const nn=a((c,u)=>{if(!c)return;const m=c.querySelector("span");m?m.textContent=u:c.textContent=
...
me-btn"),k=get("lib-usage-btn");if(u&&(u.disabled=c===0,nn(u,c?`\u524A\u9664 (${c})`:"\u524A\u9664")),
m&&(m.disabled=c===0,nn(m,c?`\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9 (${c})`:"\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9")),
f&&(f.disabled=c===0,nn(f,c?`\u6DFB\u4ED8 (${c})`:"\u6DFB\u4ED8")),v&&(v.disabled=c!==1,nn(v,"\u540D\u524D\u5909\u66F4")),
k&&(k.disabled=c!==1,nn(k,"\u4F7F\u7528\u30C1\u30E3\u30C3\u30C8")),lib.modal){const _=window.matchMedia(
"(max-width: 768px)").matches;lib.modal.classList.toggle("lib-selecting",_&&c>0)}};function Vn(c){lib.
attachMode=!!c}a(Vn,"setLibAttachMode");const Jn=a((c=!1)=>{Vn(c),showModal("lib-modal"),loadLibraryFiles(),
...
back()},get("lib-btn").onclick=()=>Jn(!1),get("lib-del-btn").onclick=deleteSelectedFiles,get("lib-do\
...
onclick=()=>showSelectedFileUsage()),get("upload-lib-btn")&&(get("upload-lib-btn").onclick=()=>Jn(!0)),
...
onclick=()=>addGemFixedPromptRow());const li=a(()=>{editingGemUuid=null,get("gem-modal-title").innerHTML=
...
back()},get("add-gem-btn").onclick=()=>li(),get("save-gem-btn").onclick=async()=>{const c=get("gem-n\
...
try{const v=decodeURIComponent(m),k=new Blob([v],{type:"text/plain"}),_=URL.createObjectURL(k),S=document.
...
t"}[f]||f;(f.length>8||/[^a-z0-9]/.test(f))&&(B="txt");let P=`code.${B}`;f==="dockerfile"&&(P="Docke\
rfile"),f==="makefile"&&(P="Makefile"),S.download=P,document.body.appendChild(S),S.click(),document.
body.removeChild(S),URL.revokeObjectURL(_)}catch(v){console.error("Download failed",v)}}if(c.target.
...
upload-list .upload-row");s.innerHTML="",T.forEach((M,F)=>{const se=M.getAttribute("data-local-url"),
K=M.getAttribute("data-filename"),he=M.querySelector("img.upload-preview")!==null;let N;if(he){let U=se;
...
.thumb-item")},N.onerror=function(){this.parentElement.replaceChild(d("ERR"),this)})}N||(N=d("FILE")),
N.style.animationDelay=`${F*32}ms`,s.appendChild(N)}),T.length>0?s.classList.remove("hidden"):s.classList.
add("hidden")}function d(T){const M=document.createElement("div");return M.className="thumb-item bg-\
gray-800 flex items-center justify-center text-gray-500 text-[9px] shadow-sm font-bold",M.innerText=
T,M}a(d,"createFileThumb");const p=collectImageUrlsForSend(),g=uploadProgressState.total,h=uploadProgressState.
completed,y=uploadProgressState.active;g===0&&(e.classList.add("hidden"),n&&n.classList.add("hidden"),
r&&r.classList.add("hidden"),s&&s.classList.add("hidden"));const b=get("send-btn"),w=get("mic-btn"),
x=get("mask-btn"),C=isStopMode;if(y>0?(b&&(b.disabled=!0),w&&(w.disabled=!0),x&&(x.disabled=!0)):C||
(b&&(b.disabled=!1),w&&(w.disabled=!1),x&&(x.disabled=!1)),y>0){const T=`Preparing... (${h}/${g})`;e.
classList.remove("hidden"),t.innerText=T,o&&(o.innerText=`(${h}/${g})`);let M=h*100,F=0;for(let he in uploadProgressState.
perFilePct)M+=uploadProgressState.perFilePct[he],F++;const se=g>0?M/(g*100)*100:0,K=`${Math.min(100,
se)}%`;n&&i&&(n.classList.remove("hidden"),i.style.width=K),r&&l&&(r.classList.remove("hidden"),l.style.
width=K)}else o&&(o.innerText=""),r&&r.classList.add("hidden"),p.length>0?(e.classList.remove("hidde\
...
g=Math.min(n*.45,Math.max(24,n*.12)),h=Math.min(i*.45,Math.max(24,i*.12)),y=g-r-d,b=n-g-r,w=h-l-p,x=i-
h-l,C=a((T,M,F)=>Number.isFinite(T)?M>F?(M+F)/2:Math.min(F,Math.max(M,T)):0,"clampOffset");markerView.
offsetX=C(markerView.offsetX,y,b),markerView.offsetY=C(markerView.offsetY,w,x)}a(clampMarkerViewOffset,
...
height);const n=a((r,l,d=null,p=!1)=>{if(!r)return;const g=Math.max(0,r.x),h=Math.max(0,r.y),y=Math.
max(1,r.w),b=Math.max(1,r.h);d&&(t.fillStyle=d,t.fillRect(g,h,y,b)),t.save(),p&&t.setLineDash([6,4]),
t.strokeStyle=l,t.lineWidth=2,t.strokeRect(g+.5,h+.5,Math.max(1,y-1),Math.max(1,b-1)),t.restore()},"\
...
o),p=normalizeAttachmentDisplayName(r)||getAttachmentNameForPath(l),g=t.get(l);if(g===void 0){const b=e.
length;t.set(l,b),e.push({path:l,source:d,name:p});return}const h=e[g];if(!h)return;const y=normalizeAttachmentSource(
h.source);(y==="unknown"&&d!=="unknown"||y==="library"&&d==="upload")&&(h.source=d),!normalizeAttachmentDisplayName(
...
n,h.dataset.displayName=o,h.dataset.defaultDisplayName=o,h.dataset.sendNameCustomized="";const y=escapeHtml(
...
h)||o));const C=h.querySelector(".upload-send-name");C&&(C.onclick=()=>promptRowAttachmentName(h));const T=h.
...
const M=h.querySelector(".upload-marker");return M&&(M.onclick=()=>openMarkerModalForRow(h)),setAttachmentSourceForPath(
...
url"),C=normalizeAttachmentDisplayName(i.dataset.displayName)||e.name||w||"";openFileViewer(x,C)});const h=i.
...
getAttribute("data-filename");x&&(currentImageUrls=currentImageUrls.filter(C=>C!==x)),setRowMarkerState(
...
"\u5931\u6557"),showToast(h,"error",!0),!1}const o=s.upload_id,r=s.chunk_size||10*1024*1024,l=Math.ceil(
e.size/r);for(let h=0;h<l;h++){const y=h*r,b=Math.min(e.size,y+r),w=e.slice(y,b);if(!await new Promise(
C=>{const T=new XMLHttpRequest;T.open("POST","/upload/chunk",!0),T.setRequestHeader("X-CSRF-Token",csrfToken),
T.upload.onprogress=F=>{if(F.lengthComputable&&t&&t.bar){const se=y+F.loaded,K=Math.min(100,Math.floor(
se/e.size*100));t.bar.style.width=`${K}%`,t.status&&(t.status.textContent=`${K}%`),t.uploadId&&updateGlobalUploadProgress(
t.uploadId,K)}window.ConnectionMonitor&&window.ConnectionMonitor.reportActivity()},T.onload=()=>{T.status>=
200&&T.status<300?C(!0):C(!1)},T.onerror=()=>C(!1);const M=new FormData;M.append("upload_id",o),M.append(
"index",String(h)),M.append("total",String(l)),M.append("chunk",w,e.name),T.send(M)}))return t&&t.status&&
...
"img.upload-preview");if(h){const y=l.filename.replace(/^\d+\//,"");h.src=buildAttachmentPreviewUrl(
...
t.row),h=defaultAttachmentDisplayName(d),y=g&&normalizeAttachmentDisplayName(t.row.dataset.displayName)||
h;t.row.dataset.defaultDisplayName=h,setRowAttachmentName(t.row,y)}d&&setAttachmentSourceForPath(d,"\
...
o),l=a((p,g)=>{for(let h=0;h<g.length;h++)r.setUint8(p+h,g.charCodeAt(h))},"writeString");l(0,"RIFF"),
...
r=markerView.scale,l={x:0,y:0},d={x:0,y:0},p=[],g=16,h="",y=null,b=null,w=null,x=null,C=!1,T=null;const M=a(
L=>{const R=e.getBoundingClientRect(),q=(L.clientX-R.left)*(e.width/R.width),Y=(L.clientY-R.top)*(e.
height/R.height);return{x:q,y:Y}},"getPoint"),F=a((L,R)=>({x:(L.x+R.x)/2,y:(L.y+R.y)/2}),"getMid"),se=a(
(L,R)=>Math.hypot(L.x-R.x,L.y-R.y),"getDist");let K=!1;const he=a(()=>{y||(y=document.createElement(
...
height)&&(w.width=e.width,w.height=e.height)},"ensureDrawBuffers"),N=a(()=>{if(!t||!y||!w)return;const L=Math.
...
ew"),U=a(()=>{x&&(x.strokeStyle=h,x.fillStyle=h,x.lineWidth=g,x.lineCap="round",x.lineJoin="round")},
"applyMarkerBrush"),Q=a(L=>{if(!L)return!1;if(p.length===0)return p.push(L),!0;const R=p[p.length-1],
q=L.x-R.x,Y=L.y-R.y,G=Math.hypot(q,Y),Z=Math.max(.35,g*.04);if(G<Z)return!1;const we=Math.max(1,g*.25),
H=Math.max(1,Math.ceil(G/we));for(let Te=1;Te<=H;Te++){const He=Te/H;p.push({x:R.x+q*He,y:R.y+Y*He})}
return!0},"appendStrokePoint"),ue=a(()=>{if(x&&(x.clearRect(0,0,w.width,w.height),p.length!==0)){if(U(),
p.length===1){const L=p[0];x.beginPath(),x.arc(L.x,L.y,g/2,0,Math.PI*2),x.fill();return}if(x.beginPath(),
...
L.x,L.y,R.x,R.y)}x.stroke()}},"renderStrokeLayer"),ae=a((L,R)=>{if(!L||!R)return null;const q=Math.min(
L.x,R.x),Y=Math.min(L.y,R.y),G=Math.abs(L.x-R.x),Z=Math.abs(L.y-R.y);return{x:q,y:Y,w:G,h:Z}},"norma\
lizeMosaicRect"),ce=a(L=>{const R=n?Number(n.value||16):16,q=Math.max(6,Math.floor(R)),Y=Math.floor(
q/2);return{x:L.x-Y,y:L.y-Y,w:q,h:q}},"buildMosaicRectFromPoint"),_e=a(()=>{const L=document.createElement(
...
e.width,e.height)}catch{return null}},"getMosaicSourceImageData"),X=a(L=>{if(!t||!L)return!1;const R=_e();
if(!R)return!1;const q=n?Number(n.value||16):16,Y=Math.max(4,Math.floor(q/2)),G=Math.max(0,Math.floor(
L.x)),Z=Math.max(0,Math.floor(L.y)),we=Math.min(e.width,Math.ceil(L.x+L.w)),H=Math.min(e.height,Math.
ceil(L.y+L.h));if(we<=G||H<=Z)return!1;for(let Te=Z;Te<H;Te+=Y)for(let He=G;He<we;He+=Y){const Se=Math.
min(Y,we-He),it=Math.min(Y,H-Te),Qe=Math.min(e.width-1,Math.max(0,He+Math.floor(Se/2))),at=(Math.min(
e.height-1,Math.max(0,Te+Math.floor(it/2)))*e.width+Qe)*4,xt=R.data[at],gt=R.data[at+1],dt=R.data[at+
2];t.fillStyle=`rgb(${xt},${gt},${dt})`,t.fillRect(He,Te,Se,it)}return!0},"applyMosaicRect"),ve=a(L=>{
if(!t)return;if(i.set(L.pointerId,{x:L.clientX,y:L.clientY}),i.size>=2){const q=Array.from(i.values()),
Y=q[0],G=q[1];s=!0,K=!1,p=[],C=!1,T=null,markerState.mosaicPreviewRect=null,o=se(Y,G)||1,r=markerView.
scale,l={x:markerView.offsetX,y:markerView.offsetY},d=F(Y,G),renderCropOverlay(),e.setPointerCapture&&
e.setPointerCapture(L.pointerId),L.preventDefault();return}if(s||markerState.mode==="crop")return;K=
!0;const R=M(L);if(markerState.mode==="mosaic")C=!0,T=R,markerState.mosaicPreviewRect=ce(R),renderCropOverlay();else{
if(he(),!b||!x)return;b.clearRect(0,0,y.width,y.height),b.drawImage(e,0,0),x.clearRect(0,0,w.width,w.
height),g=n?Number(n.value||16):16,h=normalizeMarkerHexColor(markerState.colorHex),p=[],Q(R),ue(),markerState.
hasStroke=!0,N()}e.setPointerCapture&&e.setPointerCapture(L.pointerId),L.preventDefault()},"start"),
re=a(L=>{if(i.has(L.pointerId)&&i.set(L.pointerId,{x:L.clientX,y:L.clientY}),s&&i.size>=2){const q=Array.
from(i.values()),Y=q[0],G=q[1],Z=F(Y,G),we=se(Y,G)||1,H=r*(we/o);markerView.scale=Math.min(markerView.
maxScale,Math.max(markerView.minScale,H)),markerView.offsetX=l.x+(Z.x-d.x),markerView.offsetY=l.y+(Z.
y-d.y),applyMarkerTransform(),L.preventDefault();return}if(!K||!t)return;const R=M(L);if(markerState.
mode==="mosaic"){if(!C||!T)return;markerState.mosaicPreviewRect=ae(T,R)||ce(R),renderCropOverlay()}else
Q(R)&&(ue(),N());L.preventDefault()},"move"),j=a(L=>{const R=K;if(i.delete(L.pointerId),i.size<2&&(s=
!1),i.size===0){if(K=!1,R&&t&&markerState.mode==="draw"&&p.length>0&&(ue(),N()),R&&markerState.mode===
"mosaic"&&T){const q=M(L);let Y=ae(T,q);(!Y||Y.w<2||Y.h<2)&&(Y=ce(T)),X(Y)&&(markerState.hasStroke=!0,
markerState.mosaicRects.push(Y))}p=[],C=!1,T=null,markerState.mosaicPreviewRect=null,renderCropOverlay(),
...
"pointerup",j),e.addEventListener("pointercancel",j)}a(initMarkerCanvas,"initMarkerCanvas");function initCropCanvas(){
const e=get("marker-crop-canvas");if(!e)return;const t=e.getContext("2d"),n=new Map;let i=!1,s=null,
o=null,r=null,l=!1,d=0,p=markerView.scale,g={x:0,y:0},h={x:0,y:0};const y=8,b=14,w=a((N,U,Q)=>Math.min(
Q,Math.max(U,N)),"clamp"),x=a(N=>{const U=e.getBoundingClientRect(),Q=(N.clientX-U.left)*(e.width/U.
width),ue=(N.clientY-U.top)*(e.height/U.height);return{x:Q,y:ue}},"getPoint"),C=a((N,U)=>({x:(N.x+U.
x)/2,y:(N.y+U.y)/2}),"getMid"),T=a((N,U)=>Math.hypot(N.x-U.x,N.y-U.y),"getDist"),M=a(()=>(markerState.
cropRect||resetCropRectToFull(),markerState.cropRect),"ensureCropRect"),F=a((N,U)=>{if(!U)return"mov\
...
op"?"n":"s":"move"},"hitTest"),se=a(N=>{if(markerState.mode!=="crop")return;if(n.set(N.pointerId,{x:N.
clientX,y:N.clientY}),n.size>=2){const ue=Array.from(n.values()),ae=ue[0],ce=ue[1];l=!0,i=!1,d=T(ae,
ce)||1,p=markerView.scale,g={x:markerView.offsetX,y:markerView.offsetY},h=C(ae,ce),e.setPointerCapture&&
e.setPointerCapture(N.pointerId),N.preventDefault();return}if(l)return;i=!0;const U=x(N),Q=M();o=F(U,
Q),s=U,r=Q?{x:Q.x,y:Q.y,w:Q.w,h:Q.h}:null,renderCropOverlay(),e.setPointerCapture&&e.setPointerCapture(
N.pointerId),N.preventDefault()},"start"),K=a(N=>{if(markerState.mode!=="crop")return;if(n.has(N.pointerId)&&
n.set(N.pointerId,{x:N.clientX,y:N.clientY}),l&&n.size>=2){const L=Array.from(n.values()),R=L[0],q=L[1],
Y=C(R,q),G=T(R,q)||1,Z=p*(G/d);markerView.scale=Math.min(markerView.maxScale,Math.max(markerView.minScale,
Z)),markerView.offsetX=g.x+(Y.x-h.x),markerView.offsetY=g.y+(Y.y-h.y),applyMarkerTransform(),renderCropOverlay(),
N.preventDefault();return}if(!i||!s||!r)return;const U=x(N),Q=e.width,ue=e.height,ae={x:r.x,y:r.y,w:r.
w,h:r.h},ce=r.x+r.w,_e=r.y+r.h,X=a(()=>{const L=w(U.x,0,ce-y);ae.x=L,ae.w=ce-L},"applyW"),ve=a(()=>{
ae.w=w(U.x-r.x,y,Q-r.x)},"applyE"),re=a(()=>{const L=w(U.y,0,_e-y);ae.y=L,ae.h=_e-L},"applyN"),j=a(()=>{
ae.h=w(U.y-r.y,y,ue-r.y)},"applyS");switch(o){case"move":{const L=U.x-s.x,R=U.y-s.y;ae.x=w(r.x+L,0,Q-
...
"move"),he=a(N=>{n.delete(N.pointerId),n.size<2&&(l=!1),n.size===0&&(renderCropOverlay(),i=!1,s=null,
o=null,r=null),e.releasePointerCapture&&e.releasePointerCapture(N.pointerId),N.preventDefault()},"en\
d");e.addEventListener("pointerdown",se),e.addEventListener("pointermove",K),e.addEventListener("poi\
nterup",he),e.addEventListener("pointercancel",he),e.addEventListener("pointerleave",he)}a(initCropCanvas,
...
"2d");if(!l)return;if(l.drawImage(t,0,0,o,r),l.drawImage(n,0,0,o,r),markerState.cropRect){const C=o/
n.width,T=r/n.height,M=Math.max(0,Math.floor(markerState.cropRect.x*C)),F=Math.max(0,Math.floor(markerState.
cropRect.y*T)),se=Math.min(o,Math.max(1,Math.floor(markerState.cropRect.w*C))),K=Math.min(r,Math.max(
1,Math.floor(markerState.cropRect.h*T))),he=document.createElement("canvas");he.width=se,he.height=K;
const N=he.getContext("2d");N&&(N.drawImage(s,M,F,se,K,0,0,se,K),s=he)}const d=await new Promise(C=>s.
toBlob(C,"image/png",.92));if(!d){showToast("\u7DE8\u96C6\u753B\u50CF\u306E\u751F\u6210\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
"error",!0);return}const g=(markerState.filename||"marked.png").replace(/\.[^/.]+$/,""),h=new File([
d],`${g}_marked.png`,{type:"image/png"}),y={row:e,uploadId:e.dataset.uploadId,status:e.querySelector(
".upload-status"),bar:e.querySelector(".upload-progress > div")};y.status&&(y.status.textContent="\u7DE8\u96C6\
\u53CD\u6620\u4E2D..."),updateUploadRowFile(y,h);const b=e.getAttribute("data-filename"),w=getRowAttachmentSource(
e);b&&!e.dataset.originalFilename&&(e.dataset.originalFilename=b,e.dataset.originalSource=w,setAttachmentSourceForPath(
b,w)),await uploadFileWithProgress(h,y)?(b&&(currentImageUrls=currentImageUrls.filter(C=>C!==b)),setRowAttachmentSource(
...
e);let s=null,o=null,r=null,l=null,d=[],p=null;const g=a(()=>{p&&clearTimeout(p);try{URL.revokeObjectURL(
i.src)}catch{}try{i.remove()}catch{}if(s&&s.getTracks().forEach(y=>y.stop()),r)try{r.disconnect()}catch{}
if(l)try{l.disconnect()}catch{}if(o)try{o.close()}catch{}},"cleanup"),h=a(()=>{g(),n(null)},"fail");
i.onloadedmetadata=async()=>{try{s=i.captureStream();const y=s.getAudioTracks();if(!y||!y.length)return h();
...
""),x=encodeWav(d,o.sampleRate),C=new File([x],`${w}.audio.wav`,{type:"audio/wav"});g(),n(C)},b+250)),
await i.play(),i.onended=()=>{const w=(e.name||"video").replace(/\.[^/.]+$/,""),x=encodeWav(d,o.sampleRate),
C=new File([x],`${w}.audio.wav`,{type:"audio/wav"});g(),n(C)}}catch{h()}},i.onerror=()=>h()}))}a(extractAudioFromVideo,
...
get("upload-audio-only").checked),r=getModelMediaSupport(get("model-select").value),l=a(async h=>{let y=null;
...
\u30FC\u30C9\u3067\u306F\u753B\u50CF\u30D5\u30A1\u30A4\u30EB\u3060\u3051\u3092\u6DFB\u4ED8\u3067\u304D\u307E\u3059",
"error",!0),uploadProgressState.total>0&&uploadProgressState.total--,uploadProgressState.active>0&&uploadProgressState.
active--,!1;const b=addUploadRow(h);updateFilePreview(),y=b.uploadId,uploadProgressState.perFilePct[y]=
...
const M=await window.imageCompression(h,T),F=new File([M],imageFilenameForMime(h.name,M.type||(x!=="\
original"?x:h.type)),{type:M.type||h.type,lastModified:h.lastModified||Date.now()});F.size>h.size?(showToast(
...
browserFastLocalFiles.values()).reduce((C,T)=>C+Number(T.file&&T.file.size||0),0);return browserFastLocalFiles.
...
for(;;){const h=d++;if(h>=s.length)break;await l(s[h])}});await Promise.all(g)}a(handleFiles,"handle\
...
e||""),n=t.split(/\r?\n/),i=[],s=[],o=/^(\s*)(`{3,}|~{3,})(.*)$/;let r=null,l="",d=[];for(const h of n){
if(!r){const w=h.match(o);if(w){r=w[2],l=String(w[3]||"").trim(),d=[],i.push({lang:l,code:"",open:!0}),
s.push('<div class="canvas-code-placeholder">Canvas\u3067\u8868\u793A\u4E2D</div>');continue}s.push(
h);continue}const y=String(h||"").trim();if(y&&y.replace(/\s+/g,"")===r){const w=i[i.length-1];w&&(w.
code=d.join(`
...
`),h.open=!0)}const p=i.map((h,y)=>normalizeCanvasBlock(h,y)),g=selectCanvasPreviewBlock(p,t);return{
renderText:s.join(`
...
"\u73FE\u5728\u8868\u793A\u4E2D":"\u5207\u308A\u66FF\u3048"}: ${o}`,y=`${h}\u3001${g}`;return`<butto\
n type="button" class="canvas-block-chip${r?" active":""}" data-canvas-block-index="${s}" title="${escapeHtml(
h)}" aria-label="${escapeHtml(y)}" aria-pressed="${r?"true":"false"}"><span class="canvas-block-chip\
...
"\u30B3\u30FC\u30C9\u30D6\u30ED\u30C3\u30AF\u3092\u5F85\u6A5F\u4E2D",g=o?d?`HTML Canvas Preview${t.length>
...
code:p.code,open:!1},d);s.push(g),o===-1&&p.codeKey===t.codeKey&&(o=s.length-1)}),!s.length)return null;
...
"`","~"])for(let l=3;l<=10;l++){const d=r.repeat(l);for(const p of o){const g=`${d}${p}
`,h=`
${d}`,y=g+s+h;i.includes(y)&&(i=i.split(y).join(""))}}return i}a(stripExactFencedBlock,"stripExactFe\
...
ata-code-key="${p}"><div class="code-header"><span class="code-lang"><i class="fas fa-terminal"></i>\
 ${escapeHtml(g)}</span><div class="code-actions">${y}${h}<button class="copy-btn" data-copy="code" \
...
redSvgBoxes");function renderMessage(e,t,n,i,s,o,r=null,l=!0,d=null,p=null,g=null,h=null,y=null,b=null,w=null,x=null,C=!0,T=null,M=null){
...
if(U==null){const G=g!=null?Number(g):0,Z=h!=null?Number(h):0;(g!=null||h!=null)&&(U=G+Z)}messageMeta[e]=
{tokens_in:g,tokens_out:h,tokens_total:U,tokens_content:b,tokens_thought:w,is_encrypted:y,role:t,model:o,
parent_id:T,quote_text:d,image_url:i,gem_name:M,python_executions:F?[]:he.executions||[]};let Q="";d&&
(Q=`<div class="mb-2 p-2 bg-black/20 rounded border-l-4 border-blue-400 text-xs text-gray-300 italic\
 truncate max-w-full"><i class="fas fa-quote-left mr-1 opacity-50"></i>${escapeHtml(d)}</div>`);let ue="";
...
if(i)try{const G=JSON.parse(i);if(G.length){const Z=[];if(G.forEach(we=>{let H=we,Te="unknown";if(H&&
typeof H=="object"&&(Te=normalizeAttachmentSource(H.source),H=H.filepath||H.path||H.url||H.file||""),
H=normalizeAttachmentPath(H)||H,!H)return;setAttachmentSourceForPath(H,Te);const He=H.replace(/^\d+\//,
""),Se=buildFileUrl(He),it=buildAttachmentPreviewUrl(He),Qe=H.split("/").pop(),ft=Qe.split(".").pop().
...
ck="deleteMessage('${e}')"><i class="fas fa-trash"></i></button></div>`,X=[];!F&&o&&X.push(escapeHtml(
o)),M&&(F?X.push(`<span class="text-purple-300/90"><i class="fas fa-gem mr-0.5"></i>${escapeHtml(M)}\
</span>`):X.push(`<span class="text-purple-300/90"><i class="fas fa-gem mr-0.5"></i>${escapeHtml(M)}\
</span>`));const ve=[];if(g!=null&&ve.push(`In ${g}`),h!=null){let G=`Out ${h}`;w!=null&&Number(w)>0&&
...
penTokenDetail('${e}')">${G}</button>`)}if(y!=null){const G=y?"fa-lock":"fa-lock-open",Z=isAdminUser?
y?"\u6697\u53F7\u5316\u72B6\u614B\uFF08\u30BF\u30C3\u30D7\u3067\u5FA9\u53F7\u5316\uFF09":"\u5E73\u6587\u72B6\u614B\uFF08\u30BF\u30C3\u30D7\u3067\u518D\
\u6697\u53F7\u5316\uFF09":y?"Encrypted":"Plain",we=isAdminUser?y?"text-amber-300/90 hover:text-amber\
...
"chat-container");return Y&&(Y.appendChild(q),C&&scrollToBottom(),F||(queueMessageDecorations(q,N),syncCodingTargetButtons(
...
t.tokens_thought:"-",d=t.is_encrypted===null||t.is_encrypted===void 0?"-":t.is_encrypted?"Encrypted":
...
"token-detail-encrypted").innerText=d;const p=t.model?`${t.model} (${t.role})`:`${t.role}`;get("toke\
...
"click",h,!0)}a(l,"cleanup");function d(y){if(r)return;r=!0,t&&t.checked&&(setGeminiLocalPyDialogEnabled(
!1),syncGeminiLocalPyDialogSetting()),l(),hideModal("gemini-local-python-modal"),o(y)}a(d,"finalize");
function p(){d(!0)}a(p,"onOk");function g(){d(!1)}a(g,"onCancel");function h(y){y.target===e&&(y.preventDefault(),
y.stopImmediatePropagation(),g())}a(h,"onOverlay"),n&&n.addEventListener("click",p),i&&i.addEventListener(
...
slashSelectedIndex?"bg-gray-700":""}`,w.innerHTML=`
                ...
                `;let x=!1;w.addEventListener("pointerdown",C=>{typeof C.button=="number"&&C.button!==
0||(C.preventDefault(),x=!0,selectSlashCommand(y.id))}),w.addEventListener("click",C=>{C.preventDefault(),
...
g=8;if(t.style.position="fixed",t.style.left=`${Math.max(8,o.left)}px`,t.style.zIndex="80",t.style.maxHeight=
"none",l<180&&d>l){const y=Math.min(p,d-g);t.style.top="auto",t.style.bottom=`${r-o.top+4}px`,n.style.
maxHeight=`${y}px`}else{const y=Math.min(p,l-g);t.style.top=`${o.bottom+4}px`,t.style.bottom="auto",
...
forEach(([h,y])=>{const b=AI_SETTING_JUMP_TARGETS[h]||{label:h},w=document.createElement("button");w.
...
tem";const x=document.createElement("span");x.className="min-w-0 flex-1";const C=document.createElement(
"span");C.className="block text-xs font-bold text-blue-200",C.textContent=b.label;const T=document.createElement(
"span");T.className="block mt-0.5 text-[11px] text-gray-300 break-words",T.textContent=formatAiSettingValue(
y);const M=document.createElement("i");M.className="fas fa-arrow-up-right-from-square text-[10px] te\
xt-blue-300 shrink-0",x.appendChild(C),x.appendChild(T),w.appendChild(x),w.appendChild(M),w.addEventListener(
"click",()=>openAiSettingJumpTarget(h)),p.appendChild(w)});const g=d.querySelector(".message-footer-\
meta");g?d.insertBefore(p,g):d.appendChild(p),scrollToBottom()}a(renderAiSettingsResultBubble,"rende\
...
file.type,data:await fileToBase64Payload(N.file)}});s.push({text:e});const o={},r=browserFastThinkingConfig(
...
abortController=new AbortController;let h="",y="";const b=[];let w=null,x=null,C=!1;const T={},M=[];
...
height="auto";const U=N.body.getReader(),Q=new TextDecoder;let ue="";const ae=a(re=>{const j=re.split(
...
${we}
\`\`\`
`,F=`browserFastPy_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,se=we,T[F]||(g.insertAdjacentHTML(
"afterbegin",browserFastPythonBoxHtml(F)),T[F]=g.querySelector(`[data-py-id="${F}"]`)),updateBrowserFastPythonBox(
T[F],"code",we);return}if(G&&G.codeExecutionResult&&typeof G.codeExecutionResult.output=="string"){const we=G.
codeExecutionResult.output;h+=`
**Output:**
\`\`\`
${we}
\`\`\`
`;const H=F||`browserFastPy_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;M.push({code:se||
"",output:we}),T[H]||(g.insertAdjacentHTML("afterbegin",browserFastPythonBoxHtml(H)),T[H]=g.querySelector(
`[data-py-id="${H}"]`)),updateBrowserFastPythonBox(T[H],"output",we);return}const Z=typeof G.text=="\
string"?G.text:"";Z&&(G.thought===!0?y+=Z:h+=Z)})}),!C&&(h||y)){beginPendingToStreamTransition(g);const q=g.
querySelector(".content-area");q&&q.remove(),C=!0}y&&(x||(g.insertAdjacentHTML("afterbegin",'<div cl\
ass="thought-container"><div class="thought-header" onclick="toggleThinking(this)"><i class="fas fa-\
brain text-purple-400"></i> Thinking Process</div><div class="thought-content"></div></div>'),x=g.querySelector(
".thought-content")),x.textContent=y),h&&(w||(w=document.createElement("div"),w.className="content-a\
rea prose prose-invert text-sm break-words",g.appendChild(w)),renderAiMarkdownInto(w,h,{incrementalMath:!0})),
...
split(/\r?\n\r?\n/);ue=L.pop()||"",L.forEach(ae)}if(ue+=Q.decode(),ue.trim()&&ae(ue),!h.trim())throw new Error(
"Gemini\u304B\u3089\u56DE\u7B54\u672C\u6587\u304C\u8FD4\u3055\u308C\u307E\u305B\u3093\u3067\u3057\u305F");
w&&renderAiMarkdownInto(w,h,{incrementalMath:!0}),x&&x.classList.add("collapsed"),M.length&&(h+=M.map(
re=>`
\`\`\`pyexec
${JSON.stringify(re)}
\`\`\`
...
message:e,assistant_content:h,thought_content:y,model:t,image_urls:ce,temporary_chat:temporaryChatEnabled,
...
signal:abortController.signal}),g),X=await _e.json().catch(()=>({}));if(!_e.ok||!X.thread_id)throw new Error(
...
nput").value=e);const U=N.message||"\u30A8\u30E9\u30FC";g&&g.insertAdjacentHTML("beforeend",buildChatErrorBubbleHtml(
U));try{let Q=h||"";M.length&&(Q+=M.map(X=>`
\`\`\`pyexec
${JSON.stringify(X)}
\`\`\`
...
thought_content:y||"",model:t,image_urls:ae,temporary_chat:temporaryChatEnabled,thread_id:currentThreadId||
null,parent_id:n&&n.parent_id?n.parent_id:null,thought_signatures:b,turnstile_token:botTurnstileTokenForRequest()}),
signal:abortController&&!abortController.signal.aborted?abortController.signal:void 0}),g),_e=await ce.
...
-1,tempPrompt="";const i=collectAttachmentItemsForSend(),s=i.map(O=>O.path),o=i.filter(O=>normalizeAttachmentSource(
...
d=s.some(O=>isVideoPath(O)),p=(get("model-select").value||"").toLowerCase(),g=get("enable-python"),h=!!(g&&
g.checked);if(l&&!r.audio||d&&!r.video){showToast("\u3053\u306E\u30E2\u30C7\u30EB\u306F\u97F3\u58F0/\u52D5\u753B\u5165\u529B\u306B\u5BFE\u5FDC\u3057\u3066\u3044\u307E\u305B\u3093",
...
\u3057\u3066\u304F\u3060\u3055\u3044\u3002","error",!0);return}}const y=t.trim();if(/^\/settings(?:\s|$)/i.
test(y)&&isMistralOcrModel()){showToast("Mistral OCR \u306F\u8A2D\u5B9A\u5909\u66F4\u30B3\u30DE\u30F3\u30C9\u306B\u4F7F\u3048\u307E\u305B\u3093\u3002\u30C1\u30E3\u30C3\u30C8\u30E2\u30C7\u30EB\u3092\u9078\u3093\u3067\u304F\u3060\u3055\u3044\u3002",
"error",!0);return}if(/^\/settings(?:\s|$)/i.test(y)){const O=y.replace(/^\/settings\s*/i,"").trim();
...
"error",!0);return}get("prompt-input").value="",get("prompt-input").style.height="auto",await runAiSettingsCommand(
O,ie);return}if(isGeminiLocalPythonMode(p,l,d,h)&&!await confirmGeminiLocalPythonSwitch())return;let b=null,
...
"error",!0);return}let je=3e5-Le;const De=[];for(let Fe=xe.length-1;Fe>=0;Fe--){const Pe=String(xe[Fe].
code||"").length;Pe>je||(De.unshift(xe[Fe]),je-=Pe)}w=codingTargetSelection?De.slice(-1):[...ie,...De];
...
t send start: model=${get("model-select").value} thread=${currentThreadId||"-"} text_len=${t.length}\
 attachments=${s.length} search=${get("enable-search").checked}`);const C=t,T=hasMarkerHint()?MARKER_HINT_TEXT:
null;if(isGptImageModel()&&currentMaskImage&&s.length===0){showToast("Mask \u306F\u753B\u50CF\u5165\u529B\u304C\u5FC5\u8981\u3067\u3059",
"error",!0);return}const M=editingMessageId,F=currentParentId,se=M!=null;M&&(editingMessageId=null,setEditUi(
!1)),playSendAnimation(),get("welcome-screen").classList.add("hidden");const K=[],he=a(O=>{if(O==null)
...
"hideRenderedBranchFrom"),N=a(()=>{K.forEach(({node:O,prevDisplay:ie})=>{O&&(O.style.display=ie||"")}),
K.length=0},"restoreHiddenBranch");M&&he(M);const U=Date.now(),Q=renderMessage(U,"user",C,JSON.stringify(
s),null,null,null,!0,currentQuote,null,null,null,null,null,null,null,!0,F,activeGem?activeGem.name:null);
let ue=!1;const ae=/(https?:\/\/)?(x\.com|twitter\.com)\//i,ce=ae.test(C||"")||ae.test(currentQuote||
""),_e="grok-4-fast-reasoning",X=a(()=>{get("enable-search").checked=!0,get("model-select").value!==
...
je=>{O.classList.remove("hidden");const De=a(et=>{O.classList.add("hidden"),ie.onclick=null,xe.onclick=
null,je(et)},"cleanup");ie.onclick=()=>De("enable"),xe.onclick=()=>De("disable")}).then(async je=>{je===
...
thread_id:currentThreadId,message:C,model:get("model-select").value,image_urls:s,image_items:i,uploaded_image_urls:o,
...
he").checked:!1,marker_system_prompt:T,safety_setting:get("safety-setting").value,tts_voice:isTtsModel()&&
...
none shadow-md relative">${we}</div></div>`),resumeChatAutoScroll();const H=get(R);activeStreamingBubbleId=
R,canvasModeEnabled&&resetCanvasPreviewPanel();let Te=null;const He=a(O=>!Z||!H?null:((!Te||!H.contains(
Te))&&(Te=H.querySelector(".thought-content")),Te||(H.insertAdjacentHTML("afterbegin",'<div class="t\
...
data-placeholder="1"></div></div>'),Te=H.querySelector(".thought-content")),Te&&(Te.setAttribute("da\
ta-placeholder","1"),Te.textContent=O||"\u63A8\u8AD6\u30D7\u30ED\u30BB\u30B9\u3092\u6E96\u5099\u4E2D..."),
Te),"ensureThoughtPlaceholder");Z&&He("\u63A8\u8AD6\u30D7\u30ED\u30BB\u30B9\u3092\u6E96\u5099\u4E2D..."),
...
null;const It=a((O,ie)=>{if(!ie||O==="status"&&ft||O==="thought"&&at||O==="content"&&xt)return;const xe=Math.
...
body:JSON.stringify(j),signal:abortController.signal}),H);if(sendClientDebugLog("info",`Prompt strea\
...
!0,{persist:!1}),resetUploadState(),clearQuote();const ie=a(()=>{if(!H)return;const Ce=H.querySelector(
...
const Ne=H.querySelector(".content-area");Ne&&Ne.setAttribute("data-api-accepted","1"),updatePendingSkeletonStatus(
H,"\u63A5\u7D9A\u5B8C\u4E86\u3002\u30E2\u30C7\u30EB\u5FDC\u7B54\u3092\u5F85\u6A5F\u4E2D...","\u30AD\u30E5\u30FC\u5F85\u6A5F\u3084\u521D\
\u671F\u5316\u4E2D\u306E\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059")}},"markApiAccepted");ie();
const xe=O.body.getReader(),Le=new TextDecoder;let je="",De="",et="",Fe=!0,Pe=null,Oe=null,qe=null,Wt=!1;
...
let Ke=je.split(`
`);je=Ke.pop();let Vt=!1,sn=!1;for(let rt of Ke)if(rt.trim())try{const le=JSON.parse(rt);if(le.type===
...
(H.insertAdjacentHTML("afterbegin",'<div class="search-box visible animate-pulse mb-2"><i class="fas\
 fa-globe"></i> Searching web...</div>'),qe=H.querySelector(".search-box")):le.content==="done"&&qe&&
...
H,le.content||{});continue}if(le.type==="mcp_decision_request"){openMcpDecisionModal(le.content||{});
continue}if(le.type==="status"){ie();const ge=le.content===null||le.content===void 0?"":String(le.content);
if(It("status",!!ge),Fe&&H){const Ge=ge||"\u30E2\u30C7\u30EB\u51E6\u7406\u4E2D...";if(!updatePendingSkeletonStatus(
H,Ge,"\u5FDC\u7B54\u958B\u59CB\u307E\u3067\u306E\u9032\u6357\u3092\u8868\u793A\u3057\u3066\u3044\u307E\u3059")){
const Ue=H.querySelector(".content-area");Ue&&(Ue.outerHTML=buildPendingSkeletonHtml(j.model,Ge),updatePendingSkeletonStatus(
H,Ge,"\u5FDC\u7B54\u958B\u59CB\u307E\u3067\u306E\u9032\u6357\u3092\u8868\u793A\u3057\u3066\u3044\u307E\u3059"))}}
Z&&He(ge||"\u63A8\u8AD6\u30D7\u30ED\u30BB\u30B9\u3092\u6E96\u5099\u4E2D...");continue}if(Fe){beginPendingToStreamTransition(
H);const ge=H.querySelector(".content-area");ge&&(ge.innerHTML=""),Fe=!1}if(le.type==="coding_diff")
appendCodingLiveDiff(H,le.content||{}),It("content",!0);else if(le.type==="thought"){if(Pe||(Pe=H.querySelector(
...
"afterend",ge):H.insertAdjacentHTML("afterbegin",ge),Pe=H.querySelector(".thought-content")}if(Pe&&Pe.
...
mmer")}et=le.content}Pe.classList.remove("collapsed"),sn=!0}else if(le.type==="image_analysis"){const ge=le.
content===null||le.content===void 0?"":String(le.content);if(!H)continue;let Ge=H.querySelector(".im\
...
></div>';qe?qe.insertAdjacentHTML("afterend",Xe):H.insertAdjacentHTML("afterbegin",Xe),Ge=H.querySelector(
...
output"></code></pre></div></div></div>`;qe?qe.insertAdjacentHTML("afterend",Xe):H.insertAdjacentHTML(
"afterbegin",Xe),Ot[Ge]=H.querySelector(`[data-py-id="${Ge}"]`)}const Ue=Ot[Ge];if(Ue){if(ge.code!==
...
null||le.content===void 0?"":String(le.content);De+=ge,/[`~]/.test(ge)&&activateDeferredCodingModeFromStream(
De),Oe||(Oe=H.querySelector(".content-area")||document.createElement("div"),Oe.className="prose pros\
e-invert text-sm break-words",H.contains(Oe)||H.appendChild(Oe)),Vt=!0,It("content",!!ge)}else if(le.
type==="error"){Wt=!0,Nt=!0,H.insertAdjacentHTML("beforeend",buildChatErrorBubbleHtml(le.content)),showToast(
le.content||"Unknown error","error",!0);break}}catch{}if(sn&&Pe&&(Pe.textContent=et,userAutoScroll&&
(Pe.scrollTop=Pe.scrollHeight)),Vt&&Oe){const rt=Date.now();if(rt-Ze>100){const le=snapshotCodeCollapse(
Oe);renderAiMarkdownInto(Oe,De,{incrementalMath:!0}),applyCodeCollapse(Oe,le,!0),Ze=rt}}scrollToBottom()}
if(st&&st(),Oe){const Ce=snapshotCodeCollapse(Oe);renderAiMarkdownInto(Oe,De,{incrementalMath:!0}),applyCodeCollapse(
Oe,Ce,!0)}if(scrollToBottom(),vibrateHelper([100,50,100]),H)if(queueHighlight(H,De),enableLatencyMetrics){
...
get("model-select").value)}</div>`,Ne+="</div>",H.insertAdjacentHTML("beforeend",Ne)}else H.insertAdjacentHTML(
...
const Le=H&&H.closest(".fade-in");Le&&Le.remove(),delete messageStore[U],delete messageMeta[U]}if(O.
...
name!=="AbortError"){const Le="Connection Error: "+O.message;showToast(Le,"error",!0)}M&&!ie&&N()}finally{
...
includes("reasoning")&&!r.includes("non-reasoning");let d=null;const p=a(U=>!l||!s?null:((!d||!s.contains(
...
ceholder","1"),d.textContent=U||"\u63A8\u8AD6\u30D7\u30ED\u30BB\u30B9\u3092\u6E96\u5099\u4E2D..."),d),
"ensureThoughtPlaceholder");l&&p("\u63A8\u8AD6\u30D7\u30ED\u30BB\u30B9\u3092\u6E96\u5099\u4E2D...");
let g="",h="",y="",b=!0,w=null,x=null,C=null,T=!1;const M={};let F=0,se=!1;const K=window.ProgressSpinner?
...
window.ConnectionMonitor.reportActivity(),K&&K.setPhase("receiving"),g+=ue.decode(ce,{stream:!0});let _e=g.
split(`
`);g=_e.pop();let X=!1,ve=!1;for(let re of _e)if(re.trim())try{const j=JSON.parse(re);if(j.type==="j\
ob_id"){currentJobId=j.content||t;continue}if(j.type==="search_status"){j.content==="searching"&&!C?
(s.insertAdjacentHTML("afterbegin",'<div class="search-box visible animate-pulse mb-2"><i class="fas\
 fa-globe"></i> Searching web...</div>'),C=s.querySelector(".search-box")):j.content==="done"&&C&&(C.
...
s,j.content||{});else if(j.type==="thought"){if(w||(w=s.querySelector(".thought-content")),y+=j.content,
...
/div></div>';C?C.insertAdjacentHTML("afterend",L):s.insertAdjacentHTML("afterbegin",L),w=s.querySelector(
...
"thinking-shimmer")}y=j.content}w.classList.remove("collapsed"),ve=!0}else if(j.type==="image_analys\
...
iv></div>';C?C.insertAdjacentHTML("afterend",Y):s.insertAdjacentHTML("afterbegin",Y),R=s.querySelector(
".image-analysis-box")}const q=R.querySelector(".image-analysis-text");q&&(q.textContent=L)}else if(j.
type==="python"){const L=j.content||{},R=L.id||`py_${Date.now()}`;if(!M[R]){const Y=`<div class="cod\
...
/code></pre></div></div></div>`;C?C.insertAdjacentHTML("afterend",Y):s.insertAdjacentHTML("afterbegi\
n",Y),M[R]=s.querySelector(`[data-py-id="${R}"]`)}const q=M[R];if(q){if(L.code!==void 0){const Y=L.code==
...
h+=L,/[`~]/.test(L)&&activateDeferredCodingModeFromStream(h),x||(x=s.querySelector(".content-area")||
document.createElement("div"),x.className="prose prose-invert text-sm break-words",s.contains(x)||s.
appendChild(x)),X=!0}else if(j.type==="error"){T=!0,se=!0,s.insertAdjacentHTML("beforeend",buildChatErrorBubbleHtml(
j.content)),showToast(j.content||"Unknown error","error",!0);break}}catch{}if(ve&&w&&(w.textContent=
y,userAutoScroll&&(w.scrollTop=w.scrollHeight)),X&&x){const re=Date.now();if(re-F>100){const j=snapshotCodeCollapse(
x);renderAiMarkdownInto(x,h,{incrementalMath:!0}),applyCodeCollapse(x,j,!0),F=re}}scrollToBottom()}if(K&&
K(),x){const ae=snapshotCodeCollapse(x);renderAiMarkdownInto(x,h,{incrementalMath:!0}),applyCodeCollapse(
x,ae,!0)}vibrateHelper([100,50,100]),s&&queueHighlight(s,h),s&&s.querySelectorAll(".thought-content").
forEach(ce=>ce.classList.add("collapsed")),await loadMessages(currentThreadId,{preserveDraft:!0,silent:!0}),
loadThreads(!1)}catch(U){const Q=U.name==="AbortError"&&isManualStopAbortForThread(o);U.name==="Abor\
tError"&&!Q&&await syncThreadAfterAbortedStream(o,{retries:2,retryDelayMs:180,notifyOnFailure:!0}),Q||
...
const n=get(e);if(!n)return;const i=`${e}-pull-indicator`,s=60,o=88,r=52,l=.5,d=8;let p=0,g=!1,h=0,y=null;
const b=a(()=>get(i),"indicatorEl"),w=a(()=>{const T=b();return T?T.querySelector(".ptr-pull-label"):
null},"labelEl"),x=a(T=>{const M=b();if(!M)return;M.style.height=Math.min(T,o)+"px",M.classList.toggle(
"active",T>2),M.classList.toggle("pull-ready",T>=s);const F=w();F&&(F.textContent=T>=s?"\u96E2\u3057\u3066\u66F4\u65B0":
"\u5F15\u3063\u5F35\u3063\u3066\u66F4\u65B0")},"applyPullUI"),C=a(()=>{const T=b();T&&(T.style.height=
"0px",T.classList.remove("active","pull-ready","refreshing"),T.classList.remove("dragging"))},"reset\
PullUI");n.addEventListener("touchstart",T=>{if(y){g=!1;return}if(n.scrollTop>0){g=!1;return}const M=T.
touches[0];M&&(p=M.clientY,h=0,g=!0)},{passive:!0}),n.addEventListener("touchmove",T=>{if(!g||y)return;
if(n.scrollTop>0){g=!1;return}const M=T.touches[0];if(!M)return;const F=M.clientY-p;if(F<=0){h>0&&(h=
0,x(0)),g=!1;return}const se=b();se&&!se.classList.contains("dragging")&&se.classList.add("dragging"),
h=Math.min(F*l,o),x(h),F>=d&&T.preventDefault()},{passive:!1}),n.addEventListener("touchend",()=>{if(!g||
(g=!1,y))return;const T=b();T&&T.classList.remove("dragging");const M=h>=s;if(h=0,!M){C();return}let F;
...
ion"?(y=F,F.catch(()=>{}).finally(()=>{y=null,C()})):(y=Promise.resolve(),setTimeout(()=>{y=null,C()},
400))}),n.addEventListener("touchcancel",()=>{g=!1,h=0,C()})}a(initPullToRefresh,"initPullToRefresh");
...
{scroll:!1});const o=s?snapshotCodeCollapseByMessage(get("chat-container")):null;let r="",l="",d=[];
...
h.oldest_loaded_id||(allMessages.length?allMessages[0].id:null);const y=(allMessages||[]).filter(w=>w.
...
map(T=>T.id),C=x.length>1?{current:x.indexOf(b.id)+1,total:x.length,siblings:x}:null;renderMessage(b.
id,b.role,b.content,b.image_url,b.thought_data,b.model,C,n,b.quote_text,b.tokens,b.tokens_in,b.tokens_out,
b.is_encrypted,b.tokens_content,b.tokens_thought,h,!1,b.parent_id,b.gem_name)});const y=currentThreadPending;
...
job_id}`:null;renderPendingMessage(h,n,!1,T,y.model||null)}}if(s.appendChild(h),updateTotalTokenBar(
...
{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)});h=y.ok,sendClientDebugLog(
"info","POST request finished, status: "+y.status)}g.ok&&h?(window.closeThreadModal(),showToast("\u4FDD\u5B58\u3055\
...
</div>`:"",h=d.thought_text?`<div class="thought">${escapeHtml(d.thought_text)}</div>`:"",y=p?`<div \
...
void 0&&x.push(`parent:#${d.parent_id}`);const C=x.length?`<div class="message-meta">${pdfEscapeAttr(
x.join(" \u2022 "))}</div>`:"";return`
...
async()=>{try{const g=o.contentDocument,h=o.contentWindow;if(!g||!h){l(),showToast("PDF\u5370\u5237\u30E2\u30FC\u30C0\u30EB\u306E\u6E96\u5099\u306B\u5931\u6557\u3057\
...
images||[]),x=Promise.all(w.map(T=>T.complete?Promise.resolve():new Promise(M=>{T.addEventListener("\
load",M,{once:!0}),T.addEventListener("error",M,{once:!0})})));if(await Promise.race([x,new Promise(
T=>setTimeout(T,5e3))]),t.update(80),g.getElementById("MathJax-script")){let T=0;for(;T<100&&(!h.MathJax||
typeof h.MathJax.typesetPromise!="function");)await new Promise(M=>setTimeout(M,50)),T++;if(h.MathJax&&
typeof h.MathJax.typesetPromise=="function")try{await h.MathJax.typesetPromise()}catch(M){console.error(
"PDF MathJax typeset failed",M)}}t.update(95),setTimeout(()=>{try{h.focus(),h.addEventListener("afte\
...
try{const g=renderLibraryItem(p,l++);t.appendChild(g)}catch{}})}a(renderLibraryGrid,"renderLibraryGr\
...
e.filename)}</span></div>`;n.innerHTML=`<div class="lib-thumb-media-wrap">${o}</div>${r}${g}${h}`,n.
...
open-btn").forEach(x=>{x.onclick=C=>{C.stopPropagation(),e.type==="image"?openLibraryImage(e):openFileViewer(
...
(w.onclick=async x=>{x.stopPropagation(),w.disabled=!0;try{const C=await apiFetch(CHAT_CONFIG.urls.toggleFileFavorite,
{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({filepath:e.filepath})}),
T=await C.json().catch(()=>({}));if(!C.ok||typeof T.is_favorite!="boolean")throw new Error(T.error||
...
urls.getFilesLib;let o=null,r=!1;try{const d=getLibSortOrder(),p=getLibSearchQuery(),g=e?lib.nextOffset:
0,h=new URLSearchParams({limit:String(LIBRARY_PAGE_SIZE),offset:String(g),sort:d,q:p,favorites_only:lib.
favoritesOnly?"1":"0"}),y=await apiFetch(s+"?"+h.toString(),{cache:"no-store",headers:{Accept:"appli\
...
p=FILE_THUMB_BASE_URL,g=new Set(l.map(y=>y&&y.filepath).filter(Boolean));(!e&&Array.isArray(currentImageUrls)?
currentImageUrls:[]).forEach(y=>{if(l.length>=LIBRARY_PAGE_SIZE||!y||g.has(y))return;const b=getAttachmentNameForPath(
y)||y.split("/").pop()||y,w=(b.split(".").pop()||"").toLowerCase(),x=["png","jpg","jpeg","webp","gif"].
includes(w)?"image":"file",C=x==="image"?p+y:null;l.unshift({filename:b,original_filename:b,filepath:y,
url:d+y,thumbnail_url:C,type:x,ext:w,is_favorite:!1,ts:Math.floor(Date.now()/1e3)}),g.add(y)})}catch{}
...
h.className="w-px h-4 bg-gray-700",o.appendChild(h);const y=document.createElement("div");y.className=
...
n.id);o.innerHTML="",Object.entries(r).sort((d,p)=>p[1].total-d[1].total).forEach(([d,p])=>{const g=document.
...
div>
                ...
                `,o.appendChild(g)}),get("br-name-input").value=branchLabelNames[n.id]||"";const l=get(
...
()=>{o.onclick=null,r.onclick=null,l.onclick=null},"cleanup"),p=a(g=>{g.key==="Enter"&&(g.preventDefault(),
